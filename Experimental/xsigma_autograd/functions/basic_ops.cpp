...
variable_list Error::apply_with_saved(const variable_list& inputs, SwapSavedVariables& saved)
{
...
auto DelayedError::apply(variable_list&& inputs) -> variable_list
{
    ...
    for (auto& var : inputs)
    {
        ...
        outputs.emplace_back(var.defined() ? var.tensor_data() : quarisma::Tensor());
    }
    return wrap_outputs(
        inputs,
        std::move(outputs),
...
auto UndefinedGrad::apply(variable_list&& inputs) -> variable_list
{
    ...
    for (auto& var : inputs)
    {
        outputs.emplace_back(var.defined() ? var.clone().tensor_data() : quarisma::Tensor());
    }
    return wrap_outputs(
        inputs,
        std::move(outputs),
...
variable_list GraphRoot::apply_with_saved(const variable_list& inputs, SwapSavedVariables& saved)
{
