...
namespace
{
class XSIGMA_VISIBILITY TagToIOType
{
    ...
    const xsigma::RecordFunction& fn)
{
    ...
    auto [event, corr_id] =
        torch_ops_.op_events_.emplace_back(xsigma::profiler::impl::TorchOpBasicFields{
            fn.seqNr(),
            fn.forwardThreadId(),
            fn.scope(),
            fn.isAsync(),
...
{
template <typename T>
class XSIGMA_VISIBILITY StealOrDefault
{
...
class XSIGMA_VISIBILITY SubQueueThreadCache
{
    uint32_t             key_;
...
{
    return visit(xsigma::overloaded(
...
{
    auto end_time_ns = visit(xsigma::overloaded(
...
//   `xsigma::add`          (TorchOp, collected by profiler)
...
class XSIGMA_VISIBILITY ResultGreater
{
...
void build_tree(std::vector<std::shared_ptr<Result>>& sorted_events)
{
    ...
    auto push_event = [&stacks, &end_events_](std::shared_ptr<Result>& event)
    {
        ...
        {
            auto fwd_tid = event->visit(xsigma::overloaded(
