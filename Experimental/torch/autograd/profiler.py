        ...
        if not self.use_cpu:
            if not use_kineto:
                raise AssertionError(
                    "device_option-only events supported only with Kineto (use_kineto=True)"
                )
            ...
            if (
                ...
                for f_evt in device_corr_map[fe.id]:
                    if (
                        ...
                        # make sure that 'thread' of a CPU Kineto (e.g. device_option Runtime) event is associated
