...
public:
    ...
    bool hasHalf() const { return hasHalf_; }

    bool hasBFloat16() const { return hasBFloat16_; }
    ...
    void visit(const HalfImmPtr& v) override { hasHalf_ = true; }

    void visit(const BFloat16ImmPtr& v) override { hasBFloat16_ = true; }
...
class HalfRewriter : public IRMutator
{
    ExprPtr mutate(const LoadPtr& v) override
    {
        ...
        ExprPtr ret = alloc<Cast>(child->dtype().cloneWithScalarType(ScalarType::Float), child);
    ...
    StmtPtr mutate(const StorePtr& v) override
    {
        ...
        {
            new_val = alloc<Cast>(newType.cloneWithScalarType(bufType.scalar_type()), new_val);
            inserted_half_casts_.insert(new_val);
        }
    ...
    ExprPtr mutate(const HalfImmPtr& v) override { return alloc<Cast>(kFloat, v); }

    ExprPtr mutate(const BFloat16ImmPtr& v) override { return alloc<Cast>(kFloat, v); }
    ...
    ExprPtr mutate(const CastPtr& v) override
    {
        ...
        if (isHalf(v))
        {
            if (inserted_half_casts_.count(v) < 1)
            {
                ...
                v->set_dtype(v->dtype().cloneWithScalarType(quarisma::kFloat));
                return v;
            }
        }
        ...
        if (cast_child)
        {
            ...
            if (v->dtype().is_floating_point() && cast_child->dtype().is_floating_point() &&
                not_cast_half_to_doulbe)
            {
    ...
    ExprPtr mutate(const AddPtr& v) override { return mutateArithmetic(v); }
    ExprPtr mutate(const SubPtr& v) override { return mutateArithmetic(v); }
    ExprPtr mutate(const MulPtr& v) override { return mutateArithmetic(v); }
    ExprPtr mutate(const DivPtr& v) override { return mutateArithmetic(v); }
    ExprPtr mutate(const MaxPtr& v) override { return mutateArithmetic(v); }
    ExprPtr mutate(const MinPtr& v) override { return mutateArithmetic(v); }
    ExprPtr mutate(const CompareSelectPtr& v) override { return mutateArithmetic(v); }
    ExprPtr mutate(const BroadcastPtr& v) override { return mutateArithmetic(v); }
    ExprPtr mutate(const IfThenElsePtr& v) override { return mutateArithmetic(v); }
    ExprPtr mutate(const IntrinsicsPtr& v) override { return mutateArithmetic(v); }
...
private:
    ...
    static bool isHalf(const ExprPtr& v) { return isHalf(v->dtype().scalar_type()); }
