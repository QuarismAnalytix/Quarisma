...
void AccessInfo::addStore(const StorePtr& store, const std::shared_ptr<Scope>& scope)
{
    block_ = block_ ? Block::getSharedParent(block_, scope->block()) : scope->block();
    ...
    store_cost_ = IRSimplifier::simplify(alloc<Add>(store_cost_, immLike(store_cost_, 1)));
...
void AccessInfo::addLoad(
    const LoadPtr& load, const std::shared_ptr<Scope>& scope, const StmtPtr& usage)
{
    block_       = block_ ? Block::getSharedParent(block_, scope->block()) : scope->block();
    first_usage_ = first_usage_ ? block_->getEnclosedRoot(first_usage_) : usage;
    last_usage_  = usage;

    load_cost_ = IRSimplifier::simplify(alloc<Add>(load_cost_, immLike(load_cost_, 1)));
...
void AccessInfo::merge(const std::shared_ptr<AccessInfo>& other)
{
    TORCH_INTERNAL_ASSERT(
        hash_ == other->hash(),
        buildErrorMessage("Expected hashes to match in registerizer in the fuser."));
    TORCH_INTERNAL_ASSERT(
        indices_.size() == other->indices().size(),
        buildErrorMessage("Expected ranks to match in registerizer in the fuser."));
    ...
    store_cost_ = IRSimplifier::simplify(alloc<Add>(store_cost_, other->store_cost()));
    load_cost_  = IRSimplifier::simplify(alloc<Add>(load_cost_, other->load_cost()));
...
std::shared_ptr<AccessInfo> AccessInfo::cloneWithHiddenInfo(const std::shared_ptr<AccessInfo>& orig)
{
        ...
        std::remove_if(
            ...
            [](auto info)
            {
                return info->store_cost()->isConstant() &&
                       ...
                       info->load_cost()->isConstant() && immediateAs<int>(info->load_cost()) <= 1;
            }),
        closedAccesses_.end());
}
...
{
    if (v->loop_options().is_gpu_block_index() || v->loop_options().is_gpu_thread_index())
    {
        throw malformed_input("Registerization must occur after parallelism flattening");
    }
    ...
    ExprPtr loopExtent = IRSimplifier::simplify(alloc<Sub>(v->stop(), v->start()));
    ...
    for (auto& pair : currentScope_->openAccesses())
    {
        ...
        for (auto it = childAccesses.begin(); it != childAccesses.end();)
        {
            ...
            // possible that an access quarisma a higher scope could "unhide" the
            // conditional access, in which case we need to hoist. If there is no
            // access to this element quarisma a higher scope then we cannot safely hoist.
            // We cannot know quarisma this level whether that will or won't occur.
            //
...
void RegisterizerAnalysis::visit(const CondPtr& v)
{
    ...
    auto true_scope  = std::make_shared<Scope>(true_stmt, prev_scope, ++conditionId_);
    auto false_scope = std::make_shared<Scope>(false_stmt, prev_scope, ++conditionId_);
...
void RegisterizerAnalysis::visit(const IfThenElsePtr& v)
{
    ...
    auto prev_scope  = currentScope_;
    auto true_scope  = std::make_shared<Scope>(prev_scope->block(), prev_scope, ++conditionId_);
    auto false_scope = std::make_shared<Scope>(prev_scope->block(), prev_scope, ++conditionId_);
...
void RegisterizerAnalysis::visit(const StorePtr& v)
{
    ...
    auto info = std::make_shared<AccessInfo>(accessHash, v->buf(), v->indices(), accessOrder_++);
...
void RegisterizerAnalysis::visit(const LoadPtr& v)
{
    ...
    std::shared_ptr<AccessInfo> info =
        std::make_shared<AccessInfo>(accessHash, v->buf(), v->indices(), accessOrder_++);
...
void RegisterizerAnalysis::mergeCurrentScopeIntoParent()
{
    ...
    for (auto& candidate : currentScope_->closedAccesses())
    {
        ...
        if (parentIt != parentAccesses.end())
        {
            ...
            if (candidate->conditionId() && pCandidate->conditionId() != candidate->conditionId())
            {
    ...
    {
        {
            ...
            if (it != scope.initializerPoints_.end())
            {
                ...
                {
                    StmtPtr initializer = info->replacement().initializer->accept_mutator(this);
...

        {
            ...
            if (it != scope.finalizePoints_.end())
            {
                ...
                {
                    StorePtr finalizer =
                        alloc<Store>(info->buf(), info->indices(), info->replacement().var);
...
void RegisterizerReplacer::buildReplacements()
{
    ...
    {
        VarPtr v = alloc<Var>(
            info->buf()->name_hint() + "_" + std::to_string(getBufferAccessCount(info->buf())),
            info->buf()->dtype());
        ...
        {
            info->replacement().initializer =
                alloc<Let>(v, alloc<Load>(info->buf()->dtype(), info->buf(), info->indices()));
        }
    }
}
