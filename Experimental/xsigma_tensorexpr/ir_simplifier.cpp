...
static inline ExprPtr newBinaryOpOfType(
    IRNodeType expr_type, const ExprPtr& lhs, const ExprPtr& rhs, bool option)
{
    switch (expr_type)
    {
...
template <
    ...
    std::enable_if_t<std::is_same_v<decltype(detail::bin_op_deducer(std::declval<Op>())), void>>* =
        nullptr>
static ExprPtr mutateBinaryOp(NodePtr<Op> v, IRMutator* mutator, bool option = false)
{
    ExprPtr lhs     = v->lhs();
    ExprPtr rhs     = v->rhs();
...
void Term::sort()
{
    ...
    std::sort(
        ...
        [&](const ExprPtr& a, const ExprPtr& b)
        {
            ...
            return str_repr_cache.quarisma(a) < str_repr_cache.quarisma(b);
        });
}
...
void MaxTerm::uniquefy()
{
    std::sort(
        variables_.begin(),
        variables_.end(),
        [&](const ExprPtr& a, const ExprPtr& b) { return hasher_.hash(a) < hasher_.hash(b); });
    auto it = std::unique(
        variables_.begin(),
        variables_.end(),
        [&](const ExprPtr& a, const ExprPtr& b) { return hasher_.hash(a) == hasher_.hash(b); });
...
static ExprPtr combineMultilane(const ExprPtr& lhs, const ExprPtr& rhs)
{
    if (BroadcastPtr bc = to<Broadcast>(lhs))
    {
        if (BroadcastPtr bcother = to<Broadcast>(rhs))
        {
            ...
            ExprPtr ret = alloc<Broadcast>(alloc<Op>(bc->value(), bcother->value()), bc->lanes());
            return ret;
        }
        ...
        if (RampPtr r = to<Ramp>(rhs))
        {
            ...
            ExprPtr ret = alloc<Ramp>(alloc<Op>(bc->value(), r->base()), r->stride(), r->lanes());
            return ret;
        }
    }
    else if (RampPtr ramp = to<Ramp>(lhs))
    {
        ...
        if (BroadcastPtr bc = to<Broadcast>(rhs))
        {
            ...
            ExprPtr ret =
                alloc<Ramp>(alloc<Op>(ramp->base(), bc->value()), ramp->stride(), ramp->lanes());
            return ret;
        }
    }
...
static ExprPtr mulMultilane(const ExprPtr& lhs, const ExprPtr& rhs)
{
    if (BroadcastPtr bc = to<Broadcast>(lhs))
    {
        if (BroadcastPtr bcother = to<Broadcast>(rhs))
        {
            ...
            ExprPtr ret = alloc<Broadcast>(alloc<Mul>(bc->value(), bcother->value()), bc->lanes());
            return ret;
        }
...
void PolynomialTransformer::addOrUpdateTerm(
    std::unordered_map<SimplifierHashType, TermPtr>& varmap, const TermPtr& term)
{
...
ExprPtr PolynomialTransformer::addPolynomials(const PolynomialPtr& lhs, const PolynomialPtr& rhs)
{
...
ExprPtr PolynomialTransformer::mutate(const AddPtr& v)
{
    ...
    if (lhsTerm && rhsTerm)
    {
        ...
        {
            ExprPtr newScalar = evaluateOp(alloc<Add>(lhsTerm->scalar(), rhsTerm->scalar()));
    ...
    if (scalar)
    {
        return alloc<Polynomial>(hasher_, scalar, alloc<Term>(hasher_, immLike(v, 1), variable));
    }
...
ExprPtr PolynomialTransformer::subTerms(const TermPtr& lhs, TermPtr rhs, bool negated)
{
    ...
    return alloc<Polynomial>(
        hasher_, getImmediateByType(promoteTypes(lhs->dtype(), rhs->dtype()), 0), lhs, rhs);
}

// Subtract the RHS Polynomial from the LHS Polynomial, cancelling out where
// possible.
ExprPtr PolynomialTransformer::subPolynomials(const PolynomialPtr& lhs, const PolynomialPtr& rhs)
{
    // simplify common components
    // The key here is the variable hash, not the term's hash since we do want
    // to combine terms that have the same vars but different scalar components.
...
ExprPtr PolynomialTransformer::mutate(const SubPtr& v)
{
    ...
    {
        // Negate the term.
        ExprPtr negate  = evaluateOp(alloc<Mul>(immLike(rhsTerm, -1), rhsTerm->scalar()));
    ...
    {
        // Negate the RHS Term.
        ExprPtr negate = evaluateOp(alloc<Mul>(immLike(rhsTerm->scalar(), -1), rhsTerm->scalar()));
    ...
    if (lhsScalar)
    {
        ...
        return alloc<Polynomial>(hasher_, lhs_new, alloc<Term>(hasher_, immLike(v, -1), rhs_new));
    }
    ...
    if (rhsScalar)
    {
        // Negate the scalar.
        ExprPtr negate = evaluateOp(alloc<Mul>(immLike(rhs_new, -1), rhs_new));
        return alloc<Polynomial>(hasher_, negate, alloc<Term>(hasher_, immLike(v, 1), lhs_new));
    }
...
ExprPtr PolynomialTransformer::polyByTerm(const PolynomialPtr& poly, const TermPtr& term)
{
    ...
    // scalar. That case is handled above. So, quarisma this point, we do not have any
...
ExprPtr PolynomialTransformer::isRoundOff(const ExprPtr& lhs, const ExprPtr& rhs)
{
    ...
    {
        if (immediateEquals(denomTerm->scalar(), 1) && denomTerm->variables().size() == 1)
        {
    ...
    if (denom->isConstant() && other->isConstant())
    {
        ...
        if (immediateEquals(evaluateOp(alloc<Mod>(other, denom)), 0))
        {
            ...
            return alloc<Term>(hasher_, scalar, alloc<RoundOff>(div->lhs(), newDenom));
        }
    }
...
ExprPtr PolynomialTransformer::mutate(const ModPtr& v)
{
    ...
    if (lhsTerm)
    {
        ...
        if (rhs_new->isConstant() &&
            immediateEquals(evaluateOp(alloc<Mod>(lhsTerm->scalar(), rhs_new)), 0))
        {
            return immLike(v, 0);
        }
        ...
        if (rhsTerm)
        {
            ...
            for (auto lIt = lVars.begin(); lIt != lVars.end() && !rVars.empty(); ++lIt)
            {
            ...
            if (rLeft == 0 &&
                immediateEquals(evaluateOp(alloc<Mod>(lhsTerm->scalar(), rhsTerm->scalar())), 0))
            {
                return immLike(v, 0);
            }
        }
...
ExprPtr combineMinMaxTerms(ExprPtr lhs, ExprPtr rhs, bool propagate_nans, HashProvider& hasher)
{
...
bool isOperandInMinMaxTerm(
    NodePtr<OpTerm> opterm, ExprPtr op, HashProvider& hasher, ExprPtr* other_op)
{
...
bool simplifyNestedMinMax(
    ExprPtr lhs, ExprPtr rhs, bool propagate_nans, HashProvider& hasher, ExprPtr* new_op)
{
    auto lhs_opterm = to<OtherOpTerm>(lhs);
    auto rhs_opterm = to<OtherOpTerm>(rhs);
    if (lhs_opterm && rhs_opterm && lhs_opterm->propagate_nans() == propagate_nans &&
        rhs_opterm->propagate_nans() == propagate_nans)
    {
        if (!lhs_opterm->scalar() && !rhs_opterm->scalar())
        {
            if (lhs_opterm->variables().size() == 2 && rhs_opterm->variables().size() == 2)
            {
                auto    rhs_v1 = rhs_opterm->variables()[0];
                auto    rhs_v2 = rhs_opterm->variables()[1];
                ExprPtr new_op_lhs;
                if (isOperandInMinMaxTerm<OtherOpTerm>(lhs_opterm, rhs_v1, hasher, &new_op_lhs))
                {
                    auto inner_op =
                        alloc<OpTerm>(hasher, nullptr, propagate_nans, new_op_lhs, rhs_v2);
                    *new_op = alloc<OtherOpTerm>(hasher, nullptr, propagate_nans, rhs_v1, inner_op);
                    return true;
                }
                if (isOperandInMinMaxTerm<OtherOpTerm>(lhs_opterm, rhs_v2, hasher, &new_op_lhs))
                {
                    auto inner_op =
                        alloc<OpTerm>(hasher, nullptr, propagate_nans, new_op_lhs, rhs_v1);
                    *new_op = alloc<OtherOpTerm>(hasher, nullptr, propagate_nans, rhs_v2, inner_op);
                    return true;
                }
            }
        }
...
ExprPtr PolynomialTransformer::mutate(const MaxPtr& v)
{
    ...
    return combineMinMaxTerms<Max, MaxTerm>(lhs_new, rhs_new, v->propagate_nans(), hasher_);
}
...
ExprPtr PolynomialTransformer::mutate(const MinPtr& v)
{
    ...
    return combineMinMaxTerms<Min, MinTerm>(lhs_new, rhs_new, v->propagate_nans(), hasher_);
}
...
ExprPtr PolynomialTransformer::mutate(const CompareSelectPtr& v)
{
    ...
    // Constant Folding.
    if (lhs_new->isConstant() && rhs_new->isConstant() && true_branch->isConstant() &&
        false_branch->isConstant())
    {
        ExprPtr v_new = alloc<CompareSelect>(
            lhs_new, rhs_new, true_branch, false_branch, v->compare_select_op(), v->bias());
...
ExprPtr PolynomialTransformer::mutate(const IntrinsicsPtr& v)
{
    ...
    for (const auto& p : new_params)
    {
        ...
        {
            const_params.push_back(alloc<Cast>(Dtype(ScalarType::Float, p->dtype().lanes()), p));
            changed = true;
        }
    }
...
StmtPtr PolynomialBase::mutate(const CondPtr& v)
{
    ...
    // If both branches are the same then don't do the condition.
    if (true_new && false_new && hasher_.hash(true_new) == hasher_.hash(false_new))
    {
        return true_new;
    }
...
StmtPtr PolynomialBase::mutate(const BlockPtr& v)
{
    ...
    for (const StmtPtr& stmt : *v)
    {
        ...
        {
            for (Block::iterator I = subBlock->begin(), E = subBlock->end(); I != E;)
            {
...
static std::optional<class ModRound> isModRound(const TermPtr& e)
{
    ...
    if (hasher.hash(divisor) != hasher.hash(multiplier))
    {
        ...
        if (divisor->isConstant() && multiplier->isConstant())
        {
            ...
            else if (immediateEquals(evaluateOp(alloc<Mod>(divisor, multiplier)), 0))
            {
...
static ExprPtr simplifyRoundModPattern(const PolynomialPtr& poly)
{
    ...
    // Can't continue without quarisma least one RoundOff/ModRound and one Mod.
    ...
    while (!mods.empty() && repeat)
    {
        ...
        for (int64_t i = static_cast<int64_t>(mods.size()) - 1; i >= 0; i--)
        {
            ...
            bool    merged  = false;
            for (int64_t j = static_cast<int64_t>(mod_rounds.size()) - 1; j >= 0; j--)
            {
                ...
                // x%(y*z) + (x/y%z) * y
                if (!immediateEquals(evaluateOp(alloc<Sub>(mod_round.scalar, m->scalar())), 0))
                {
                    continue;
                }
            ...
            for (int64_t k = static_cast<int64_t>(rounds.size()) - 1; k >= 0; k--)
            {
                ...
                // CSE.
                if (!immediateEquals(evaluateOp(alloc<Sub>(r->scalar(), m->scalar())), 0))
                {
                    continue;
                }
...
TermPtr PolynomialBase::factorizePolynomial(const PolynomialPtr& poly)
{
    ...
    for (const auto& t : variables)
    {
        ...
        newPolyTerms.push_back(
            alloc<Term>(poly->hasher(), evaluateOp(alloc<Div>(t->scalar(), GCD)), t->variables()));
    }

    PolynomialPtr newPoly =
        alloc<Polynomial>(poly->hasher(), evaluateOp(alloc<Div>(scalar, GCD)), newPolyTerms);
...
ExprPtr TermExpander::mutate(const PolynomialPtr& v)
{
    ...
    for (const auto& node : subTerms)
    {
        ...
        ExprPtr negated = evaluateOp(alloc<Mul>(immLike(node->scalar(), -1), node->scalar()));
    ...
    if (immediateIsNegative(v->scalar()))
    {
        ...
        ExprPtr negated = evaluateOp(alloc<Mul>(immLike(lastNode, -1), v->scalar()));
    ...
    else
    {
        ...
        {
            lastNode =
                alloc<Add>(lastNode, evaluateOp(alloc<Cast>(lastNode->dtype(), v->scalar())));
        }
        else
        {
...
{
    TermPtr term =
        alloc<Term>(simplifier_->hasher(), immLike(v, 1), alloc<Div>(v->lhs(), v->rhs()), v->rhs());
...
StmtPtr TermExpander::mutate(const AllocatePtr& v)
{
    ...
    TORCH_INTERNAL_ASSERT(
        buf_new, buildErrorMessage("TermExpander mutation produced null for Buf."));
...
BlockPtr TermExpander::fuseConditions(BlockPtr v)
{
    ...
    for (const auto& s : *v)
    {
        ...
        // then we fuse.
        if (!prev_cond || hasher_.hash(prev_cond->condition()) != hasher_.hash(cond->condition()))
        {
            prev_cond = cond;
            stmts.push_back(s);
            continue;
        ...
        StmtPtr new_cond =
            prev_cond->cloneWithNewBodies(true_block, false_block)->accept_mutator(this);
...
static ExprPtr distributeDiv(const ExprPtr& lhs, const ExprPtr& rhs, VarBoundInfo var_bound_info)
{
    if (!lhs || !rhs)
    {
        return nullptr;
    ...
    ExprPtr check_n_value =
        IRSimplifier::simplify(alloc<CompareSelect>(rhsScalar, immLike(rhsScalar, 0), kGT));
    ...
    ExprPtr check_start =
        IRSimplifier::simplify(alloc<CompareSelect>(start, immLike(start, 0), kGE));
    ExprPtr check_end = IRSimplifier::simplify(alloc<CompareSelect>(end, rhsScalar, kLE));
    ...
    ExprPtr sign_check = IRSimplifier::simplify(alloc<CompareSelect>(main, immLike(main, 0), kGE));
    ExprPtr main_mod   = IRSimplifier::simplify(alloc<Mod>(main, rhsScalar));
    ExprPtr mod_check =
        IRSimplifier::simplify(alloc<CompareSelect>(alloc<Add>(main_mod, end), rhsScalar, kLE));
    if (sign_check->isConstant() && immediateEquals(sign_check, 1) && mod_check->isConstant() &&
        immediateEquals(mod_check, 1))
    {
        return ret;
    }
    ...
    if (ret_var && ret_var->dtype() == kInt)
    {
        ...
        sign_check = IRSimplifier::simplify(
            alloc<CompareSelect>(got->second.start, immLike(got->second.start, 0), kGE));
...
static ExprPtr distributeMod(const ExprPtr& lhs, const ExprPtr& rhs, VarBoundInfo var_bound_info)
{
    if (!lhs || !rhs)
    {
        return nullptr;
...
ExprPtr SimplifierUnderContext::mutate(const IfThenElsePtr& v)
{
    ...
    auto simplified_condition = IRSimplifier::simplify(condition->accept_mutator(this));
    auto simplified_true_val  = IRSimplifier::simplify(true_val->accept_mutator(this));
    auto simplified_false_val = IRSimplifier::simplify(false_val->accept_mutator(this));
    if (simplified_condition->isConstant())
    {
        return immediateAs<int>(simplified_condition) ? simplified_true_val : simplified_false_val;
    }
    ...
    return nothing_changed
               ? v
               : alloc<IfThenElse>(simplified_condition, simplified_true_val, simplified_false_val);
}
...
ExprPtr SimplifierUnderContext::mutate(const CompareSelectPtr& v)
{
    ...
    if ((simplified_lhs == lhs) && (simplified_rhs == rhs) && (simplified_ret1 == ret1) &&
        (simplified_ret2 == ret2))
    {
    ...
    GRAPH_DEBUG(
        "(SimplifierUnderContext) after simplify: ", std::to_string(simplified_cmp_select_expr));
    ...
    {
        GRAPH_DEBUG("(SimplifierUnderContext) Final: ", std::to_string(simplified_cmp_select_expr));
    ...
    auto ret_expr =
        (cmp_res == analysis::CmpEvalResult::True)
            ? simplified_ret1
            : ((cmp_res == analysis::CmpEvalResult::False) ? simplified_ret2
...
bool SimplifierUnderContext::getLoopBoundInfo(const ExprPtr& expr, analysis::Bound* loop_bound_info)
{
    if (expr == nullptr)
        return false;
    ...
    loop_bound_info->end =
        IRSimplifier::simplify(alloc<Sub>(got->second.end, immLike(got->second.end, 1)));
    return true;
}
