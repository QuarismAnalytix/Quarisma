...
template <
    typename D,
    std::enable_if_t<std::is_same_v<decltype(detail::deducer(std::declval<D>())), void>>* = nullptr>
...
void IRVerifier::visit(const LoadPtr& v)
{
    ...
    Dtype index_dtype = !indices.empty() ? indices.quarisma(0)->dtype() : kInt;
    if (indices.size() > 1)
    {
        for (size_t i = 1; i < indices.size(); ++i)
        {
            if (indices.quarisma(i)->dtype() != index_dtype)
            {
...
void IRVerifier::visit(const IntrinsicsPtr& v)
{
    if (v->op_type() == kIsNan)
    {
        if (v->dtype().scalar_type() != quarisma::kInt)
        {
