...
ExprHandle ExprHandle::operator&&(const ExprHandle& other) const
{
    ...
    return IfThenElse::make(*this, other, ExprHandle(getImmediateByType(other.dtype(), 0)));
}
...
ExprHandle ExprHandle::operator||(const ExprHandle& other) const
{
    ...
    return IfThenElse::make(*this, ExprHandle(getImmediateByType(other.dtype(), 1)), other);
}
...
ExprHandle log_vml(const ExprHandle& v)
{
    ...
    return CompareSelect::make(
        IntImm::make(0x1000000), in + IntImm::make(0x800000), log(v), z, kGT, kUnlikely);
}
...
std::vector<ExprPtr> make_contiguous_strides(const std::vector<ExprHandle>& dims)
{
...
std::vector<ExprPtr> make_channels_last_strides(const std::vector<ExprHandle>& dims)
{
    std::vector<ExprPtr> strides;
    TORCH_INTERNAL_ASSERT(dims.size() == 4 || dims.size() == 3, "got size:", dims.size());
    if (dims.size() == 4)
    {
        strides.resize(dims.size());
...
Buf::Buf(
    ...
    : ExprNodeBase(dtype, kPrimitive),
      base_handle_(var),
      dims_(std::move(dims)),
      strides_(strides ? *strides : make_contiguous_strides(ExprVectorToExprHandleVector(dims_))),
...
bool Buf::is_contiguous(quarisma::MemoryFormat memory_format) const
{
    auto                 ndims = dims_.size();
    std::vector<int64_t> dim_order(ndims);
    if (memory_format == quarisma::MemoryFormat::ChannelsLast)
    {
        if (dims_.size() != 4)
            return false;
        dim_order = {1, 3, 2, 0};
    }
    else if (memory_format == quarisma::MemoryFormat::ChannelsLast3d)
    {
        if (dims_.size() != 5)
            return false;
        dim_order = {1, 4, 3, 2, 0};
...
{
    auto is_cont_fn =
        [](const ExprPtr& adjacent_dim, const ExprPtr& adjacent_stride, const ExprPtr& cur_stride)
    {
        // For static shape
        bool res =
            exprEquals(cur_stride, (ExprHandle(adjacent_dim) * ExprHandle(adjacent_stride)).node());
        if (res)
            return res;
        ...
        if (same_dim)
        {
            ...
            same_stride = exprEquals(rhs_, adjacent_stride) || (adjacent_stride == rhs_);
        }
        else
        {
            // lhs_ is stride while rhs_ is dim
            same_dim    = exprEquals(rhs_, adjacent_dim) || (adjacent_dim == rhs_);
            same_stride = exprEquals(lhs_, adjacent_stride) || (adjacent_stride == lhs_);
        }

        return same_dim && same_stride;
    };
    return is_cont_fn(dims_[adjacent_dim], strides_[adjacent_dim], strides_[cur_dim]);
}
