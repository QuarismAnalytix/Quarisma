...
{
public:
    ExprPtr src_value() const { return src_value_; }

    void set_src_value(ExprPtr src_value) { src_value_ = std::move(src_value); }
...
{
    return BitCast::make(Dtype(ToDtype<T>(), src_value.dtype().lanes()), src_value);
}
...
public:
    ...
    void set_lhs(ExprPtr lhs) { lhs_ = std::move(lhs); }
...
{
public:
    Add(ExprPtr lhs, ExprPtr rhs) : BinaryOpNode(std::move(lhs), std::move(rhs), IRNodeType::kAdd)
    {
    }
};
...
{
public:
    Sub(ExprPtr lhs, ExprPtr rhs) : BinaryOpNode(std::move(lhs), std::move(rhs), IRNodeType::kSub)
    {
    }
};
...
{
public:
    Mul(ExprPtr lhs, ExprPtr rhs) : BinaryOpNode(std::move(lhs), std::move(rhs), IRNodeType::kMul)
    {
    }
};
...
{
public:
    Div(ExprPtr lhs, ExprPtr rhs) : BinaryOpNode(std::move(lhs), std::move(rhs), IRNodeType::kDiv)
    {
    }
};
...
{
public:
    Mod(ExprPtr lhs, ExprPtr rhs) : BinaryOpNode(std::move(lhs), std::move(rhs), IRNodeType::kMod)
    {
    }
};
...
{
public:
    ExprPtr base() const { return base_; }
    ExprPtr stride() const { return stride_; }

    void set_base(ExprPtr base) { base_ = std::move(base); }

    void set_stride(ExprPtr stride) { stride_ = std::move(stride); }

    static ExprHandle make(const ExprHandle& base, const ExprHandle& stride, int64_t lanes)
    {
...
public:
    ...
    static ExprHandle make(
        Dtype dtype, const BufHandle& buf, const std::vector<ExprHandle>& indices);
    static ExprHandle make(const BufHandle& buf, const std::vector<ExprHandle>& indices);
...
public:
    ...
    Broadcast(ExprPtr value, int64_t lanes)
        : ExprNodeBase(Dtype(value->dtype(), lanes)), value_(std::move(value)), lanes_(lanes)
    {
    }
...
public:
    ...
    ExprPtr true_value() const { return true_; }

    // Lazily evaluated only if condition is false
    ExprPtr false_value() const { return false_; }

    void set_condition(ExprPtr condition) { condition_ = std::move(condition); }

    void set_true_value(ExprPtr true_value) { true_ = std::move(true_value); }

    void set_false_value(ExprPtr false_value) { false_ = std::move(false_value); }
...
{
public:
    CompareSelectOperation compare_select_op() const { return compare_op_; }
    ExprPtr                lhs() const { return this->lhs_; }
    ExprPtr                rhs() const { return this->rhs_; }
    ExprPtr                ret_val1() const { return this->ret_val1_; }
    ExprPtr                ret_val2() const { return this->ret_val2_; }

    void set_lhs(ExprPtr lhs) { lhs_ = std::move(lhs); }

    void set_rhs(ExprPtr rhs) { rhs_ = std::move(rhs); }

    void set_ret_val1(ExprPtr ret_val1) { ret_val1_ = std::move(ret_val1); }

    void set_ret_val2(ExprPtr ret_val2) { ret_val2_ = std::move(ret_val2); }

    CompareSelectBias bias() const { return bias_; }
    ...
    CompareSelect(
        ExprPtr lhs, ExprPtr rhs, CompareSelectOperation cmp_op, CompareSelectBias bias = kUnbiased)
        : ExprNodeBase(kInt),
          lhs_(std::move(lhs)),
          rhs_(std::move(rhs)),
...
public:
    ...
    Intrinsics(IntrinsicsOp op_type, Dtype dtype)
        : ExprNodeBase(IntrinsicsDtype(op_type, dtype)), params_({}), op_type_(op_type)
    {
    ...
    Intrinsics(IntrinsicsOp op_type, const std::vector<ExprPtr>& params)
        : ExprNodeBase(IntrinsicsDtype(op_type, params)), params_(params), op_type_(op_type)
    {
    ...
    bool isPure() const { return op_type_ != kRand; }

    size_t nparams() const { return params_.size(); }

    ExprPtr                     param(size_t index) const { return params_[index]; }
    const std::vector<ExprPtr>& params() const { return params_; }

    void set_params(std::vector<ExprPtr> params) { params_ = std::move(params); }
...
private:
    ...
    static Dtype IntrinsicsDtype(IntrinsicsOp op_type, const std::vector<ExprPtr>& params);

    std::vector<ExprPtr> params_;
    IntrinsicsOp         op_type_;
};

TORCH_API std::vector<ExprPtr> ExprHandleVectorToExprVector(const std::vector<ExprHandle>& /*v*/);
TORCH_API std::vector<ExprHandle> ExprVectorToExprHandleVector(const std::vector<ExprPtr>& /*v*/);
TORCH_API std::vector<VarPtr> VarHandleVectorToVarVector(const std::vector<VarHandle>& /*v*/);
TORCH_API std::vector<VarHandle> VarVectorToVarHandleVector(const std::vector<VarPtr>& /*v*/);
