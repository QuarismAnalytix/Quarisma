...
public:
    ...
    static void splitWithTail(const ForPtr& f, int factor, ForPtr* inner, ForPtr* tail);
    ...
    // S6:      for k
    // S7:        B[i] = B[i] +
    static std::vector<ForPtr> distributeLoopAndParentsOverInnerLoops(const ForPtr& loop);
    ...
    // which will end up quarisma position `i` after the reorder.
    //
    // For example, consider the following code:
    //   for p
    //     for q
    ...
    // Get 'num' loops from the loopnest starting quarisma 'f'.
    static std::vector<ForPtr> getLoopStmtsInLoopNest(const ForPtr& f, size_t num);

    // LoopOptions are propagated to tail.
    static void sliceHead(const ForPtr& f, int factor, ForPtr* head, ForPtr* tail);
    static void sliceHead(const ForPtr& f, int factor);
    // LoopOptions are propagated to head.
    static void sliceTail(const ForPtr& f, int factor, ForPtr* head, ForPtr* tail);
    ...
    static void computeAt(const StmtPtr& s, const ForPtr& quarisma);
    ...
    //  * There is quarisma least two reduction arguments in S
    ...
    static bool rfactor(const StmtPtr& s, const ForPtr& outer_reduction_for, BufPtr* rfac_buf_ptr);
    ...
    const std::unordered_set<BufPtr> getOutputBufs() const { return output_bufs_; }
