...
public:
    ...
    StmtPtr get_parent() const { return parent_ ? parent_->getptr() : nullptr; }
...
protected:
    static void set_parent(const StmtPtr& s, Stmt* new_parent) { s->parent_ = new_parent; }
    std::shared_ptr<Stmt> getptr() { return shared_from_this(); }
...
public:
    ...
    void    accept(IRVisitor* visitor) override { visitor->visit(static_to<Op>(getptr())); }
...
public:
    ...
    size_t nstmts() const { return stmts_.size(); }
    ...
    void insert_stmt_before(const StmtPtr& s, const StmtPtr& before)
    {
        ...
        if (pos == stmts_.end())
        {
            throw malformed_input("Inserting after statement that is not in block", s);
        }
    ...
    bool replace_stmt(const StmtPtr& old_stmt, const StmtPtr& new_stmt)
    {
        ...
        {
            throw malformed_input("Block replace Stmt with existing parent", new_stmt);
        }
    ...
    std::list<StmtPtr> stmts() const { return stmts_; }
    ...
    explicit Block(const std::vector<StmtPtr>& stmts) { init(stmts); }

    typedef std::list<StmtPtr>::iterator       iterator;
    typedef std::list<StmtPtr>::const_iterator const_iterator;

    iterator begin() { return stmts_.begin(); }

    const_iterator begin() const { return stmts_.begin(); }

    iterator end() { return stmts_.end(); }

    const_iterator end() const { return stmts_.end(); }

    StmtPtr front() { return stmts_.front(); }

    StmtPtr front() const { return stmts_.front(); }

    StmtPtr back() { return stmts_.back(); }

    StmtPtr back() const { return stmts_.back(); }
...
{
public:
    VarPtr               base_handle() const { return buf_->base_handle(); }
    ...
    void set_buf(BufPtr buf) { buf_ = std::move(buf); }

    void set_indices(std::vector<ExprPtr> indices) { indices_ = std::move(indices); }

    void set_value(ExprPtr value) { value_ = std::move(value); }
...
// buffer var. The life span is quarisma most through the current program, until it is
...
public:
    ...
    VarPtr buffer_var() const { return buf_->base_handle(); }

    Dtype dtype() const { return buf_->dtype(); }

    const std::vector<ExprPtr> dims() const { return buf_->dims(); }
...
public:
    static PlacementAllocatePtr make(
        const BufHandle& buf_handle, const BufHandle& buf_handle_to_reuse)
    {
        return alloc<PlacementAllocate>(buf_handle.node(), buf_handle_to_reuse.node());
    }

    BufPtr buf() const { return buf_; }

    BufPtr buf_to_reuse() const { return buf_to_reuse_; }

    void set_buf(BufPtr buf) { buf_ = std::move(buf); }

    void set_buf_to_reuse(BufPtr buf) { buf_to_reuse_ = std::move(buf); }
...
{
public:
    static FreePtr make(const BufHandle& buf_handle) { return alloc<Free>(buf_handle.node()); }
...
public:
    ...
    std::vector<BufPtr> bufs() const { return bufs_; }

    void set_bufs(std::vector<BufPtr> bufs) { bufs_ = std::move(bufs); }
...
public:
    ...
    ExprPtr value() const { return val_; }

    void set_var(VarPtr var) { var_ = std::move(var); }

    void set_val(ExprPtr val) { val_ = std::move(val); }
...
public:
    ...
    ExprPtr condition() const { return condition_; }

    BlockPtr true_stmt() const { return true_stmt_; }

    BlockPtr false_stmt() const { return false_stmt_; }

    void set_condition(ExprPtr condition) { condition_ = std::move(condition); }
...
public:
    ...
    // GPU Block Index
    bool is_gpu_block_index() const { return gpu_block_index_ != IDX_UNSET; }

    int gpu_block_index() const { return gpu_block_index_; }
    ...
    // GPU Thread Index
    bool is_gpu_thread_index() const { return gpu_thread_index() != IDX_UNSET; }

    int gpu_thread_index() const { return gpu_thread_index_; }
    ...
    void set_parallel() { is_parallel_ = true; }
    ...
    {
        return gpu_block_index_ == IDX_UNSET && gpu_thread_index_ == IDX_UNSET && !is_parallel_;
    }
...
public:
    ...
    BlockPtr      body() const { return body_; }
    static ForPtr make(
        const VarHandle& var, const ExprHandle& start, const ExprHandle& stop, const StmtPtr& body)
    {
        if (!body)
        {
            return nullptr;
        ...
        const LoopOptions& loop_options)
    {
        ...
        return alloc<For>(var.node(), start.node(), stop.node(), body, loop_options);
    }
    const LoopOptions loop_options() const { return loop_options_; }
    ...
    For(VarPtr var, ExprPtr start, ExprPtr stop, StmtPtr body, LoopOptions loop_options)
        : var_(std::move(var)),
    ...
    void set_gpu_block_index(int block_index) { loop_options_.set_gpu_block_index(block_index); }
    ...
    void set_parallel() { loop_options_.set_parallel(); }

    bool is_parallel() const { return loop_options_.is_parallel(); }
    ...
    void set_start(ExprPtr start) { start_ = std::move(start); }

    void set_stop(ExprPtr stop) { stop_ = std::move(stop); }
...
public:
    ...
    std::vector<BufPtr> buf_args() const { return buf_args_; }

    std::vector<ExprPtr> args() const { return args_; }

    void set_buf(BufPtr buf) { buf_ = std::move(buf); }

    void set_buf_args(std::vector<BufPtr> buf_args) { buf_args_ = std::move(buf_args); }

    void set_args(std::vector<ExprPtr> args) { args_ = std::move(args); }

    ExternalCall(
        BufPtr buf, std::string func_name, std::vector<BufPtr> buf_args, std::vector<ExprPtr> args)
        : buf_(std::move(buf)),
...
public:
    ...
    std::vector<BufPtr> buf_out_args() const { return buf_out_args_; }
