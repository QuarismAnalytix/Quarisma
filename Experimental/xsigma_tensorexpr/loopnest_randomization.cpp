...
static std::vector<std::vector<ForPtr>> GetAllPerfectlyNestedLoopNests(std::vector<ForPtr> loops)
{
    ...
    for (size_t i = 1; i < loops.size(); i++)
    {
        ...
        if (last_loop->body()->nstmts() == 1 && last_loop->body()->front() == next_loop)
        {
...
static std::tuple<std::vector<T>, std::vector<int>> select_n_randomly(
    std::vector<T>& objects, int n, std::default_random_engine& random_engine)
{
...
{
    message =
        "Random Transform Sequence - Transformations[" + std::to_string(index) + "] = " + message;
    GRAPH_DEBUG(message);
}
...
static std::string join(const std::vector<std::string>& indices, char sep = ',')
{
    std::string s;
    for (const auto& index : indices)
    {
...
static std::string indexOf(const std::vector<T>& objects, const T& object)
{
    return std::to_string(
        std::distance(objects.begin(), std::find(objects.begin(), objects.end(), object)));
}
...
void loopnestRandomization(int64_t seed, LoopNest& l)
{
    ...
    int n_transforms =
        randomization_helper::max_transformations(std::rand() % max_allowed_transformations);
    std::string message;
    // clang-format off
  ...
  //       static void computeAt(StmtPtr s, ForPtr quarisma);
        ...
        for (int n_transform = 0; n_transform < n_transforms; n_transform++)
        {
            ...
            case COMPUTE_INLINE:
            {
                if (can_inline)
                {
                    ...
                    if (!bufs.empty())
                    {
                        ...
                        message        = "computeInline(" + bufs[buf_number]->name_hint() + ");\n";
                        randomization_helper::printHistory(n_transform, message);
            ...
            case INLINE_ALL:
            {
                if (can_inline)
                {
                    ...
                    message       = "inlineIntermediateBufs(" + std::to_string(allow_dup) + ");\n";
                    randomization_helper::printHistory(n_transform, message);
            ...
            case DIST1:
            {
                ...
                int                  loop_n = std::rand() % (int)loops.size();
                auto                 loop   = loops[loop_n];
                std::vector<StmtPtr> stmts(loop->body()->begin(), loop->body()->end());
                if (stmts.empty())
                {
                    break;
                }
                int n_pivots                  = (std::rand() % (int)stmts.size()) + 1;
                auto [pivots, chosen_indices] = randomization_helper::select_n_randomly<StmtPtr>(
                    stmts, n_pivots, random_engine);
                std::unordered_set<StmtPtr> pivots_set(pivots.begin(), pivots.end());
                message = "distributeLoop(loops[" + std::to_string(loop_n) + "], pivots=stmts(" +
                          randomization_helper::join(chosen_indices) + "))\n";
                randomization_helper::printHistory(n_transform, message);
            ...
            case DIST4:
            {
                ...
                message = "distributeLoopOverInnerLoops(loops[" + std::to_string(loop_n) + "])\n";
                randomization_helper::printHistory(n_transform, message);
            ...
            case DIST5:
            {
                ...
                message = "distributeLoopAndParentsOverInnerLoops(loops[" + std::to_string(loop_n) +
                          "])\n";
                randomization_helper::printHistory(n_transform, message);
            ...
            case FUSE_LOOPS:
            {
                ...
                int num_loops_to_fuse = std::max(2, (std::rand() % (int)loops.size()));

                auto [loops_to_fuse, chosen_indices] =
                    randomization_helper::select_n_randomly<ForPtr>(
                        loops, num_loops_to_fuse, random_engine);

                message = "fuseLoops(loops[" + randomization_helper::join(chosen_indices) +
                          "], &fused_loop);\n";
                randomization_helper::printHistory(n_transform, message);
                // Fuse the loops
                ForPtr fused_loop;
            ...
            case REORDER_AXIS:
            {
                ...
                for (const auto i : quarisma::irange(loops.size()))
                {
                    for (const auto j : quarisma::irange(i + 1, loops.size()))
                    {
                ...
                auto loop_pair    = valid_pairs.quarisma(valid_pair_n);
                auto first_loop   = std::get<0>(loop_pair);
                auto second_loop  = std::get<1>(loop_pair);

                std::string first_index  = randomization_helper::indexOf(loops, first_loop);
                std::string second_index = randomization_helper::indexOf(loops, second_loop);
            ...
            case REORDER:
            {
                ...
                auto all_nested_loops = randomization_helper::GetAllPerfectlyNestedLoopNests(loops);
                ...
                auto nested_loops = all_nested_loops.quarisma(index);
            ...
            case TILE:
            {
                ...
                int         index        = rand() % (int)all_nested_loops.size();
                auto const& nested_loops = all_nested_loops.quarisma(index);
                if (nested_loops.size() < 2)
                {
                    break;
                }
                int  loop_number = rand() % ((int)nested_loops.size() - 1);
                auto x_loop      = nested_loops.quarisma(loop_number);
                auto y_loop      = nested_loops.quarisma(loop_number + 1);
                ...
                std::string x_loop_index = randomization_helper::indexOf(loops, x_loop);
                std::string y_loop_index = randomization_helper::indexOf(loops, y_loop);
                message                  = "tile(loops[";
            ...
            case FLATTEN:
            {
                ...
                message = "flatten(loops[" + randomization_helper::join(indices) + "]);\n";
                randomization_helper::printHistory(n_transform, message);
                // flatten
            ...
            case COMPRESS_BUFFER:
            {
                ...
                message =
                    "compressBuffer(buffers[" + std::to_string(buffer_n) + "], l.root_stmt());\n";
                randomization_helper::printHistory(n_transform, message);
            ...
            case COMPUTE_AT:
            {
                // To find valid compute quarisma pairs, we need to collect the producer
                ...
                auto pc_pair = producer_consumer_pairs.quarisma(pair_n);
