...
CmpEvalResult TORCH_API
compareBound(const Bound& a, const Bound& b, const CompareSelectOperation& cmp_op)
{
    switch (cmp_op)
    {
    case CompareSelectOperation::kGT:
        return (a > b) ? CmpEvalResult::True
...
std::vector<Bound> subtractBound(const Bound& a, const Bound& b)
{
    ...
    if (hasHead)
    {
        res.emplace_back(a.start, IRSimplifier::simplify(alloc<Sub>(b.start, immLike(b.start, 1))));
    }

    if (hasTail)
    {
        ExprPtr tailStart = IRSimplifier::simplify(alloc<Add>(b.end, immLike(b.end, 1)));
...
std::vector<IndexBounds> subtractIndicesBounds(
    const IndexBounds& A, const IndexBounds& B, OverlapKind overlap)
{
    if (overlap == OverlapKind::NoOverlap)
    {
        return {A};
    ...
    for (size_t i = 0; i < A.size(); ++i)
    {
        ...
        for (const auto& slice : slices)
        {
            ...
            TORCH_INTERNAL_ASSERT(remainingOuterBounds.size() == i, buildErrorMessage());
...
std::vector<IndexBounds> TORCH_API subtractIndicesBounds(const IndexBounds& A, const IndexBounds& B)
{
