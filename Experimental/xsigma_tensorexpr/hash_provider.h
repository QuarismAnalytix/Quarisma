...
{
    size_t operator()(const torch::jit::tensorexpr::SimplifierHashType& k) const { return k._h; }
};
...
public:
    ...
    bool cachedHash(const ExprPtr& e) { return exprToHash_.find(e) != exprToHash_.end(); }
    bool cachedHash(const StmtPtr& s) { return stmtToHash_.find(s) != stmtToHash_.end(); }
...
private:
    ...
    {
        seed._h ^= te_hash(val.ToCppString()) + 0x1f752c19 + (seed._h << 7) + (seed._h >> 4);
    }
    ...
    void _hash_combine(SimplifierHashType& seed, const T& val, const Types&... args)
    {
    ...
    size_t te_hash(SimplifierHashType val) { return val._h; }
