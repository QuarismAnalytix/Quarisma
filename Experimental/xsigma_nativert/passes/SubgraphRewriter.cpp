...
bool compareConstants(const Constant& a, const Constant& b)
{
    return std::visit(
        [](const auto& lhs, const auto& rhs) -> bool
        {
            ...
            if constexpr (
                std::is_same_v<LType, RType> && !std::is_same_v<LType, std::unique_ptr<Graph>>)
            {
                return lhs == rhs;
            }
...
auto findMatchingAttribute(const Node* target_node, const Attribute& attr)
{
    return std::find_if(
        ...
        { return attr.name == otherAttr.name && compareConstants(attr.value, otherAttr.value); });
}
...
auto findInputByName(const Node* pattern_node, const std::string& inputName)
{
    return std::find_if(
        ...
        [&](const NamedArgument& patternInput) { return inputName == patternInput.name; });
}
}  // namespace
...
bool SubgraphMatcher::tryMatchNodeInputs(const Node* pattern_node, Node* target_node, Match& match)
{
    ...
    for (const auto i : quarisma::irange(target_node->numInputs()))
    {
        ...
        const auto& inputMatch = findInputByName(pattern_node, target_node->inputs()[i].name);
    ...
    for (const auto i : quarisma::irange(target_node->attributes().size()))
    {
        ...
        Value* dummyOutput =
            dummyNode->addOutput(targetGraph->getUniqueValueName(), Type::Kind::None);
        targetGraph->insertBefore(dummyNode, target_node);
        if (match.value_map.find(patternInput->value) != match.value_map.end())
        {
            return match.value_map[patternInput->value]->producer()->target() == kDummyTarget;
        }
...
bool SubgraphMatcher::tryMatchNode(const Node* pattern_node, Node* target_node, Match& match)
{
    ...
    int64_t deltaAttributesCount = static_cast<int64_t>(pattern_node->attributes().size()) -
...
bool SubgraphMatcher::tryMatchValue(const Value* pval, Value* tval, Match& match)
{
    ...
    if (pval->users().size() != tval->users().size() && pProducer->target() != "prim.Input" &&
        !isOutputValue(pval))
    {
        return false;
    }
...
bool SubgraphRewriter::run(Graph* graph, const std::vector<MatchFilter>& filters)
{
    bool mutated = false;
    for (const auto& [pattern, replacement] : patterns_)
    {
        ...
        mutated |= runForPattern(graph, *pattern_graph.get(), *replacement_graph.get(), filters);
    }
    return mutated;
}
    ...
    const std::vector<MatchFilter>& filters)
{
    ...
    VLOG(1) << "[GraphPasses] Found " << matches.size() << " matches for : " << name_;
    ...
    for (auto& m : matches)
    {
        if (!std::all_of(
                filters.begin(),
                filters.end(),
                [&](const MatchFilter& f) { return f(m, getVmap(pattern)); }))
        {
            continue;
        }
    ...
    bool mutated = (valuesToRewrite_.size() + valueRewrites_.size() + replacedNodes_.size()) > 0;
...
bool SubgraphRewriter::overlapsWithUsedNodes(
    const Match& match, const std::unordered_set<Node*>& usedNodes)
{
