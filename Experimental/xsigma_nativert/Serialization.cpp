...
Value* symbolicToValue(const torch::_export::Argument& arg, Graph& graph, Node* insertBefore)
{
    switch (arg.tag())
    {
    ...
    case torch::_export::Argument::Tag::AS_TENSORS:
    {
        ...
        auto listPack = graph.createListPack(std::move(listValue), Type::Kind::Tensor);
    ...
    case torch::_export::Argument::Tag::AS_OPTIONAL_TENSORS:
    {
        ...
        for (const auto& listEl : arg.get_as_optional_tensors())
        {
            ...
            {
                listValue.push_back(graph.getValue(listEl.get_as_tensor().get_name()));
                break;
            }
            case torch::_export::OptionalTensorArgument::Tag::AS_NONE:
            {
                listValue.push_back(graph.addValue(std::nullopt, Type::Kind::None, nullptr));
                break;
            }
            default:
                QUARISMA_CHECK(
    ...
    case torch::_export::Argument::Tag::AS_SYM_INTS:
    {
        ...
        auto listPack = graph.createListPack(std::move(listValue), Type::Kind::SymInt);
        return graph.insertBefore(listPack, insertBefore)->outputs()[0];
    }
...
std::pair<std::vector<torch::_export::InputSpec>, std::vector<torch::_export::Argument>>
enforceInputOrder(
    ...
    bool                                  loadNodeMetadata)
{
    ...
    for (const auto& jsonNode : jsonGraph.get_nodes())
    {
        ...
        for (const auto& output : jsonNode.get_outputs())
        {
            ...
            case torch::_export::Argument::Tag::AS_TENSORS:
            {
                ...
                Node* listUnpack = graph->insertNode("prim.ListUnpack", {{"input", outputValue}});
    ...
    for (const auto& output : jsonGraph.get_outputs())
    {
        ...
        if (isSymbolic(output))
        {
            ...
            case torch::_export::Argument::Tag::AS_SYM_INT:
            {
                ...
                QUARISMA_CHECK(asSymInt.tag() == torch::_export::SymIntArgument::Tag::AS_NAME);
    ...
    for (const auto& [inputName, weightName] : graph->signature().inputsToWeights())
    {
...
Constant constantToValue(const torch::_export::Argument& jsonArg, bool loadNodeMetadata)
{
    switch (jsonArg.tag())
    {
    case torch::_export::Argument::Tag::AS_NONE:
    ...
    {
        return jsonToSubgraph(*jsonArg.get_as_graph().get_graph(), nullptr, loadNodeMetadata);
    }
    case torch::_export::Argument::Tag::AS_TENSOR:
    case torch::_export::Argument::Tag::AS_TENSORS:
    case torch::_export::Argument::Tag::AS_OPTIONAL_TENSORS:
        QUARISMA_CHECK(false, "Tensor values are symbolic, not constant.");
    case torch::_export::Argument::Tag::AS_SYM_INT:
    case torch::_export::Argument::Tag::AS_SYM_INTS:
    case torch::_export::Argument::Tag::AS_SYM_BOOL:
    case torch::_export::Argument::Tag::AS_SYM_BOOLS:
        QUARISMA_CHECK(false, "Symint/Symbool Values are symbolic, not constant.");
    case torch::_export::Argument::Tag::AS_CUSTOM_OBJ:
        QUARISMA_CHECK(false, "custom obj is symbolic, not constant");
    case torch::_export::Argument::Tag::AS_OPERATOR:
        return jsonArg.get_as_operator();
    case torch::_export::Argument::Tag::AS_SYM_FLOAT:
    {
        QUARISMA_CHECK(false, "SymFloat is not yet implemented");
    }
    case torch::_export::Argument::Tag::AS_SYM_FLOATS:
    {
        QUARISMA_CHECK(false, "SymFloats is not yet implemented");
    }
    default:
        QUARISMA_CHECK(false, "Got unknown json argument");
    }
}

std::unique_ptr<Graph> jsonToGraph(
    const torch::_export::GraphModule& jsonGraphModule, bool loadNodeMetadata)
{
    auto graph = jsonToSubgraph(
        jsonGraphModule.get_graph(), &jsonGraphModule.get_signature(), loadNodeMetadata);
    return graph;
}
