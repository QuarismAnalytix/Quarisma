...
{
    if (ivalue.isInt() || ivalue.isNone() || ivalue.isBool() || ivalue.isDouble() ||
        ivalue.isComplexDouble() || ivalue.isString() || ivalue.isDevice() || ivalue.isEnum())
    {
        return true;
    }
    if (ivalue.isTensor())
    ...
    if (ivalue.isList() || ivalue.isTuple())
    {
        ...
        return std::all_of(
            elems.begin(),
            elems.end(),
            [](const IValue& tup_elem) { return insertableIValue(tup_elem); });
    }
...
Value* insertConstant(
    Graph& g, const IValue& val, std::optional<SourceRange> loc, std::optional<ScopePtr> scope)
{
    ...
    Graph& g, const IValue& val, std::optional<SourceRange> loc, std::optional<ScopePtr> scope)
{
    ...
    {
        bool fast_path_list = val.isBoolList() || val.isIntList() || val.isDoubleList();
    ...
    else if (val.isObject())
    {
        ...
        if (!ref.type()->is_module() &&
            (ref.is_weak_compilation_ref() || ref.is_empty_strong_compilation_ref()))
        {
            n->ival_(attr::value, val);
            n->output()->setType(val.type());
        }
...
std::optional<IValue> toIValue(const Value* v)
{
    ...
    else if (type->isSubtypeOf(*NumberType::get()) && node->kindOf(attr::value) == AttributeKind::i)
    {
        return node->i(attr::value);
    }
    else if (type->isSubtypeOf(*NumberType::get()) && node->kindOf(attr::value) == AttributeKind::f)
    {
        return node->f(attr::value);
    }
    else if (type->isSubtypeOf(*NumberType::get()) && node->kindOf(attr::value) == AttributeKind::c)
    {
        return node->c(attr::value);
    }
    else if (type->cast<ListType>() && node->kindOf(attr::value) == AttributeKind::ival)
    {
        const auto& list = node->ival(attr::value);
        TORCH_INTERNAL_ASSERT(list.isList());
        return list;
    }
    else if (type->cast<DictType>() && node->kindOf(attr::value) == AttributeKind::ival)
    {
        const auto& dict = node->ival(attr::value);
        TORCH_INTERNAL_ASSERT(dict.isGenericDict());
        return dict;
    }
    else if (type->cast<TupleType>() && node->kindOf(attr::value) == AttributeKind::ival)
    {
