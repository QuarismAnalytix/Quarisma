...
static inline const char* toString(AttributeKind kind)
{
    ...
    static constexpr const char* names[] = {
        "f", "c", "cs", "fs", "i", "is", "s", "ss", "t", "ts", "g", "gs", "ty", "tys", "ival"};
...
struct ScalarAttributeValue : public AttributeValue
{
    ...
    Ptr           clone() const override { return Ptr(new ScalarAttributeValue(name, value_)); }
...
struct TORCH_API GraphAttr : public AttributeValue
{
    ...
    GraphAttr(Symbol name, ConstructorType value_) : AttributeValue(name), value_(std::move(value_))
    {
    }
    ValueType&    value() { return value_; }
    Ptr           clone() const override;
    AttributeKind kind() const override { return AttributeKind::g; }
...
struct TORCH_API GraphsAttr : public AttributeValue
{
    ...
    ValueType&                      value() { return value_; }
    AttributeKind                   kind() const override { return AttributeKind::gs; }
    std::unique_ptr<AttributeValue> clone() const override;
