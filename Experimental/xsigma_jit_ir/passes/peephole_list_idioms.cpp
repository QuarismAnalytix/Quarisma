...
{
    ListLenRefiner(std::shared_ptr<Graph> graph, std::unordered_set<Value*>& mutated_lists)
    ...
    void collectListsToRefine(Block* b, std::unordered_set<Value*>& li_with_len_use)
    {
        for (Node* n : b->nodes())
        {
            ...
            if (first_input->type()->castRaw<ListType>() && !mutated_lists_.count(first_input))
            {
        ...
        for (Node* n : b->nodes())
        {
                ...
                n->matches("aten::ne(int a, int b) -> bool"))
            {
                ...
                for (size_t const_index : {0, 1})
                {
                    ...
                    boolean_value_refinements_[n->output()] =
                        n->kind() == aten::eq
            ...
            else if (n->kind() == prim::If)
            {
                ...
                auto           true_block_refinements = RefineListLens(
                    if_n.thenBlock(),
                    has_cond_ref ? boolean_value_refinements_[if_n.cond()].true_refine() : empty);
                auto false_block_refinements = RefineListLens(
                    if_n.elseBlock(),
                    has_cond_ref ? boolean_value_refinements_[if_n.cond()].false_refine() : empty);
            ...
            {
                handleCommonRefinentOperators(n, throwing_blocks_, boolean_value_refinements_);
            }
        }
...
{
    PeepholeOptimizeListIdiomsImpl(std::shared_ptr<Graph> graph, bool refine_list_len)
...
private:
    ...
    bool optimizeSlice(Node* slice_node, Node* list_construct_node)
    {
        ...
        if (start_val == std::nullopt || end_val == std::nullopt || step_val == std::nullopt)
        {
            return false;
        }

        int64_t start =
            start_val->isInt() ? start_val->to<int64_t>() : std::numeric_limits<int64_t>::max();
        int64_t end =
            end_val->isInt() ? end_val->to<int64_t>() : std::numeric_limits<int64_t>::max();
        ...
        auto slice_list_construct = graph_->insertNode(graph_->create(prim::ListConstruct));
    ...
    bool runBlock(Block* block)
    {
        ...
        for (Node* node : block->nodes())
        {
            ...
            if (node->inputs().empty() || !node->input(0)->type()->castRaw<ListType>())
            {
                continue;
            }
            ...
            else if (node->kind() == aten::__getitem__)
            {
                if (auto index = toIValue(node->input(1)))
                {
                    ...
                    {
                        node->output()->replaceAllUsesWith(list_creation_node->input(*norm_index));
                        changed = true;
                    }
                }
            }
...
bool PeepholeOptimizeListIdioms(const std::shared_ptr<Graph>& graph, bool refine_list_len)
{
