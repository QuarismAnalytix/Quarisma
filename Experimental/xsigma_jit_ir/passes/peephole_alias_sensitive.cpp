...
{
    PeepholeOptimizeAliasSensitiveImpl(std::shared_ptr<Graph> graph, bool shape_peepholes)
    ...
    bool run() { return runBlock(graph_->block()); }
    ...
    bool runBlock(Block* block)
    {
        ...
        for (Node* node : block->nodes())
        {
                ...
                node->kind() == aten::conv3d)
            {
                ...
                auto    kind        = node->kind();
                int64_t output_size = kind == aten::conv1d ? 3 : (kind == aten::conv2d ? 4 : 5);
...
bool PeepholeOptimizeAliasSensitive(const std::shared_ptr<Graph>& graph, bool shape_peepholes)
{
