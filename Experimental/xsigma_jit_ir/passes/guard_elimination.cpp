...
struct GuardElimination
{
    ...
    bool moveGuardsToDefs(Block* b)
    {
        ...
        if (b->owningNode() &&
            isLoweredGradOf(b->owningNode()) /*b->owningNode()->kind() == prim::If*/)
        {
            for (auto it = b->nodes().begin(); it != b->nodes().end();)
            {
                auto block_node = *it++;
        ...
        for (auto it = b->nodes().begin(); it != b->nodes().end(); it++)
        {
            ...
            if (n->kind() == prim::Guard)
            {
                ...
                for (auto use : users)
                {
                    ...
                    {
                        GRAPH_UPDATE("Removing dominated guard ", user, " and replacing with ", n);
    ...
    bool guardsOutput(Node* guard)
    {
        ...
        while (it != output)
        {
            ...
            {
                GRAPH_DEBUG(
                    "found an unexpected node ", *it, " while trying to eliminate ", *guard);
                return false;
            }
            it = it->prev();
        }
    ...
    bool checkInputs(Node* n, const std::unordered_set<size_t>& except, bool allow_numbers)
    {
        ...
        for (auto input : n->inputs())
        {
            ...
            {
                AT_ASSERT(
                    input->node()->kind() != prim::Guard || input->type()->expect<TensorType>());
            }
            else
            {
                GRAPH_DEBUG("input ", input->debugName(), " isn't guarded, type ", *input->type());
        ...
        case aten::_grad_sum_to_size:
            ...
            if (checkInputs(n, std::unordered_set<size_t>{1}, false))
            {
                ...
                else if (asize->matches("aten::size(Tensor self) -> int[]"))
                {
                    ...
                    if (asize->input()->type()->expectRef<TensorType>().sizes().concrete_sizes())
                    {
                        return true;
                    }
                }
