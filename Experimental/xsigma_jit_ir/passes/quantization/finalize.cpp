...
{
    std::vector<QuantFusionInfo> patterns_and_replacements = linear_prepack_unpack_patterns();
...
{
    std::vector<QuantFusionInfo> patterns_and_replacements = conv_prepack_unpack_patterns();
...
void removePackedParamInsertionAndFPWeightsSetAttr(
    std::shared_ptr<Graph>& g, const std::unordered_set<std::string>& packed_param_attr_names)
{
        ...
        if (n->kind() == prim::CallMethod)
        {
            ...
            if (attr_name == "calculate_qparams")
            {
                ...
                if (observer_node->kind() == prim::GetAttr &&
                    observer_node->s(attr::name).find("_observer_") != std::string::npos)
                {
                    nodes_to_delete.push_back(n);
                }
            }
...
Module Finalize(
    Module& module, QuantType quant_type, const std::vector<std::string>& preserved_attrs)
{
...
Module FinalizeOnDevicePTQ(Module& module, QuantType quant_type, const std::string& method_name)
{
    // Tracing annotates the resulting graph with shape information. In many case,
    // user applies different input shapes to traced graph. It is on the user to
    // know it is correct to do so. The quantized module needs to be clean up and
    ...
    removePackedParamInsertionAndFPWeightsSetAttr(quantized_graph, packed_param_attr_names);
