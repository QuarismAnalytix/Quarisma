...
{
    explicit LivenessAnalyzer(std::shared_ptr<Graph> graph) : graph_(std::move(graph)) {}
    ...
    void dump(const std::unordered_map<Node*, std::vector<Value*>>& liveness_sets)
    {
...
private:
    SparseBitVector toSparseBitVector(quarisma::ArrayRef<Value*> values)
    {
        SparseBitVector sbv;
        for (auto v : values)
        {
