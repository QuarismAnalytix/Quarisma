...
struct WorkBlock : public std::pair<Node*, Node*>
{
    ...
    Node* begin() { return this->first; }
    Node* end() { return this->second; }
};
...
public:
    ...
    void buildupSubgraphs()
    {
        ...
        for (Node* n : block_->nodes())
        {
            ...
            {
                SubgraphSlicer(subBlock, graph_, minSubgraphSize_, aliasDb_, diff_nodes_)
                    .buildupSubgraphs();
            }
        }
    }
...
private:
        ...
        while (any_changed)
        {
            ...
            for (auto n : b->nodes().reverse())
            {
                if (n->kind() == prim::DifferentiableGraph)
                {
                    ...
                    GRAPH_DEBUG(
                        "any_changed on ", any_changed, " ", n->g(attr::Subgraph)->toString(false));
                }
            }
        }
    ...
    std::optional<Node*> tryMerge(Node* consumer, Node* producer)
    {
        ...
        SubgraphUtils::mergeNodeIntoSubgraphAndUpdateAliasing(producer, consumer, aliasDb_);
        return consumer;
    }
...
struct ContextMapping
{
    ...
    const Node* get(const Node* n) const
    {
        ...
        TORCH_INTERNAL_ASSERT(
            it != node_to_ctx_.end(), "Cannot find node in node-to-context mapping.");
        return it->second;
    }

    bool has(const Node* n) const { return node_to_ctx_.find(n) != node_to_ctx_.end(); }
};

std::optional<bool> findRequiresGradForOutput(
    Node* diff_graph, Value* output, const ContextMapping& ctx_mapping)
{
    for (auto& use : output->uses())
    {
        ...
        if (ctx_mapping.has(use.user) && ctx_mapping.get(use.user) != ctx_mapping.get(diff_graph))
        {
            continue;
        }
...
void AddRequiresGradToDifferentiableGraph(Node* diff_graph, const ContextMapping& ctx_mapping)
{
    ...
    for (auto i : quarisma::irange(subgraph->outputs().size()))
    {
        ...
        output->setType(output->type()->expectRef<TensorType>().withRequiresGrad(requires_grad));
    }
}

void AddRequiresGradOnOutputNodes(Block* block, const ContextMapping& ctx_mapping)
{
    for (Node* n : block->nodes())
    {
        if (n->kind() == prim::DifferentiableGraph)
