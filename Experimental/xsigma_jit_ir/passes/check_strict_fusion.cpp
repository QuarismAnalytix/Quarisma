...
bool isStrictFusion(Value* value)
{
    ...
    return class_name.has_value() && (*class_name == "__torch__.torch.jit.strict_fusion");
}
...
static std::unordered_set<Node*> collectValuesUsedInGuard(Node* guarding_if, Node* enter_node)
{
    // DFS to collect
...
static void checkForUnfusedOps(Node* enter_node)
{
    ...
    for (Node* node = enter_node->next(); node->kind() != prim::Exit; node = node->next())
    {
        if (node->kind() == prim::If && fusionGuardCheck(node->input()->node()->kind()))
        {
    ...
    if (guarding_ifs.size() == 1)
    {
        guarding_check_nodes = collectValuesUsedInGuard(guarding_ifs[0], enter_node);
    }
