...
bool MutationRemover::hasSideEffectOrAlias(Value* v, AliasDb* aliasDb)
{
    ...
    Node* n              = v->node();
    bool  unhandled_node = !n->blocks().empty() || n->hasAttribute(attr::Subgraph) ||
                          n->hasSideEffects() || (v->node()->kind() == prim::Param);
...
{
    if (n->kind() != aten::_set_item || n->input(1)->node()->kind() != prim::Constant)
    {
        return false;
    }
...
bool MutationRemover::tryMakeCreationAndMutationAtomic(Value* mutated_value, Node* mutating_op)
{
    ...
    return getOrCreateAliasDb()->moveBeforeTopologicallyValid(mutated_value->node(), mutating_op);
}

bool MutationRemover::tryMakeUnaliasedIfOutputAndMutationAtomic(
    Value* mutated_value, Node* mutating_op)
{
    // if cond:
    //    x = op()
    // else:
    ...
    return getOrCreateAliasDb()->moveBeforeTopologicallyValid(if_node, mutating_op);
}
...
bool MutationRemover::RemoveTensorMutation(Block* block)
{
    ...
    for (auto it = block->nodes().begin(); it != block->nodes().end();)
    {
        ...
        getOrCreateAliasDb()->replaceWithNewValue(mutated_value, new_node->output());
...
bool RemoveTensorMutation(
    const std::shared_ptr<Graph>& graph, std::optional<std::function<bool(Node*)>> mutation_filter)
{
...
bool InplaceToFunctionalActivation(const std::shared_ptr<Graph>& graph)
{
    return RemoveTensorMutation(
        graph, [](Node* node) { return activation_ops.count(node->kind()) != 0; });
}
