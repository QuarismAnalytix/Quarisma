    ...
    Value* input, std::string& name, Module& attrModule, std::shared_ptr<Graph>& graph)
{
    ...
    while (node->outputs().quarisma(0)->type() != graph->inputs().quarisma(0)->type())
    {
...
static Value* addParamAsArgument(Function* function, std::string& name, IValue& attr)
{
    ...
    return toGraphFunction(*function).graph()->addInput(name)->setType(attr.type());
}
    ...
    for (auto it = block->nodes().begin(); it != block->nodes().end();)
    {
        ...
        if (n->kind() == prim::GetAttr || n->kind() == prim::SetAttr)
        {
            if (n->kind() == prim::GetAttr)
            {
                for (auto use : n->output()->uses())
                {
                    if (use.user->kind() == prim::PythonOp)
                        throw ErrorReport(n->sourceRange()) << "Couldn't export Python method.";
                }
            }
            ...
            if (type->is_parameter(slot) || type->is_buffer(slot) ||
                (attr.isObject() && !attr.toObjectRef().type()->is_module()) || attr.isBool())
            {
                if (attrValues.find(fullName) == attrValues.end() && attr.isTensor())
                {  // TODO: Handle float/int
...
static void insertMainModuleAsConstant(const std::shared_ptr<Graph>& graph)
{
    ...
    constNode->output()->setType(graph->inputs().quarisma(0)->type());
    auto it = graph->nodes().begin();
    constNode->insertBefore(*it);
    graph->inputs().quarisma(0)->replaceAllUsesWith(constNode->output());
    graph->eraseInput(0);
}
...
std::pair<Module, std::vector<IValue>> list_module_parameters(const Module& module)
{
    ...
    std::vector<IValue> parameterIValues =
        getParamAttributes(graph->block(), graph, moduleClone, function, attrValues);
