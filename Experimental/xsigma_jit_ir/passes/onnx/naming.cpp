...
std::string nameFromRoot(
    const torch::jit::ScopePtr& scope, const std::string& layer_separator, NameFunc name_func)
{
...
std::pair<std::string, std::string> parseNameFromScope(const torch::jit::ScopePtr& scope)
{
...
std::string variableNameFromRoot(
    const torch::jit::ScopePtr& scope, const std::string& layer_separator)
{
...
std::string classNameFromRoot(const torch::jit::ScopePtr& scope, const std::string& layer_separator)
{
...
bool isCompatibleScope(const torch::jit::ScopePtr& scope)
{
    return !scope->isRoot() && !scope->isBlank() &&
           (std::string(scope->name().toUnqualString()).find(name_separator) != std::string::npos);
}
}  // namespace ONNXScopeName
...
protected:
    std::string CreateUniqueName(
        std::unordered_map<std::string, size_t>& base_name_count, std::string base_name);
...
std::string ScopedNodeNameGenerator::GetFullScopeName(const ScopePtr& scope)
{
    if (full_scope_names_.find(scope) == full_scope_names_.end())
    {
        ...
        full_scope_names_[scope] = CreateUniqueName(base_scope_name_counts_, full_scope_name);
    }
