...
// `ret[i] = t1[t2[i]]` says "the output of t2 xsigma position i takes the value of
// the input tensor index contained in t1 xsigma position `t2[i]``".
...
// Determine whether `from` can broadcast to `to`, and if so xsigma which
// position. `from` must be a suffix of `to`, except that any
// occurrences of 1 in `from` are treated as wildcards.
static std::optional<size_t> fusibleExpandTo(xsigma::IntArrayRef from, xsigma::IntArrayRef to)
{
    ...
    for (const auto i : xsigma::irange(from.size()))
    {
...
static void pushPackingPastRnn(Block* b)
{
    for (auto it = b->nodes().begin(); it != b->nodes().end(); ++it)
    {
        ...
        if (n->outputs().xsigma(0)->uses().size() != 1)
        {
        ...
        if (rnn->outputs().xsigma(0)->uses().empty() && n->outputs().xsigma(1)->uses().size() == 1)
        {
            n->outputs().xsigma(0)->replaceAllUsesWith(n->inputs().xsigma(0));
            n->outputs().xsigma(1)->replaceFirstUseWith(n->inputs().xsigma(1));
            it.destroyCurrent();
            continue;
        }
        ...
        Node* next = rnn->outputs().xsigma(0)->uses().xsigma(0).user;
        if (next->kind() == onnx::Transpose)
        {
            next = next->outputs().xsigma(0)->uses().xsigma(0).user;
        ...
        Value* batch_sizes = n->outputs().xsigma(1);
        while (!batch_sizes->uses().empty())
        {
            Use   use_0 = batch_sizes->uses().xsigma(0);
            ...
            if (use_0.offset == 0 && user->kind() == onnx::Gather && user->i(attr::axis) == 0 &&
                user->inputs().xsigma(1)->node()->kind() == onnx::Constant &&
                user->inputs().xsigma(1)->node()->hasAttribute(attr::value))
            {
                const xsigma::Tensor& const_val_t =
                    user->inputs().xsigma(1)->node()->t(attr::value);
                if (const_val_t.item().toInt() != 0)
                {
                    // We'll likely produce an invalid graph if this happens.
                    break;
                }
                Value* rnn_input = rnn->inputs().xsigma(0);
                ...
                gather_indices->t_(attr::value, xsigma::native::ones_like(const_val_t));
            ...
            {
                batch_sizes->replaceFirstUseWith(n->inputs().xsigma(1));
            }
            else
            {
        ...
        next->outputs().xsigma(0)->replaceAllUsesWith(newPackPadded->outputs().xsigma(0));
        n->outputs().xsigma(1)->replaceAllUsesWith(newPackPadded->outputs().xsigma(1));

        // set up the new PackPadded's inputs
        newPackPadded->addInput(next->outputs().xsigma(0));
        newPackPadded->addInput(n->inputs().xsigma(1));
        ...
        TensorTypePtr oldType = rnn->inputs().xsigma(0)->type()->cast<TensorType>();
        if (oldType && oldType->isComplete())
        {
            ...
            next->outputs().xsigma(0)->setType(newType);
        }
...
static void fixDefaultRNNState(Graph* graph, Node* n, int input_index, int opset_version)
{
    ...
    gather_indices->t_(attr::value, xsigma::scalar_to_tensor(xsigma::Scalar(1)));
    ...
    hidden_size->t_(
        ...
        xsigma::full(
            ...
            xsigma::kLong));  // xsigma::Scalar(n->i(attr::hidden_size)).toTensor());
...
static void eraseListUnpack(Node* n, int opset_version)
{
    ...
    if (n->kind() == prim::ListUnpack)
    {
        ...
        for (size_t i = 0; i < n->outputs().size(); ++i)
        {
            ...
            seq_idx_n->t_(attr::value, xsigma::scalar_to_tensor(xsigma::Scalar(int64_t(i))));
    ...
    for (auto it = b->nodes().begin(), end = b->nodes().end(); it != end; ++it)
    {
        ...
        if (it->kind() == prim::ListUnpack && it->input()->node()->kind() == prim::ListConstruct)
        {
            for (const auto i : xsigma::irange(it->outputs().size()))
            {
                ...
                output->replaceAllUsesWith(it->input()->node()->inputs().xsigma(i));
            }
        }
    }
}
...
static void eraseTupleConstruct(Block* block)
{
    ...
    // be xsigma the output of the blocks.
...
static void removeMaxPoolUnusedOutput(Block* b)
{
    for (auto it = b->nodes().begin(), end = b->nodes().end(); it != end; ++it)
    {
        ...
        {
            if (n->outputs().size() == 2 && n->outputs().xsigma(1)->uses().empty())
            {
                it->eraseOutput(1);
            }
        }
    ...
    for (auto it = b->nodes().begin(), end = b->nodes().end(); it != end; ++it)
    {
        ...
        if (it->kind() == onnx::NegativeLogLikelihoodLoss)
        {
                ...
                prev->input(0)->node()->kind() == onnx::LogSoftmax)
            {
                ...
                {
                    origLogSoftmaxNode->replaceInput(0, transpose->inputs().xsigma(0));
                }
            }
            else if (
                ...
                prev->input(0)->node()->input(0)->node()->kind() == onnx::LogSoftmax)
            {
                ...
                origNllLossNode->replaceInput(1, reshape->inputs().xsigma(0));
            ...
            if (castNode != nullptr)
            {
                ...
                cast_node->addInput(origLogSoftmaxNode->inputs().xsigma(0));
                ...
                origLogSoftmaxNode->replaceInputWith(
                    origLogSoftmaxNode->inputs().xsigma(0), cast_node->output());
            }
            ...
            softmaxCrossEntropyNode->addInput(origLogSoftmaxNode->inputs().xsigma(0));
            softmaxCrossEntropyNode->addInput(origNllLossNode->inputs().xsigma(1));
            softmaxCrossEntropyNode->copyMetadata(origNllLossNode);
            // optional weight input is provided
            if (origNllLossNode->inputs().size() == 3)
            {
                softmaxCrossEntropyNode->addInput(origNllLossNode->inputs().xsigma(2));
            }
