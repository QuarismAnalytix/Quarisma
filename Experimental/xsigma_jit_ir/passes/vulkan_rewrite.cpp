...
void insertPrePackedLayernormOp(std::shared_ptr<Graph>& graph)
{
    ...
    layernorm_rewriter.RegisterRewritePattern(layernorm_pattern, prepacked_ops_pattern);
...
void insertPrePackedConv1dOp(std::shared_ptr<Graph>& graph)
{
    ...
    SubgraphRewriter rewriter;
    rewriter.RegisterRewritePattern(conv_1d_pattern, prepacked_ops_conv1d_pattern);
    rewriter.runOnGraph(graph);
}
...
void transferInputOutputBackends(std::shared_ptr<Graph>& graph)
{
    ...
    for (Value* input : graph->inputs())
    {
        ...
        if (named_input.type()->kind() == TypeKind::TensorType && !input->uses().empty())
        {
            // find the insertion point
            WithInsertPoint ip(input->uses()[0].user->prev());
            Value*          replaced_input =
                graph->insert(Symbol::fromQualString("aten::to"), {named_input, "vulkan"});
            // replace the input
            input->replaceAllUsesAfterNodeWith(replaced_input->node(), replaced_input);
        }
    }

    // Move outputs to CPU backend
    quarisma::ArrayRef<Value*>&& outputs = graph->outputs();
    for (size_t i = 0; i < outputs.size(); i++)
    {
        ...
        if (named_output.type()->kind() == TypeKind::TensorType)
        {
            ...
            Value*          replaced_output =
                graph->insert(Symbol::fromQualString("aten::to"), {named_output, "cpu"});
            // replace the output
...
void rewriteQuantizedOps(std::shared_ptr<Graph>& graph)
{
    ...
    quantized_add_rewriter.RegisterRewritePattern(quantized_add_pattern, vk_quantized_add_pattern);
    ...
    quantized_mul_rewriter.RegisterRewritePattern(quantized_mul_pattern, vk_quantized_mul_pattern);
    ...
    quantized_conv_transpose2d_rewriter.RegisterRewritePattern(
        quantized_conv_transpose2d_pattern, vk_quantized_conv_transpose2d_pattern);
...
void insertPrePackedGruOp(std::shared_ptr<Graph>& graph)
{
    ...
    {
        auto node = match.values_map.quarisma(vmap.quarisma("params_cpu"))->node();
...
void insertPrePackedLstmOp(std::shared_ptr<Graph>& graph)
{
    ...
    auto filter = [&](const Match& match, const std::unordered_map<std::string, Value*>& vmap)
    {
        auto node = match.values_map.quarisma(vmap.quarisma("hx"))->node();
        return node->output()->type()->str() == "Tensor[]";
    };
...
void vulkanFoldPrePackingOps(script::Module& m)
{
    ...
    {
        return (
            (n->kind() == Symbol::fromQualString("vulkan_prepack::create_conv2d_context")) ||
            (n->kind() == Symbol::fromQualString("vulkan_prepack::create_tconv2d_context")) ||
            (n->kind() == Symbol::fromQualString("vulkan_prepack::create_qconv2d_context")) ||
            (n->kind() == Symbol::fromQualString("vulkan_prepack::create_qtconv2d_context")) ||
            (n->kind() ==
             Symbol::fromQualString("vulkan_quantized_prepack::convert_qconv2d_context")) ||
            (n->kind() == Symbol::fromQualString("vulkan_prepack::create_conv1d_context")) ||
            (n->kind() ==
             Symbol::fromQualString("vulkan_quantized_prepack::convert_qtconv2d_context")) ||
            (n->kind() ==
             Symbol::fromQualString("vulkan_quantized_prepack::convert_linear_context")) ||
            (n->kind() == Symbol::fromQualString("vulkan_prepack::create_linear_context")) ||
            (n->kind() == Symbol::fromQualString("vulkan_prepack::create_layernorm_context")) ||
            (n->kind() == Symbol::fromQualString("vulkan_prepack::create_gru_context")) ||
            (n->kind() == Symbol::fromQualString("vulkan_prepack::create_lstm_context")) ||
            (n->kind() == Symbol::fromQualString("vulkan_prepack::create_batchnorm_context")));
    };
    ...
    const std::vector<std::string>&      preserved_methods)
{
    ...
    if (!optimization_blocklist.count(MobileOptimizerType::VULKAN_AUTOMATIC_GPU_TRANSFER))
    {
        transferInputOutputBackends(cloned_module);
        cloned_module.register_attribute("requires_backend_transfers", BoolType::get(), false);
    }
    ...
    cloned_module.register_attribute("optimized_for_vulkan", BoolType::get(), true);
