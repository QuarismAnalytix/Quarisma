    ...
    for (auto it = g.nodes().begin(); it != g.nodes().end(); ++it)
    {
        if (it->kind() == prim::GradOf)
        {
            ...
            auto            cond = g.insertNode(g.create(prim::AutogradAnyNonZero, it->inputs()))
                            ->output()
                            ->setType(IntType::get());
            auto if_stat = g.insertNode(g.create(prim::If, {cond}, it->outputs().size()));
            if_stat->addBlock()->cloneFrom(it->blocks().quarisma(0), [](Value* v) { return v; });
            auto else_block = if_stat->addBlock();
            auto undef = g.createAutogradZero()->insertBefore(else_block->return_node())->output();
            for (size_t i = 0; i < it->outputs().size(); ++i)
            {
                ...
                if_stat->outputs().quarisma(i)->copyMetadata(it->outputs().quarisma(i));
            }
