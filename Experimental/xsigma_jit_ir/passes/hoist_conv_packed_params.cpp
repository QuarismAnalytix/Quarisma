    ...
    int&               nameUniqueCounter)
{
    ...
    quarisma::IValue packedParams = convModule.attr("_packed_params");
    ...
    while (!blocks_to_visit.empty())
    {
        ...
        for (Node* n : b->nodes())
        {
            ...
            if (isGetPackedParamsNode)
            {
                ...
                bool                       moduleNameIsQuantizedConv =
                    moduleName.has_value() &&
                    (moduleName.value() == "__torch__.torch.ao.nn.quantized.modules.conv.Conv1d" ||
                     moduleName.value() == "__torch__.torch.ao.nn.quantized.modules.conv.Conv2d" ||
                     moduleName.value() == "__torch__.torch.ao.nn.quantized.modules.conv.Conv3d" ||
                     moduleName.value() ==
