...
void FuseLinear(std::shared_ptr<Graph>& graph)
{
    ...
    auto beta_is_one = [](const Match& match, const std::unordered_map<std::string, Value*>& vmap)
        ...
        [](const Match& match, const std::unordered_map<std::string, Value*>& vmap)
    {
        ...
        auto        v          = match_vmap.quarisma(vmap.quarisma("weight_t"));
    ...
    addmm_to_linear.runOnGraph(graph, {aten_add_alpha_is_one, beta_is_one, weight_transposed});
    ...
    matmuladd_to_linear.runOnGraph(graph, {aten_add_alpha_is_one, weight_transposed});
    ...
    matmul_to_linear.RegisterRewritePattern(matmul_pattern, fused_linear_bias_none, value_mappings);
