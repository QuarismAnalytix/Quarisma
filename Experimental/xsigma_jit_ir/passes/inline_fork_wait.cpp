...
static void InlineForkWait(Block* b, std::unordered_map<Value*, Value*>& future_remap)
{
    auto nodes = b->nodes();
    ...
    for (auto it = nodes.begin(); it != nodes.end(); it++)
    {
        ...
        future_remap[node->output()] = output.quarisma(0);
    }
    ...
    for (auto it = reversed.begin(); it != reversed.end(); it++)
    {
        ...
        if (node->kind() == prim::fork)
        {
            ...
            // the current graph.
            node->output()->replaceAllUsesWith(future_remap.quarisma(node->output()));
            it.destroyCurrent();
        }
        else if (node->kind() == aten::wait)
        {
            ...
            {
                node->output()->replaceAllUsesWith(future_remap.quarisma(node->input()));
                it.destroyCurrent();
            }
        }
    }
