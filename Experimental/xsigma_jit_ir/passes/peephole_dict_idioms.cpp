...
public:
    ...
    bool canOptimize() { return !has_overlap_ && !has_non_const_key_; }
...
{
public:
    DictNodeImpl(std::function<KeyType(const IValue&)> ivalue_converter, Node* dict_creation_node)
...
public:
    explicit DictNode(Node* dict_creation_node)
    {
        ...
        {
            auto ivalue_converter = [](const IValue& ival) { return ival.toDouble(); };
        ...
        {
            auto ivalue_converter = [](const IValue& ival) { return *ival.toString(); };
...
private:
    ...
    const DictNode& getDictNode(Node* creation_node)
    {
        ...
        {
            cached = dict_cache_.emplace(creation_node, DictNode(creation_node)).first;
        }
