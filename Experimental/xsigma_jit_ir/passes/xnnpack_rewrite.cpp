...
void replaceConv1dWithConv2d(std::shared_ptr<Graph>& graph)
{
    ...
    rewriter.RegisterRewritePattern(conv_1d_pattern, conv_2d_pattern, value_mappings);
...
void insertPrePackedLinearOp(std::shared_ptr<Graph>& graph)
{
    ...
    linear_rewriter.RegisterRewritePattern(linear_pattern, prepacked_ops_pattern, value_mappings);
...
void insertPrePackedConv2dOp(std::shared_ptr<Graph>& graph)
{
    ...
    SubgraphRewriter rewriter;
    rewriter.RegisterRewritePattern(conv_2d_pattern, prepacked_ops_conv2d_pattern, value_mappings);
    rewriter.runOnGraph(graph);
    ...
    value_mappings = {{"output_min_max", "res"}, {"packed_weight_bias", "res"}, {"res", "res"}};

    SubgraphRewriter transpose_rewriter;
    transpose_rewriter.RegisterRewritePattern(
        conv_2d_transpose_pattern, prepacked_ops_conv2d_transpose_pattern, value_mappings);
...
void fuseHardtanhWithPackedOps(std::shared_ptr<Graph>& graph)
{
    ...
    rewriter.RegisterRewritePattern(
        linear_prepack_run_hardtanh, linear_prepack_run_hardtanh_fused, value_mappings);
    ...
    value_mappings = {{"packed_weight_bias", "packed_weight_bias"}, {"res", "res"}};

    rewriter.RegisterRewritePattern(
        conv2d_prepack_run_hardtanh, conv2d_prepack_run_hardtanh_fused, value_mappings);
    ...
    rewriter.RegisterRewritePattern(
        linear_prepack_run_hardtanh_inplace, linear_prepack_run_hardtanh_fused, value_mappings);

    value_mappings = {{"packed_weight_bias", "packed_weight_bias"}, {"res", "res"}};

    rewriter.RegisterRewritePattern(
        conv2d_prepack_run_hardtanh_inplace, conv2d_prepack_run_hardtanh_fused, value_mappings);
...
void fuseReluWithPackedOps(std::shared_ptr<Graph>& graph)
{
    ...
    rewriter.RegisterRewritePattern(
        linear_prepack_run_relu_inplace, linear_prepack_run_relu_fused, value_mappings);
...
void FoldPrePackingOps(script::Module& m)
{
    ...
    {
        return (
            (n->kind() == Symbol::fromQualString("prepacked::linear_clamp_prepack")) ||
            n->kind() == Symbol::fromQualString("prepacked::conv2d_clamp_prepack") ||
            n->kind() == Symbol::fromQualString("prepacked::conv2d_transpose_clamp_prepack"));
    };
    ...
    const std::vector<std::string>&      preserved_methods)
{
    ...
    if (!optimization_blocklist.count(MobileOptimizerType::INSERT_FOLD_PREPACK_OPS))
    {
    ...
    if (!optimization_blocklist.count(MobileOptimizerType::HOIST_CONV_PACKED_PARAMS) &&
...
void insertPrePackedOps(std::shared_ptr<Graph>& graph)
{
    TORCH_INTERNAL_ASSERT(false, "XNNPACK is not enabled. Please build with USE_XNNPACK=1");
}
