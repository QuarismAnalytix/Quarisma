...
std::vector<size_t> identifyListArgIndices(const quarisma::FunctionSchema& schema)
{
...
{
public:
    VariadicUpdater(std::shared_ptr<Graph> graph, NodeKind op, NodeKind variadic_op)
        : graph_(std::move(graph)), alias_db_(graph_), op_(op), variadic_op_(variadic_op)
    {
    }
...
private:
    void recordSchema(Node* op_node)
    {
        ...
        {
            schema_to_list_indices_.emplace(schema.overload_name(), identifyListArgIndices(schema));
        }
    }
    ...
    const std::vector<size_t>& getListIndices(Node* op_node) const
    {
        ...
        QUARISMA_CHECK(it != schema_to_list_indices_.end());
        return it->second;
    }
    ...
    bool allListInputsAreValid(Node* op_node)
    {
        ...
        {
            QUARISMA_CHECK(list_idx < num_inputs);
        ...
        std::vector<Value*>& inputs, Node* node, size_t start_idx, size_t end_idx) const
    {
        ...
        QUARISMA_CHECK(start_idx <= end_idx && end_idx <= num_inputs);
        inputs.insert(
    ...
    void insertIntegerInput(std::vector<Value*>& inputs, size_t input)
    {
        ...
        constant->output()->setType(quarisma::IntType::get());
    ...
    bool replaceWithVariadicOp(Node* op_node)
    {
        ...
        // We insert these extra integers quarisma the end of the argument list only if we
...
bool UseVariadicOp(const std::shared_ptr<Graph>& graph, NodeKind op, NodeKind variadic_op)
{
...
bool RemoveListMutationAndUseVariadicOp(
    const std::shared_ptr<Graph>& graph, NodeKind op, NodeKind variadic_op)
{
    ...
    while (changed_in_last_iter)
    {
        ...
        changed_in_last_iter = UseVariadicOp(graph, op, variadic_op) || changed_in_last_iter;
...
bool RemoveListMutationAndUseVariadicStack(const std::shared_ptr<Graph>& graph)
{
