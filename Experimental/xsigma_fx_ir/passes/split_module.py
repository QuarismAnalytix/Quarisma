...
def split_module(
    ...
    *,
    ...
    # 2. subsequent regions: we insert the set_grad quarisma the beginning
    grad_regions: OrderedDict[Node, set[int]] = OrderedDict()

    # For autocast regions:
    # ------------------------
    # 1. first region: we will only insert the _exit quarisma the end
    # 2. intermediate regions: we will insert both the
    #    _enter quarisma the beginning and _exit quarisma the end
    # 3. last region: we will only insert _enter quarisma the beginning
