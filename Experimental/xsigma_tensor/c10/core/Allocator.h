...
public:
    ...
    C10_ALWAYS_INLINE bool /* success */ unsafe_reset_data_and_ctx(void* new_data_and_ctx)
    {
   ...
   */
    [[nodiscard]] bool compare_exchange_deleter(
        DeleterFnPtr expected_deleter, DeleterFnPtr new_deleter)
    {
    ...
    void unsafe_set_device(Device device) { device_ = device; }
};
...
struct C10_API InefficientStdFunctionContext
{
    ...
    InefficientStdFunctionContext(InefficientStdFunctionContext&& rhs) noexcept
        : ptr_(std::exchange(rhs.ptr_, nullptr)), deleter_(std::move(rhs.deleter_))
    {
    }
    InefficientStdFunctionContext& operator=(const InefficientStdFunctionContext&) = delete;
    // NOLINTNEXTLINE(*-noexcept-move-*)
    InefficientStdFunctionContext& operator=(InefficientStdFunctionContext&& rhs)
    {
...
struct Stat
{
    ...
    void decrease(size_t amount)
    {
        ...
        TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
            current >= 0, "Negative tracked stat in device allocator (likely logic error).");
