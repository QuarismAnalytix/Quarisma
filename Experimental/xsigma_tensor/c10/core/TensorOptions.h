...
inline caffe2::TypeMeta dtype_or_default(std::optional<caffe2::TypeMeta> dtype)
{
...
/// stock Quarisma functions return plain Tensors).  If you mix these functions
/// up, you WILL BE SAD.
///
...
DispatchKey computeDispatchKey(
    std::optional<ScalarType> dtype, std::optional<Layout> layout, std::optional<Device> device);
...
struct C10_API TensorOptions
{
    ...
    /* implicit */ TensorOptions(Layout layout) : TensorOptions() { this->set_layout(layout); }
    ...
    /* implicit */ TensorOptions(ScalarType dtype) : TensorOptions() { this->set_dtype(dtype); }
    ...
    [[nodiscard]] TensorOptions device_index(c10::DeviceIndex device_index) const noexcept
    {
        return device(Device::Type::CUDA, device_index);
    }

    /// Return a copy of `TensorOptions` with `dtype` set to the given one.
    [[nodiscard]] TensorOptions dtype(std::optional<caffe2::TypeMeta> dtype) const noexcept
    {
        TensorOptions r = *this;
        r.set_dtype(dtype);
        return r;
    ...
    [[nodiscard]] TensorOptions dtype(std::optional<ScalarType> dtype) const noexcept
    {
        TensorOptions r = *this;
        r.set_dtype(dtype);
        return r;
    ...
    [[nodiscard]] TensorOptions layout(std::optional<Layout> layout) const noexcept
    {
        TensorOptions r = *this;
        r.set_layout(layout);
        return r;
    ...
    [[nodiscard]] TensorOptions requires_grad(std::optional<bool> requires_grad) const noexcept
    {
        TensorOptions r = *this;
    ...
    [[nodiscard]] TensorOptions pinned_memory(std::optional<bool> pinned_memory) const noexcept
    {
        TensorOptions r = *this;
    ...
    caffe2::TypeMeta dtype() const noexcept { return dtype_or_default(dtype_opt()); }
    ...
    bool requires_grad() const noexcept { return has_requires_grad_ ? requires_grad_ : false; }
    ...
    {
        return has_requires_grad_ ? std::make_optional(requires_grad_) : std::nullopt;
    }

    /// Returns the `pinned_memory` property of the `TensorOptions`.
    bool pinned_memory() const noexcept { return pinned_memory_or_default(pinned_memory_opt()); }
    ...
    {
        return layout_ == c10::Layout::SparseCsr || layout_ == c10::Layout::SparseCsc ||
    ...
    {
        return has_pinned_memory_ ? std::make_optional(pinned_memory_) : std::nullopt;
    }
    ...
    // Resolves the Quarisma backend specified by the current construction axes.
    // TODO: Deprecate this
...
static_assert(sizeof(TensorOptions) <= sizeof(int64_t) * 2, "TensorOptions must fit in 128-bits");
...
C10_API std::ostream& operator<<(std::ostream& stream, const TensorOptions& options);
...
#undef DO_CASE
        ...
        case c10::DeviceType::IDEEP:
            TORCH_INTERNAL_ASSERT(
                ...
                device_.type(),
                ", it shouldn't ever convert to a DispatchKey.  File a bug describing what you "
                "were doing if you think this is in error.");
        default:
            TORCH_CHECK_NOT_IMPLEMENTED(
                false, "Unsupported device type for dense layout: ", device_.type());
        }
    }
    case Layout::Sparse:
        switch (device_.type())
...
#undef DO_CASE
        default:
            TORCH_CHECK_NOT_IMPLEMENTED(
                false, "Unsupported device type for sparse layout: ", device_.type());
        }
    case Layout::Mkldnn:
        switch (device_.type())
        {
        ...
        default:
            TORCH_CHECK_NOT_IMPLEMENTED(
                false, "Unsupported device type for mkldnn layout: ", device_.type());
        }
...
#undef DO_CASE
        default:
            TORCH_CHECK_NOT_IMPLEMENTED(
                false, "Unsupported device type for ", layout_, " layout: ", device_.type());
        }
    default:
