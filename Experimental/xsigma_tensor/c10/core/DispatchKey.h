...
// in aten/src/Quarisma/core/VariableFallbackKernel.cpp
...
enum class DispatchKey : uint16_t
{
    ...
    // intended for out of tree use; tested by aten/src/Quarisma/test/rng_test.cpp
    CustomRNGKeyId,
    ...
    MkldnnCPU,  // registered at build/aten/src/Quarisma/RegisterMkldnnCPU.cpp
    ...
    ZeroTensor,  // registered at build/aten/src/Quarisma/RegisterZeroTensor.cpp
    ...
    // aten/src/Quarisma/core/dispatch/backend_fallback_test.cpp for a usage example.
    ...
    // aten/src/Quarisma/core/dispatch/backend_fallback_test.cpp
    // for a usage example
...
#undef DEFINE_PER_BACKEND_KEYS_FOR_BACKEND
    ...
    // build/aten/src/Quarisma/RegisterCompositeImplicitAutograd.cpp
    ...
    // build/aten/src/Quarisma/RegisterFuncTorchBatchedDecomposition.cpp
    ...
    // build/aten/src/Quarisma/RegisterCompositeImplicitAutogradNestedTensor.cpp
    CompositeExplicitAutograd,  // registered at
    // build/aten/src/Quarisma/RegisterCompositeExplicitAutograd.cpp
...
constexpr bool isPerBackendFunctionalityKey(DispatchKey k)
{
    if (k == DispatchKey::Dense || k == DispatchKey::Quantized || k == DispatchKey::Sparse ||
        k == DispatchKey::SparseCsr || k == DispatchKey::AutogradFunctionality ||
...
constexpr BackendComponent toBackendComponent(DispatchKey k)
{
    if (k >= DispatchKey::StartOfDenseBackends && k <= DispatchKey::EndOfDenseBackends)
    {
        return static_cast<BackendComponent>(
            static_cast<uint8_t>(k) - static_cast<uint8_t>(DispatchKey::StartOfDenseBackends));
    }
    else if (k >= DispatchKey::StartOfQuantizedBackends && k <= DispatchKey::EndOfQuantizedBackends)
    {
        return static_cast<BackendComponent>(
            static_cast<uint8_t>(k) - static_cast<uint8_t>(DispatchKey::StartOfQuantizedBackends));
    }
    else if (k >= DispatchKey::StartOfSparseBackends && k <= DispatchKey::EndOfSparseBackends)
    {
        return static_cast<BackendComponent>(
            static_cast<uint8_t>(k) - static_cast<uint8_t>(DispatchKey::StartOfSparseBackends));
    }
    else if (k >= DispatchKey::StartOfSparseCsrBackends && k <= DispatchKey::EndOfSparseCsrBackends)
    {
        return static_cast<BackendComponent>(
            static_cast<uint8_t>(k) - static_cast<uint8_t>(DispatchKey::StartOfSparseCsrBackends));
    }
    else if (
