...
#include <Quarisma/functorch/BatchedTensorImpl.h>
...
BatchedTensorImpl::BatchedTensorImpl(
    DispatchKeySet key_set, Tensor value, int64_t bdim, int64_t level)
    : TensorImpl(
          key_set.add(
      ...
      bdim_(bdim)
{
    ...
    {
        TORCH_CHECK(
            bdim_ == 0, "Nested tensors can only be vmapped over dim=0, but got dim=", bdim_);
        TORCH_CHECK(
            level_ == 1, "Only one level of vmap is supported when vmapping over nested tensors");
    }
...
Tensor makeBatched(Tensor tensor, int64_t bdim, int64_t level)
{
    ...
    if (batched)
    {
        ...
        TORCH_INTERNAL_ASSERT(
            level > batched_level, " batched_level: ", batched_level, " level: ", level);
    }
