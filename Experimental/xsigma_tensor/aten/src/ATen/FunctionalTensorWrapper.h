...
struct TORCH_API FunctionalTensorWrapper : public c10::TensorImpl
{
    ...
    bool     has_metadata_mutation() const { return has_metadata_mutation_; }
    uint64_t mutation_counter() const { return functional_storage_impl()->mutation_counter(); }
    void     mark_mutation() { functional_storage_impl()->mark_mutation(); }
    ...
    bool was_storage_changed() { return was_storage_changed_; }
    ...
    uint64_t storage_changed_counter() { return storage_changed_counter_; }

    // A FunctionalTensor is considered a base if its not a view of another
    // tensor.
    bool isBaseTensor() const { return view_metas_.empty(); }
    ...
    bool is_multi_output_view() { return is_multi_output_view_; }
...
inline FunctionalTensorWrapper* unsafeGetFunctionalWrapper(const Tensor& tensor)
{
...
TORCH_API bool isFunctionalTensor(const c10::List<std::optional<Tensor>>& t_list);
TORCH_API bool isFunctionalTensor(ITensorListRef list);

TORCH_API Tensor to_functional_tensor(const Tensor& tensor);
TORCH_API std::optional<Tensor> to_functional_tensor(const std::optional<Tensor>& tensor);
...
TORCH_API std::optional<Tensor> from_functional_tensor(
    const std::optional<Tensor>& t, bool assert_functional = true);
...
TORCH_API void mark_mutation_hidden_from_autograd(const Tensor& functional_tensor);

TORCH_API bool are_all_mutations_hidden_from_autograd(const Tensor& functional_tensor);

TORCH_API bool are_all_mutations_under_no_grad_or_inference_mode(const Tensor& functional_tensor);

// These two methods are XLA-specific logic and are no-ops
// for the normal functionalization flow.
TORCH_API void propagate_xla_data(const Tensor& functional_tensor, const Tensor& other);
TORCH_API void propagate_xla_data(const ITensorListRef functional_tensor, ITensorListRef other);

TORCH_API void propagate_xla_data_direct(const Tensor& tensor, const Tensor& other);
TORCH_API void propagate_xla_data_direct(const ITensorListRef tensor, ITensorListRef other);
...
void mutate_view_meta(const Tensor& self, const std::shared_ptr<functionalization::ViewMeta>& meta);

TORCH_API Tensor apply_view_meta_sequence(
    const Tensor& base, const std::vector<std::shared_ptr<functionalization::ViewMeta>>& sequence);

void set_sizes_strides_offset(const Tensor& out, const Tensor& meta_out);
void set_sizes_strides_offset(
    const std::vector<Tensor>& outs, const std::vector<Tensor>& meta_outs);
...
public:
    ...
    ~FunctionalizationReapplyViewsGuard() { setFunctionalizationReapplyViewsTLS(prev_); }

    FunctionalizationReapplyViewsGuard(const FunctionalizationReapplyViewsGuard&) = delete;
    FunctionalizationReapplyViewsGuard operator=(const FunctionalizationReapplyViewsGuard&) =
        delete;
...
struct _functionalize_aten_op<Op, symint, ReturnType(ParameterTypes...)> final
{
    ...
    {
        using FuncType =
            ReturnType(typename c10::maybe_keep_symint<symint, ParameterTypes>::type...);
