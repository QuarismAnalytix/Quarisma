...
inline void compare_base_kernel(
    const Tensor& result1,
    const Tensor& result2,
    const Tensor& self,
    int64_t       dim,
    ...
    const func_t& f)
{
    ...
    compare_base_kernel_core<scalar_t, scalar_t_2>(result1, result2, self, dim, keepdim, loop);
}

void min_kernel_impl(
    const Tensor& result, const Tensor& indice, const Tensor& self, int64_t dim, bool keepdim)
{
...
void isposinf_kernel_impl(TensorIteratorBase& iter)
{
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        [&]()
        {
            cpu_kernel(
                iter,
                [](scalar_t a) -> bool { return a == std::numeric_limits<scalar_t>::infinity(); });
        });
}
...
void mode_kernel_impl(
    Tensor& values, Tensor& indices, const Tensor& self, int64_t dim, bool keepdim)
{
    ...
    AT_DISPATCH_ALL_TYPES_AND3(
        ...
        [&]
        {
            ...
            compare_base_kernel_core<scalar_t>(values, indices, self, dim, keepdim, loop);
        });
}
    ...
    AT_DISPATCH_ALL_TYPES_AND2(
        ...
        [&]()
        {
            ...
            cpu_kernel_vec(
                ...
                [=](Vectorized<scalar_t> a) { return vec::clamp(a, min_vec, max_vec); });
        });
}
