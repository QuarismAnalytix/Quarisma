...
{
    return !at::isFloat8Type(input_t) && at::isReducedFloatingType(input_t) && output_t == kFloat;
}

bool reduced_output(ScalarType input_t, ScalarType output_t)
{
    return !at::isFloat8Type(output_t) && at::isReducedFloatingType(output_t) && input_t == kFloat;
}
}  // namespace
...
static bool reduced_float_type_copy(bool requires_conj, TensorIteratorBase& iter)
{
    ...
    return (
        !requires_conj &&
        ((reduced_input(iter.dtype(1), iter.dtype(0)) && sizeof(float) == strides_out[0] &&
...
static void reduced_float_copy_kernel(TensorIteratorBase& iter, bool requires_neg)
{
    ...
    auto                         get_strides =
        [](int64_t* strides, IntArrayRef strides_out, IntArrayRef strides_in, int64_t ndim)
    {
    ...
    if (reduced_input(iter.dtype(1), iter.dtype(0)))
    {
        AT_DISPATCH_REDUCED_FLOATING_TYPES(
            ...
            [&]()
            {
                ...
                scalar_t*                  input_data =
                    const_cast<scalar_t*>(iter.tensor_base(1).const_data_ptr<scalar_t>());
                ptrs[0] = reinterpret_cast<char*>(output_data);
                ptrs[1] = reinterpret_cast<char*>(input_data);

                int64_t grain_size = at::internal::GRAIN_SIZE;

                auto loop = [strides_in, requires_neg](
                                char** base, const int64_t* strides, int64_t size0, int64_t size1)
                {
                    ...
                    for ([[maybe_unused]] const auto it : c10::irange(size1))
                    {
                        ...
                        if (i < size0)
                        {
                            ...
                            {
                                Vecs data_vec =
                                    Vecs::loadu(data[1] + i * sizeof(scalar_t), size0 - i);
                                auto [data_vec0, data_vec1] = convert_to_float<scalar_t>(data_vec);
                                if (requires_neg)
                                {
                                    data_vec0 = data_vec0.neg();
                                ...
                                data_vec0.store(
                                    data[0] + i * sizeof(dest_t),
                                    ((size0 - i) > Vecd::size()) ? Vecd::size() : (size0 - i));
                                data_vec1.store(
                                    data[0] + (i + Vecd::size()) * sizeof(dest_t),
                                    ((size0 - i) > Vecd::size()) ? (size0 - i - Vecd::size()) : 0);
                            }
                            else
                            {
                                dst_s.store(
    ...
    else if (reduced_output(iter.dtype(1), iter.dtype(0)))
    {
        AT_DISPATCH_REDUCED_FLOATING_TYPES(
            ...
            [&]()
            {
                ...
                dest_t*                    output_data = iter.tensor_base(0).data_ptr<dest_t>();
                source_t*                  input_data =
                    const_cast<source_t*>(iter.tensor_base(1).const_data_ptr<source_t>());
                    ...
                    for ([[maybe_unused]] const auto it : c10::irange(size1))
                    {
                        ...
                        for (; i <= size0 - 2 * Vecs::size(); i += 2 * Vecs::size())
                        {
                            if (strides_in[0] != 0)
                            {
                                ...
                                Vecs data_vec1 =
                                    Vecs::loadu(data[1] + (i + Vecs::size()) * sizeof(source_t));
                        ...
                        if (i < size0)
                        {
                            if (strides_in[0] != 0)
                            {
                                Vecs data_vec0 = Vecs::loadu(
                                    data[1] + i * sizeof(source_t),
                                    ((size0 - i) > Vecs::size()) ? Vecs::size() : (size0 - i));
                                Vecs data_vec1 = Vecs::loadu(
                                    data[1] + (i + Vecs::size()) * sizeof(source_t),
                                    ((size0 - i) > Vecs::size()) ? (size0 - i - Vecs::size()) : 0);
                                auto data_vec = convert_from_float<dest_t>(data_vec0, data_vec1);
                                if (requires_neg)
                                {
                                    data_vec = data_vec.neg();
...
#define _AT_DISPATCH_ALL_TYPES(TYPE, NAME, ...) \
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(     \
        ...
        __VA_ARGS__)
#define _AT_DISPATCH_ALL_TYPES_NO_CF(TYPE, NAME, ...) \
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(kBool, kHalf, kBFloat16, TYPE, NAME, __VA_ARGS__)
#endif
            ...
            [&]
            {
                ...
                _AT_DISPATCH_ALL_TYPES(
                    ...
                    [&]
                    {
                        ...
                        {
                            cpu_kernel(
                                iter, [](scalar_t x) -> dest_t { return c10::convert<dest_t>(x); });
                        }
                    });
            });
