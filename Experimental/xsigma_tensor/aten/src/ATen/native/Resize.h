...
inline void maybe_resize_storage_cpu(TensorImpl* self, size_t new_size_bytes)
{
    ...
    if (!storage)
    {
        auto new_storage = c10::make_intrusive<StorageImpl>(
            StorageImpl::use_byte_size_t(), new_size_bytes, c10::GetCPUAllocator(), true);
...
TORCH_API TensorImpl* resize_impl_cpu_(
    TensorImpl* self, IntArrayRef size, at::OptionalIntArrayRef stride, bool resize_storage = true);
    ...
    const Storage&          new_storage)
{
    ...
    if (stride.data())
    {
        storage_size_bytes = at::detail::computeStorageNbytes(size, stride, data_type.itemsize());
        storage_size_plus_offset_bytes =
            at::detail::computeStorageNbytes(size, stride, data_type.itemsize(), storage_offset);
    }
    else
    {
        storage_size_bytes = at::detail::computeStorageNbytesContiguous(size, data_type.itemsize());
        storage_size_plus_offset_bytes =
            at::detail::computeStorageNbytesContiguous(size, data_type.itemsize(), storage_offset);
    }
    ...
    bool        check_offset_in_bounds = true)
{
    ...
    if (check_offset_in_bounds)
    {
        ...
        {
            checkInBoundsForStorage(size, stride, storage_offset, result.dtype(), storage);
        }
    }
 ...
 */
template <typename T>
inline void setStrided(const Tensor& self, ArrayRef<T> size, ArrayRef<T> stride, T storage_offset)
{
