...
#else
#include <Quarisma/ops/_aminmax_native.h>
#include <Quarisma/ops/_assert_async_native.h>
#include <Quarisma/ops/_assert_scalar_native.h>
#include <Quarisma/ops/_functional_assert_async_native.h>
#include <Quarisma/ops/_functional_assert_scalar_native.h>
#include <Quarisma/ops/_make_per_tensor_quantized_tensor.h>
#include <Quarisma/ops/_print_native.h>
#include <Quarisma/ops/_unique.h>
#include <Quarisma/ops/allclose_native.h>
#include <Quarisma/ops/aminmax.h>
#include <Quarisma/ops/argsort_native.h>
#include <Quarisma/ops/cat.h>
#include <Quarisma/ops/clamp.h>
#include <Quarisma/ops/clamp_max.h>
#include <Quarisma/ops/clamp_max_native.h>
#include <Quarisma/ops/clamp_min.h>
#include <Quarisma/ops/clamp_min_native.h>
#include <Quarisma/ops/clamp_native.h>
#include <Quarisma/ops/clip_native.h>
#include <Quarisma/ops/empty.h>
#include <Quarisma/ops/empty_like.h>
#include <Quarisma/ops/eq.h>
#include <Quarisma/ops/fill.h>
#include <Quarisma/ops/imag.h>
#include <Quarisma/ops/index.h>
#include <Quarisma/ops/is_nonzero_native.h>
#include <Quarisma/ops/isclose.h>
#include <Quarisma/ops/isclose_native.h>
#include <Quarisma/ops/isfinite.h>
#include <Quarisma/ops/isfinite_native.h>
#include <Quarisma/ops/isin.h>
#include <Quarisma/ops/isin_native.h>
#include <Quarisma/ops/isinf.h>
#include <Quarisma/ops/isinf_native.h>
#include <Quarisma/ops/isnan_native.h>
#include <Quarisma/ops/isneginf_native.h>
#include <Quarisma/ops/isposinf_native.h>
#include <Quarisma/ops/isreal_native.h>
#include <Quarisma/ops/max.h>
#include <Quarisma/ops/max_native.h>
#include <Quarisma/ops/min.h>
#include <Quarisma/ops/min_native.h>
#include <Quarisma/ops/mode.h>
#include <Quarisma/ops/mode_native.h>
#include <Quarisma/ops/ne.h>
#include <Quarisma/ops/ones_like.h>
#include <Quarisma/ops/real.h>
#include <Quarisma/ops/result_type_native.h>
#include <Quarisma/ops/scalar_tensor.h>
#include <Quarisma/ops/where.h>
#include <Quarisma/ops/where_native.h>
...
{
    TORCH_CHECK_NOT_IMPLEMENTED(!isComplexType(dtype), "clamp is not supported for complex types");
}

static inline void check_for_unsupported_clamp_dtypes(const Scalar& s)
{
    TORCH_CHECK_NOT_IMPLEMENTED(!s.isComplex(), "clamp is not supported for complex types");
}
...
)
{
    ...
    set_output_raw_strided(
        0, elements.sizes(), {}, TensorOptions(elements.device()).dtype(ScalarType::Bool));
}
...
)
{
    ...
    check_for_unsupported_isin_dtype(test_elements.scalar_type());
    set_output_raw_strided(
        0, {0}, {}, TensorOptions(test_elements.device()).dtype(ScalarType::Bool));
}
...
TORCH_META_FUNC(isposinf)(const Tensor& self)
{
    ...
    TORCH_CHECK(
        maybe_get_output().defined() ? maybe_get_output().dtype() == at::kBool : true,
...
(const Tensor& self, int64_t dim, bool keepdim)
{
    ...
    return TORCH_PRECOMPUTE_STRUCT2(max, dim)().set_dim(maybe_wrap_dim(dim, self.dim()));
}
...
(const Tensor& self, int64_t dim, bool keepdim)
{
    ...
    resize_reduction_with_indices(*this, self, dim, keepdim, self.scalar_type());
    return TORCH_PRECOMPUTE_STRUCT2(min, dim)().set_dim(maybe_wrap_dim(dim, self.dim()));
}
...
#define _AT_DISPATCH_INF_TYPES(TYPE, NAME, ...) \
    AT_DISPATCH_FLOATING_TYPES_AND3(kHalf, kBFloat16, kFloat8_e5m2, TYPE, NAME, __VA_ARGS__)
#else
#define _AT_DISPATCH_INF_TYPES(TYPE, NAME, ...) \
...
{
    TORCH_CHECK(
        native::is_nonzero(self), "Expected Tensor with single nonzero value, but got zero");
}

void _assert_async_msg_cpu(const Tensor& self, std::string_view assert_msg)
{
    TORCH_CHECK(native::is_nonzero(self), !assert_msg.empty() ? assert_msg : "Assertion is failed");
}

void _assert_scalar(const Scalar& scalar, std::string_view assert_msg)
{
    TORCH_SYM_CHECK(scalar.toSymBool(), !assert_msg.empty() ? assert_msg : "Assertion is failed");
}

Tensor _functional_assert_scalar(
    const Scalar& scalar, std::string_view assert_msg, const Tensor& dep_token)
{
...
Tensor _functional_assert_async_msg_cpu(
    const Tensor& self, std::string_view assert_msg, const Tensor& dep_token)
{
...
Tensor& where_self_out(
    const Tensor& condition, const Tensor& self, const Tensor& other, Tensor& out)
{
    ...
    if (device != at::kCPU)
    {  // allow CPU scalars on non-cpu device
    ...
    {
        TORCH_WARN_ONCE(
            "where received a uint8 condition tensor. This behavior is deprecated and will be "
            "removed in a future version of PyTorch. Use a boolean condition instead.");
...
(const Tensor& self, int64_t dim, bool keepdim, const Tensor& values, const Tensor& indices)
{
...
std::tuple<Tensor, Tensor> qmax(const Tensor& self, int64_t dim, bool keepdim)
{
    TORCH_CHECK(
        ...
        "Please open an issue on https://github.com/pytorch/pytorch/issues if you need per channel "
    ...
    return std::tuple<Tensor, Tensor>(
        at::_make_per_tensor_quantized_tensor(max, self.q_scale(), self.q_zero_point()),
        max_indices);
}
...
std::tuple<Tensor, Tensor> qmin(const Tensor& self, int64_t dim, bool keepdim)
{
    ...
    return std::tuple<Tensor, Tensor>(
        at::_make_per_tensor_quantized_tensor(min, self.q_scale(), self.q_zero_point()),
        min_indices);
}
...
{
    TORCH_WARN_ONCE(
        "_aminmax is deprecated as of PyTorch 1.11 and will be removed in a future release. Use "
        "aminmax instead."
...
std::tuple<Tensor&, Tensor&> min_out(
    const Tensor& self, Dimname dim, bool keepdim, Tensor& min, Tensor& min_indices)
{
...
std::tuple<Tensor&, Tensor&> max_out(
    const Tensor& self, Dimname dim, bool keepdim, Tensor& max, Tensor& max_indices)
{
...
std::tuple<Tensor&, Tensor&> mode_out(
    const Tensor& self, Dimname dim, bool keepdim, Tensor& values, Tensor& indices)
{
 ...
 const Tensor& out)
{
    ...
    if (test_elements.numel() <
        static_cast<int64_t>(10.0f * std::pow(static_cast<double>(elements.numel()), 0.145)))
    {
        out.fill_(invert);
