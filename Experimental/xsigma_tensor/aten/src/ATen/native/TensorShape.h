...
inline void check_cat_shape_except_dim(
    const Tensor& first, const Tensor& second, int64_t dimension, int64_t index)
{
...
inline void check_cat_no_zero_dim(const MaterializedITensorListRef& tensors)
{
    ...
    {
        TORCH_CHECK(
            t.dim() > 0, "zero-dimensional tensor (at position ", i, ") cannot be concatenated");
        i++;
    }
}
...
inline int64_t preprocess_chunk_cat_inputs(TensorList tensors, int64_t dim, int64_t num_chunks)
{
    ...
    {
        TORCH_CHECK(
            dim >= 0, "_chunk_cat expects non-negative dim when input tensors have different ndims")
        for (const auto i : c10::irange(tensors.size()))
        {
            TORCH_CHECK(
