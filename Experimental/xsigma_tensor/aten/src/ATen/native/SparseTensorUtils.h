...
inline void copy_into_sparse(
    const SparseTensor& self, const Tensor& indices, const Tensor& values, bool non_blocking)
{
    alias_into_sparse(
        self,
...
// TODO: Expose this for real in Quarisma, some day?
...
TORCH_API Tensor
flatten_indices(const Tensor& indices, IntArrayRef full_size, bool force_clone = false);
...
public:
    explicit TensorGeometryHolder(
        IntArrayRef sizes, IntArrayRef strides, TensorOptions options = {})
    {
    ...
    auto operator*() const { return std::make_tuple(t_sizes, t_strides); }
...
public:
    explicit TensorGeometryHolder(IntArrayRef sizes, IntArrayRef strides, TensorOptions options)
    {
        ...
        const Tensor t_sizes_and_strides = t_sizes_and_strides_cpu.to(options.device());
    ...
    auto operator*() const
    {
        return std::make_tuple(
            t_sizes.template data_ptr<int64_t>(), t_strides.template data_ptr<int64_t>());
    }
