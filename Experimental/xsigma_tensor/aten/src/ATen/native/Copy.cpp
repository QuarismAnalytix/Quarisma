...
#include <Quarisma/metal/Context.h>
#include <Quarisma/native/Copy.h>
#include <Quarisma/native/TensorShape.h>
#include <Quarisma/native/mps/Copy.h>
#include <Quarisma/native/quantized/Copy.h>
#include <Quarisma/native/vulkan/ops/Copy.h>
#include <Quarisma/quantized/Quantizer.h>
#include <Quarisma/vulkan/Context.h>
...
#else
#include <Quarisma/ops/_copy_from.h>
#include <Quarisma/ops/_foreach_copy.h>
#include <Quarisma/ops/_foreach_copy_native.h>
#include <Quarisma/ops/_propagate_xla_data.h>
#include <Quarisma/ops/_propagate_xla_data_native.h>
#include <Quarisma/ops/copy.h>
#include <Quarisma/ops/copy_native.h>
#include <Quarisma/ops/empty.h>
#include <Quarisma/ops/empty_strided.h>
#include <Quarisma/ops/expand_copy.h>
#endif
...
bool copy_transpose_valid(const Tensor& self, const Tensor& src)
{
    ...
    return self.is_contiguous() && src.numel() != 0 && src.dim() == 2 && src.stride(0) == 1 &&
           src.stride(1) == src.size(0) && self.scalar_type() == src.scalar_type() &&
           !isBitsType(self.scalar_type()) && self.sizes().equals(src.sizes()) &&
...
// https://github.com/pytorch/pytorch/issues/88543
        ...
        (self.sizes() == src.sizes()))
    {
        ...
        {
            auto* output_ptr = reinterpret_cast<fbgemm::float16*>(self.data_ptr<at::Half>());
                ...
                at::parallel_for(
                    ...
                    [&](int64_t begin, int64_t end)
                    {
                        fbgemm::FloatToFloat16_simd(
                            src.const_data_ptr<float>() + begin, output_ptr + begin, end - begin);
                    });
            }
        }
        else
        {
            auto in_data = reinterpret_cast<const fbgemm::float16*>(src.const_data_ptr<at::Half>());
    ...
    if (self.is_quantized() && src.is_quantized())
    {
        TORCH_CHECK(self.qscheme() == src.qscheme(), "Quantized Copy only works with same qscheme");
    ...
    if (!self.is_quantized() && src.is_quantized())
    {
        TORCH_CHECK(
            false,
            "Copying from quantized Tensor to non-quantized Tensor is not allowed, please use "
            "dequantize to get a float Tensor from a quantized Tensor");
    }
    ...
    const bool is_same_data =
        (self.is_alias_of(src) && self.storage_offset() == src.storage_offset() &&
         self.strides().equals(src.strides()) && self.sizes().equals(src.sizes()) &&
         self.scalar_type() == src.scalar_type() && self.is_conj() == src.is_conj() &&
...
Tensor& copy_(Tensor& self, const Tensor& src, bool non_blocking)
{
    auto maybe_outnames = namedinference::compute_broadcast_outnames(self, src);
    {
        ...
        if (self._is_zerotensor())
        {
            TORCH_CHECK(
                false,
                "ZeroTensors are immutable. Please materialize the tensor using `.clone()`, if you "
                "want a mutable zero tensor.");
        }
