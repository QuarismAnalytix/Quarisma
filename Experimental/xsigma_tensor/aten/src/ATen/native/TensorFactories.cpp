...
#include <Quarisma/CPUGeneratorImpl.h>
...
#else
#include <Quarisma/ops/_cast_Byte_native.h>
#include <Quarisma/ops/_cast_Char_native.h>
#include <Quarisma/ops/_cast_Double_native.h>
#include <Quarisma/ops/_cast_Float_native.h>
#include <Quarisma/ops/_cast_Half_native.h>
#include <Quarisma/ops/_cast_Int_native.h>
#include <Quarisma/ops/_cast_Long_native.h>
#include <Quarisma/ops/_cast_Short_native.h>
#include <Quarisma/ops/_dim_arange_native.h>
#include <Quarisma/ops/_efficientzerotensor_native.h>
#include <Quarisma/ops/_empty_affine_quantized.h>
#include <Quarisma/ops/_empty_per_channel_affine_quantized.h>
#include <Quarisma/ops/_sparse_compressed_tensor_with_dims_native.h>
#include <Quarisma/ops/arange.h>
#include <Quarisma/ops/arange_native.h>
#include <Quarisma/ops/bartlett_window_native.h>
#include <Quarisma/ops/blackman_window_native.h>
#include <Quarisma/ops/clone_native.h>
#include <Quarisma/ops/complex.h>
#include <Quarisma/ops/complex_native.h>
#include <Quarisma/ops/cumprod.h>
#include <Quarisma/ops/empty.h>
#include <Quarisma/ops/empty_like.h>
#include <Quarisma/ops/empty_like_native.h>
#include <Quarisma/ops/empty_native.h>
#include <Quarisma/ops/empty_permuted_native.h>
#include <Quarisma/ops/empty_strided.h>
#include <Quarisma/ops/empty_strided_native.h>
#include <Quarisma/ops/eye.h>
#include <Quarisma/ops/eye_native.h>
#include <Quarisma/ops/fill_native.h>
#include <Quarisma/ops/flip.h>
#include <Quarisma/ops/from_file_native.h>
#include <Quarisma/ops/full_like_native.h>
#include <Quarisma/ops/full_native.h>
#include <Quarisma/ops/hamming_window_native.h>
#include <Quarisma/ops/hann_window_native.h>
#include <Quarisma/ops/kaiser_window_native.h>
#include <Quarisma/ops/linspace.h>
#include <Quarisma/ops/linspace_native.h>
#include <Quarisma/ops/logspace.h>
#include <Quarisma/ops/logspace_native.h>
#include <Quarisma/ops/new_empty_native.h>
#include <Quarisma/ops/new_empty_strided_native.h>
#include <Quarisma/ops/new_full_native.h>
#include <Quarisma/ops/new_ones_native.h>
#include <Quarisma/ops/new_zeros_native.h>
#include <Quarisma/ops/normal_native.h>
#include <Quarisma/ops/ones.h>
#include <Quarisma/ops/ones_like_native.h>
#include <Quarisma/ops/ones_native.h>
#include <Quarisma/ops/polar.h>
#include <Quarisma/ops/polar_native.h>
#include <Quarisma/ops/promote_types.h>
#include <Quarisma/ops/rand_like_native.h>
#include <Quarisma/ops/rand_native.h>
#include <Quarisma/ops/randint_like_native.h>
#include <Quarisma/ops/randint_native.h>
#include <Quarisma/ops/randn_like_native.h>
#include <Quarisma/ops/randn_native.h>
#include <Quarisma/ops/randperm.h>
#include <Quarisma/ops/randperm_native.h>
#include <Quarisma/ops/range.h>
#include <Quarisma/ops/range_native.h>
#include <Quarisma/ops/scalar_tensor_native.h>
#include <Quarisma/ops/tril_indices_native.h>
#include <Quarisma/ops/triu_indices_native.h>
#include <Quarisma/ops/vander_native.h>
#include <Quarisma/ops/zeros_like_native.h>
...
void window_function_checks(
    const char* function_name, const TensorOptions& options, int64_t window_length)
{
    TORCH_CHECK(
    ...
    std::optional<bool>       pin_memory)
{
    ...
    Tensor result = set_to_integral_dtype ? at::empty({0}, options.dtype(at::ScalarType::Long))
                                          : at::empty({0}, options);
...
{
    TORCH_CHECK(
        (a.scalar_type() == kFloat || a.scalar_type() == kDouble || a.scalar_type() == kHalf) &&
...
static void complex_check_dtype(const Tensor& result, const Tensor& a, const Tensor& b)
{
    ...
    std::optional<MemoryFormat> optional_memory_format)
{
    ...
    TORCH_CHECK(
        options.layout() == Layout::Strided, "NYI: named tensors only support strided layout");
    TORCH_CHECK(
        options.device().is_cpu() || options.device().is_cuda() || options.device().is_xpu() ||
            options.device().is_privateuseone(),
    ...
    std::optional<bool>       pin_memory_opt)
{
    ...
    Tensor phys_tensor = at::empty_symint(
        phys_size, dtype_opt, layout_opt, device_opt, pin_memory_opt, std::nullopt);
...
Tensor& empty_out(
    IntArrayRef size, std::optional<c10::MemoryFormat> optional_memory_format, Tensor& result)
{
    ...
    std::optional<c10::MemoryFormat> optional_memory_format)
{
    ...
    TensorOptions options =
        self.options().merge_in(options_).merge_memory_format(optional_memory_format);
    ...
    if (memory_format == MemoryFormat::Preserve)
    {
        if (self.is_non_overlapping_and_dense())
        {
            result = at::empty_strided_symint(
                self.sym_sizes(), self.sym_strides(), options.memory_format(std::nullopt));
        }
    ...
    else
    {
        // See Note [Explicit nullopt MemoryFormat argument]
        result =
            at::empty_symint(self.sym_sizes(), options.memory_format(memory_format), std::nullopt);
    }
...
{
    auto dtype =
        dtype_opt.has_value() ? dtype_opt : optTypeMetaToScalarType(self.options().dtype_opt());
    ...
    auto pin_memory =
        pin_memory_opt.has_value() ? pin_memory_opt : self.options().pinned_memory_opt();
...
TensorOptions infer_full_options(const Scalar& fill_value, const TensorOptions& options)
{
    ...
    std::optional<bool>       pin_memory)
{
    ...
    TORCH_CHECK(options.layout() != kSparse, "full(...) is not implemented for sparse layout");
...
{
    TORCH_CHECK(!result.is_sparse(), "full(...) is not implemented for sparse layout");
    ...
    std::optional<bool>       pin_memory)
{
    Tensor r = self.new_empty(
        size, TensorOptions().dtype(dtype).layout(layout).device(device).pinned_memory(pin_memory));
    r.fill_(fill_value);
    return r;
}
...
TensorOptions linspace_logspace_infer_options(
    const Scalar& start, const Scalar& end, const TensorOptions& options, const char* fn_name)
{
    ...
    std::optional<bool>       pin_memory)
{
    return native::randint(
        high, size, std::nullopt /* generator*/, dtype, layout, device, pin_memory);
}
...
Tensor& randint_out(
    int64_t high, IntArrayRef size, std::optional<Generator> generator, Tensor& result)
{
    result.resize_(size);
...
Tensor& randint_out(
    int64_t low, int64_t high, IntArrayRef size, std::optional<Generator> generator, Tensor& result)
{
    result.resize_(size);
    return result.random_(low, high, std::move(generator));
}
    ...
    std::optional<c10::MemoryFormat> optional_memory_format)
{
    ...
    return at::native::randint_like(
        self, high_scalar, dtype, layout, device, pin_memory, optional_memory_format);
}
...
Tensor& normal_out(
    double mean, double std, IntArrayRef size, std::optional<Generator> generator, Tensor& result)
{
    result.resize_(size);
    return result.normal_(mean, std, std::move(generator));
}
...
Tensor& randperm_out_cpu(int64_t n, std::optional<Generator> generator, Tensor& result)
{
    ...
    TORCH_CHECK(
        !generator.has_value() || (generator.has_value() && result.device() == generator->device()),
        "Expected a '",
        result.device(),
    ...
    result.resize_({n});
    auto gen =
        get_generator_or_default<CPUGeneratorImpl>(generator, detail::getDefaultCPUGenerator());
    ...
    std::optional<bool>       pin_memory_opt)
{
    ...
    auto result =
        at::native::empty_cpu({2, tril_size}, dtype_opt, layout_opt, device_opt, pin_memory_opt);
    ...
    std::optional<bool>       pin_memory_opt)
{
    ...
    // create an empty Tensor with correct size
    auto result =
        at::native::empty_cpu({2, triu_size}, dtype_opt, layout_opt, device_opt, pin_memory_opt);
    ...
    std::optional<bool>       pin_memory)
{
    ...
    TORCH_CHECK(
        size.size() >= 2,
        "torch.zeros: Only batched sparse compressed (non-block) tensors are supported, but got "
        "size ",
        size);
    ...
    return at::_sparse_compressed_tensor_unsafe(
        compressed_indices, plain_indices, values, size_, dtype, layout, device, pin_memory);
}
...
Tensor vander(const Tensor& x, std::optional<int64_t> N, bool increasing)
{
    ...
    auto result = at::empty(
        {x.size(0), n},
        x.options().dtype(at::promote_types(x.scalar_type(), c10::ScalarType::Long)));
...
template <typename T>
static Tensor tensor_complex_cpu(ArrayRef<T> values, const TensorOptions& options)
{
    return at::detail::tensor_complex_cpu(values, options);
}

template <typename T>
static Tensor tensor_complex_backend(ArrayRef<T> values, const TensorOptions& options)
{
    ...
    std::optional<bool>       pin_memory)
{
    ...
    auto    storage_impl = c10::make_intrusive<at::StorageImpl>(
        ...
        size_bytes,
        MapAllocator::makeDataPtr(std::string(filename), flags, size_bytes, nullptr),
        /*allocator=*/nullptr,
        /*resizable=*/false);
    auto tensor = detail::make_tensor<at::TensorImpl>(storage_impl, at::DispatchKey::CPU, my_dtype);
