...
static TensorImpl* _resize_impl_(
    TensorImpl* self, ArrayRef<T> size, at::OptionalArrayRef<T> stride, bool resize_storage)
{
    if (self->generic_sizes<T>() == size &&
...
TensorImpl* resize_impl_cpu_(
    TensorImpl* self, IntArrayRef size, at::OptionalIntArrayRef stride, bool resize_storage)
{
...
static const Tensor& _resize_(
    const Tensor& self, ArrayRef<T> size, std::optional<MemoryFormat> optional_memory_format)
{
    auto*   self_ = self.unsafeGetTensorImpl();
    int64_t old_storage_nbytes =
        self_->unsafe_storage() ? self_->unsafe_storage().sym_nbytes().maybe_as_int().value_or(-1)
                                : 0;
    ...
    if (optional_memory_format.has_value())
    {
        auto memory_format = optional_memory_format.value();
        TORCH_CHECK(
            memory_format != MemoryFormat::Preserve, "Unsupported memory format", memory_format);
        self_->empty_tensor_restride(memory_format);
    }
    // See Note [Enabling Deterministic Operations]
    if (C10_UNLIKELY(
            at::globalContext().deterministicAlgorithms() &&
            at::globalContext().deterministicFillUninitializedMemory() && old_storage_nbytes != -1))
    {
...
void resize_bytes_nocuda(const Storage& storage, const c10::SymInt& newsize)
{
    ...
    {
        at::detail::getPrivateUse1Hooks().resizePrivateUse1Bytes(storage, newsize.expect_int());
    }
    else if (device_type == at::kXPU || device_type == at::kHPU || device_type == at::kMTIA)
    {
        ...
        if (original_data_ptr == src_tensor.storage().data_ptr().get())
        {
            ...
            storage.set_data_ptr_noswap(std::move(new_tensor.storage().mutable_data_ptr()));
    ...
    {
        TORCH_CHECK(false, "UntypedStorage.resize_: got unexpected device type ", device_type);
    }
}
