...
static CPUCapability compute_cpu_capability()
{
    ...
    if (envar.has_value())
    {
        ...
        int sve_vl =
            cpuinfo_get_max_arm_sve_length();  //Returns maximum SVE VL supported by your HW.
...
)
{
    ...
    if (std::find(supported_devices.begin(), supported_devices.end(), device_type) ==
    ...
    case DeviceType::HPU:
        return hpu_dispatch_ptr != nullptr ? DispatchResult(hpu_dispatch_ptr)
                                           : ErrorType::MissingDeviceKernel;

    case DeviceType::PrivateUse1:
        return privateuse1_dispatch_ptr != nullptr ? DispatchResult(privateuse1_dispatch_ptr)
                                                   : ErrorType::MissingDeviceKernel;
...
)
{
    ...
    if (std::holds_alternative<ErrorType>(result))
    {
        ...
        case ErrorType::MissingDeviceKernel:
            TORCH_INTERNAL_ASSERT(false, "DispatchStub: missing kernel for ", device_type);
            return nullptr;
