...
inline void check_args(int64_t row, int64_t col, std::optional<Layout> layout_opt)
{
    ...
    {
        TORCH_CHECK(
            *layout_opt == at::kStrided, "only support layout=torch.strided, got", *layout_opt)
    }
}
...
inline void check_supported_max_int_with_precision(int64_t n, const Tensor& tensor)
{
    ...
    case at::ScalarType::Double:  // Unlikely to happen, but doesn't hurt to
                                  // check
        TORCH_CHECK(
            n <= (int64_t(1) << 53) + 1, "n cannot be greater than 2^53+1 for Double type.");
        break;
    default:
        break;
    }
...
inline Tensor& fill_empty_deterministic_(Tensor& tensor)
{
    if (tensor.is_floating_point() || tensor.is_complex())
    {
        AT_DISPATCH_V2(
            ...
            AT_WRAP([&]() { tensor.fill_(std::numeric_limits<scalar_t>::quiet_NaN()); }),
