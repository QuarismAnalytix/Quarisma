...
{
    auto curr_storage_size =
        value_.unsafeGetTensorImpl()->unsafe_storage().unsafeGetStorageImpl()->sym_nbytes();
    // storage resizing is severely limited: we only support resizing either to zero, or from zero bytes.
    TORCH_CHECK(
        new_size == 0 || curr_storage_size == 0,
        "new_size: ",
        new_size,
        ". curr_storage_size: ",
        curr_storage_size);
...
void FunctionalTensorWrapper::maybe_replace_storage(const Tensor& other)
{
    ...
    TORCH_CHECK(
        storage().use_count() == 1,
        "Attempted to resize a view tensor to a larger size. This is not allowed in the "
...
FunctionalTensorWrapper::view_metas() const
{
    return view_metas_;
}
    ...
    bool                           allow_tensor_metadata_change)
{
    TensorImpl::copy_tensor_metadata(
        src_impl, dest_impl, version_counter, allow_tensor_metadata_change);
...
void FunctionalTensorWrapper::shallow_copy_from(const c10::intrusive_ptr<TensorImpl>& impl)
{
    ...
    auto functional_impl = static_cast<FunctionalTensorWrapper*>(impl.get());
    copy_tensor_metadata_and_refresh(
...
c10::List<::std::optional<Tensor>> to_functional_tensor(
...
c10::List<::std::optional<Tensor>> from_functional_tensor(
    const c10::List<::std::optional<Tensor>>& t_list)
{
    c10::List<::std::optional<Tensor>> outputs;
    outputs.reserve(t_list.size());
...
void propagate_xla_data(const Tensor& functional_tensor, const Tensor& other)
{
    ...
    {
        at::_propagate_xla_data(
            at::functionalization::impl::unsafeGetFunctionalWrapper(functional_tensor)->value(),
            other);
    }
}
...
void propagate_xla_data_direct(const ITensorListRef tensor, ITensorListRef other)
{
...
{
    TORCH_CHECK(isFunctionalTensor(functional_tensor));
    return unsafeGetFunctionalWrapper(functional_tensor)
    ...
    int64_t                                             out_idx)
{
    ...
    return at::detail::make_tensor<FunctionalTensorWrapper>(
        view_to_wrap, functional_base_impl, meta_);
}
...
void mutate_view_meta(
    const at::Tensor& self, const std::shared_ptr<functionalization::ViewMeta>& meta)
{
...
void set_sizes_strides_offset(const Tensor& out, const Tensor& reference_out)
{
    out.unsafeGetTensorImpl()->set_sizes_and_strides(
        reference_out.sym_sizes(), reference_out.sym_strides(), reference_out.sym_storage_offset());
}

void set_sizes_strides_offset(
    const std::vector<Tensor>& outs, const std::vector<Tensor>& reference_outs)
{
...
void functionalize_op_helper(const c10::OperatorHandle& op, torch::jit::Stack* stack)
{
    ...
    for (uint64_t idx = 0; idx < num_arguments; ++idx)
    {
        ...
        if (ivalue.isTensor())
        {
            ...
            if (t.defined())
            {
                TORCH_INTERNAL_ASSERT(
                    ...
                    "The composite op functionalization fallback expects its inputs all not to be "
                    "functional tensors");
        ...
        else if (ivalue.isTensorList())
        {
            ...
            TORCH_INTERNAL_ASSERT(
                !at::functionalization::impl::isFunctionalTensor(tensors),
                "The composite op functionalization fallback expects its inputs all not to be "
                "functional tensors");
        ...
        else if (ivalue.isOptionalTensorList())
        {
            ...
            TORCH_INTERNAL_ASSERT(
                !at::functionalization::impl::isFunctionalTensor(opt_tensors),
                "The composite op functionalization fallback expects its inputs all not to be "
                "functional tensors");
            auto t_new =
                c10::IValue(at::functionalization::impl::to_functional_tensor(opt_tensors));
            (*stack)[arguments_begin + idx] = t_new;
        }
    }
    ...
    for (const auto idx : c10::irange(num_returns))
    {
        ...
        else if (ivalue.isOptionalTensorList())
        {
            ...
            auto t_new =
                c10::IValue(at::functionalization::impl::from_functional_tensor(opt_tensors));
            (*stack)[returns_begin + idx] = t_new;
        }
    }
}
