...
#define AT_DISPATCH_ALL_SPARSE_COMPRESSED_LAYOUTS(LAYOUT, NAME, ...)                            \
    ...
    {                                                                                           \
        ...
        default:                                                                                \
            TORCH_CHECK(                                                                        \
                false, NAME, " expected sparse compressed tensor layout but got ", the_layout); \
        }                                                                                       \
    }()
...
public:
    ...
    CheckSparseTensorInvariants& operator=(const CheckSparseTensorInvariants&) = delete;
    CheckSparseTensorInvariants& operator=(CheckSparseTensorInvariants&&)      = delete;
...
{
    return AT_DISPATCH_ROW_SPARSE_COMPRESSED_LAYOUTS(
        layout, "isCompressedColumn", [&] { return false; }, [&] { return true; });
}
...
{
    return AT_DISPATCH_ROW_SPARSE_COMPRESSED_LAYOUTS(
        layout, "plainIndicesName", [&] { return "col_indices"; }, [&] { return "row_indices"; });
}
...
inline size_t compressedDimension(Layout layout, IntArrayRef size, size_t dense_ndim = 0)
{
    return size.size() - dense_ndim - (isCompressedRow(layout) ? 2 : 1);
}

inline size_t plainDimension(Layout layout, IntArrayRef size, size_t dense_ndim = 0)
{
...
inline std::pair<Tensor, Tensor> getCompressedPlainIndices(Tensor const& self)
{
    return AT_DISPATCH_ROW_SPARSE_COMPRESSED_LAYOUTS(
        ...
        [&self] { return std::make_pair(self.crow_indices(), self.col_indices()); },
        [&self] { return std::make_pair(self.ccol_indices(), self.row_indices()); });
}
...
{
    auto [compressed_indices, plain_indices] = at::sparse_csr::getCompressedPlainIndices(input);
...
inline std::tuple<Tensor, Tensor> create_acc_buffer(
    TensorOptions option, ScalarType type, int64_t nnz = -1)
{
    ...
    if (nnz != -1)
    {
        return std::make_tuple(new_values.resize_(nnz), new_values_acc.resize_(nnz));
    }
    else
    {
