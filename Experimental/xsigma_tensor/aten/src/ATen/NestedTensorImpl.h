...
struct TORCH_API NestedTensorImpl : public c10::TensorImpl
{
    ...
    explicit NestedTensorImpl(const at::Tensor& buffer, const at::Tensor& nested_sizes);
    ...
    const Tensor& get_nested_sizes() const { return nested_sizes_; }
    // TODO: don't expose private implementation details like this
    const Tensor& get_nested_strides() const { return nested_strides_; }
    const Tensor& get_storage_offsets() const { return storage_offsets_; }
   ...
   */
    ...
    size_t get_buffer_size() const { return storage_.nbytes() / data_type_.itemsize(); }
...
protected:
    ...
    int64_t      size_custom(int64_t d) const override { return this->size(d); }
    c10::SymInt  sym_size_custom(int64_t d) const override { return c10::SymInt{this->size(d)}; }
...
private:
   ...
   */
    inline c10::DispatchKeySet generate_buffer_key_set() const
    {
        ...
        buffer_key_set = buffer_key_set - c10::DispatchKeySet{
                                              c10::DispatchKey::NestedTensor,
                                              c10::DispatchKey::AutogradNestedTensor};

        // Add dense tensor specific keys
        buffer_key_set = buffer_key_set | c10::DispatchKeySet{c10::DispatchKey::Dense};
        buffer_key_set = Autograd ? c10::DispatchKeySet{c10::DispatchKey::Autograd} | buffer_key_set
                                  : buffer_key_set;
...
{
    TORCH_CHECK(tensor.is_nested(), "get_nested_tensor_impl requires a NestedTensor.");
    return static_cast<NestedTensorImpl*>(tensor.unsafeGetTensorImpl());
}
...
inline bool nested_tensor_impl_is_contiguous(const NestedTensorImpl* nt)
{
    ...
    const Tensor & sizemat = nt->get_nested_sizes(), &stridemat = nt->get_nested_strides();
