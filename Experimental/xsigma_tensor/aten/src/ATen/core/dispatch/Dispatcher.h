...
public:
   ...
   */
    ...
    void redispatchBoxed(
        const OperatorHandle& op, DispatchKeySet dispatchKeySet, Stack* stack) const;
    ...
    // Used by torchdeploy/multiply for multiple interpreters racing.
   ...
   */
    RegistrationHandleRAII registerFallback(
        DispatchKey dispatch_key, KernelFunction kernel, std::string debug);
...
private:
    ...
    void checkSchemaCompatibility(
        const OperatorHandle& op, const FunctionSchema& schema, const std::string& debug);
...
public:
    ...
    const OperatorName& operator_name() const { return operatorDef_->op.operator_name(); }

    bool hasSchema() const { return operatorDef_->op.hasSchema(); }

    const FunctionSchema& schema() const { return operatorDef_->op.schema(); }

    const std::string& debug() const { return operatorDef_->op.debug(); }

    std::string dumpState() const { return operatorDef_->op.dumpState(); }
    ...
    std::string dumpComputedTable() const { return operatorDef_->op.dumpComputedTable(); }

    void checkInvariants() const { operatorDef_->op.checkInvariants(); }

    c10::ArrayRef<at::Tag> getTags() const { return operatorDef_->op.getTags(); }
    ...
    void callBoxed(Stack* stack) const { c10::Dispatcher::singleton().callBoxed(*this, stack); }

    void callBoxed(Stack& stack) const { callBoxed(&stack); }
...
struct CaptureKernelCall
{
    ...
    ReturnType release() && { return std::move(output_); }
...
struct CaptureKernelCall<void>
{
        ...
        Args&&... args)
    {
        ...
        kernel.template call<void, Args...>(op, dispatchKeySet, std::forward<Args>(args)...);
    }
    Stack getOutputs() { return Stack(); }
    void  release() && {}
};
...
{
    auto dispatchKeySet =
        op.operatorDef_->op.dispatchKeyExtractor().template getDispatchKeySetUnboxed<Args...>(
            args...);
    ...
    if (C10_UNLIKELY(step_callbacks.has_value() && op.operatorDef_->op.isObserved()))
    {
        return callWithDispatchKeySlowPath<Return, Args...>(
            op, *step_callbacks, dispatchKeySet, kernel, std::forward<Args>(args)...);
    }
