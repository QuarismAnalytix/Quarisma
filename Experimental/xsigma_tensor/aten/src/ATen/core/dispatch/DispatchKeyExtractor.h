...
{
    DispatchKeySet ts;
    void           operator()(const at::Tensor& x) { ts = ts | x.key_set(); }
...
public:
    ...
    void deregisterSchema() { dispatch_arg_indices_reverse_ = c10::utils::bitset(); }
...
private:
    static bool isDispatchType(const Type& type)
    {
        ...
        return type.isSubtypeOf(c10::TypeFactory::get<TensorType>()) ||
               type.isSubtypeOf(ct.listOfTensors) || type.isSubtypeOf(ct.listOfOptionalTensors) ||
               type.isSubtypeOf(ct.optionalOfTensor);
#else   // C10_MOBILE
        return type.isSubtypeOf(*TensorType::get()) || type.isSubtypeOf(*ListType::ofTensors()) ||
