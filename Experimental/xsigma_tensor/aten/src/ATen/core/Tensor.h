...
public:
    ...
    ~OptionalTensorRef() { ref_.unsafeReleaseTensorImpl(); }

    OptionalTensorRef(const TensorBase& src) : ref_(Tensor::unsafe_borrow_t{}, src)
    {
        TORCH_INTERNAL_ASSERT_DEBUG_ONLY(src.defined());
    }

    OptionalTensorRef(const OptionalTensorRef& rhs) : ref_(Tensor::unsafe_borrow_t{}, rhs.ref_) {}
    ...
    bool has_value() const { return ref_.defined(); }

    const Tensor& getTensorRef() const& { return ref_; }

    const Tensor& operator*() const& { return ref_; }

    const Tensor* operator->() const& { return &ref_; }

    operator bool() const { return ref_.defined(); }
...
{
public:
    ~TensorRef() { ref_.unsafeReleaseTensorImpl(); }
...
auto Tensor::register_hook(T&& hook) const -> Tensor::hook_return_void_t<T>
{
    ...
    static_assert(std::is_same_v<decltype(hook(Tensor())), void>, "Expected hook to return void");
