...
static void check_unique_names(DimnameList names)
{
    ...
    for (auto it = names.begin(); it != names.end(); ++it)
    {
        ...
        {
            TORCH_CHECK(
                false, "Cannot construct a tensor with duplicate names. Got names: ", names, ".");
        }
    }
}
...
{
    TORCH_CHECK(
        impl->layout() == Layout::Strided, "NYI: named tensors only support strided layout");
    TORCH_CHECK(
        impl->device().is_cpu() || impl->device().is_cuda() || impl->device().is_xpu() ||
            impl->device().is_privateuseone(),
    ...
    if (meta == nullptr)
    {
        ...
        impl->set_named_tensor_meta(
            std::make_unique<NamedTensorMeta>(NamedTensorMeta::HasNonWildcard, *names));
    }
    else
    {
...
void internal_set_names_inplace(TensorImpl* impl, std::vector<Dimname>&& names, bool validate_names)
{
    ...
    {
        impl->set_named_tensor_meta(
            std::make_unique<NamedTensorMeta>(NamedTensorMeta::HasNonWildcard, std::move(names)));
    }
    else
    {
