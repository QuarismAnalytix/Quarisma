...
inline bool variable_excluded_from_dispatch()
{
    ...
    return true;
#else
    return c10::impl::tls_local_dispatch_key_set().excluded_.isSupersetOf(
...
// to recompile all of Quarisma and its dependencies.
//
...
public:
    ...
    bool is_floating_point() const { return at::isFloatingType(this->scalar_type()); }

    bool is_signed() const { return at::isSignedType(this->scalar_type()); }

    c10::SymInt sym_size(int64_t dim) const { return impl_->sym_size(dim); }
    ...
    int64_t size(int64_t dim) const { return impl_->size(dim); }
    ...
    TensorImpl* unsafeReleaseTensorImpl() { return impl_.release(); }
    ...
    bool defined() const { return impl_; }
    ...
    bool   is_same(const TensorBase& other) const noexcept { return impl_ == other.impl_; }
    size_t use_count() const noexcept { return impl_.use_count(); }
    size_t weak_use_count() const noexcept { return impl_.weak_use_count(); }
    ...
    // See impl::get_opt_names in Quarisma/NamedTensor.h for docs.
    std::optional<DimnameList> opt_names() const
    {
        return impl::get_opt_names(unsafeGetTensorImpl());
    }
    // See impl::get_names in Quarisma/NamedTensor.h for docs.
    ...
    bool is_non_overlapping_and_dense() const { return impl_->is_non_overlapping_and_dense(); }
    ...
    c10::SymInt sym_numel() const { return impl_->sym_numel(); }

    c10::SymInt sym_storage_offset() const { return impl_->sym_storage_offset(); }
    ...
    int64_t element_size() const { return static_cast<int64_t>(impl_->itemsize()); }
    ...
    // to simplify the storage lifetime management logic in Quarisma
    ...
    void share_memory_() { at::share_memory_(*this); }

    inline bool _is_zerotensor() const { return impl_->_is_zerotensor(); }

    inline void _set_zero(bool zero) const { impl_->_set_zero(zero); }
    ...
    inline void _set_conj(bool conjugate) const { impl_->_set_conj(conjugate); }

    inline bool is_neg() const { return impl_->is_neg(); }
    ...
    inline void _set_neg(bool negative) const { impl_->_set_neg(negative); }
    ...
    inline Device device() const { return impl_->device(); }
    ...
    bool is_xla() const { return impl_->is_xla(); }

    /// Returns if a `Tensor` has MTIA backend.
    bool is_mtia() const { return impl_->is_mtia(); }

    /// Returns if a `Tensor` has HPU backend.
    bool is_hpu() const { return impl_->is_hpu(); }
    ...
    bool is_meta() const { return impl_->is_meta(); }
    ...
    bool is_nested() const { return impl_->is_nested(); }
    ...
    const void* const_data_ptr() const { return this->unsafeGetTensorImpl()->data(); }

    void* mutable_data_ptr() const { return this->unsafeGetTensorImpl()->mutable_data(); }
    ...
    void* data_ptr() const { return mutable_data_ptr(); }
    ...
    {
        static_assert(
            N > 0, "accessor is used for indexing tensor, for scalars use *data_ptr<T>()");
    ...
    GenericPackedTensorAccessor<T, N, PtrTraits, index_t> generic_packed_accessor() const&
    {
        ...
        return GenericPackedTensorAccessor<T, N, PtrTraits, index_t>(
            static_cast<typename PtrTraits<T>::PtrType>(ptr), sizes().data(), strides().data());
    }
    template <
        typename T,
        size_t N,
    ...
    {
        TORCH_CHECK(
            impl_->numel() <= static_cast<int64_t>(std::numeric_limits<int32_t>::max()),
    ...
    const Tensor& _fw_grad(uint64_t level) const { return impl_->_fw_grad(level, *this); }
    ...
    using hook_return_void_t =
        std::enable_if_t<std::is_void_v<typename std::invoke_result_t<T&, TensorBase>>, unsigned>;
    template <typename T>
    using hook_return_var_t = std::enable_if_t<
        std::is_same_v<typename std::invoke_result_t<T&, TensorBase>, TensorBase>,
        unsigned>;
...
auto TensorBase::register_hook(T&& hook) const -> TensorBase::hook_return_void_t<T>
{
    ...
    static_assert(
        std::is_same_v<decltype(hook(TensorBase())), void>, "Expected hook to return void");
    return _register_hook(
        [fn = std::forward<T>(hook)](const TensorBase& grad)
        {
            fn(grad);
            return TensorBase();
        });
...
inline c10::MaybeOwned<TensorBase> borrow_from_optional_tensor(const std::optional<TensorBase>& opt)
{
    return opt.has_value() ? c10::MaybeOwned<TensorBase>::borrowed(*opt)
