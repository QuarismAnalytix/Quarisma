...
{
    static auto value = TensorType::create({}, {}, SymbolicShape(), VaryingShape<Stride>{}, {});
    return value;
}
...
VaryingShape<Stride> TensorType::computeStrideProps(
    at::IntArrayRef sizes, at::IntArrayRef strides, bool tensor_contiguity)
{
    int                 n_dim = static_cast<int>(sizes.size());
    std::vector<size_t> stride_indices(n_dim);
                ...
                else
                {
                    contiguous_ = strides[stride_indices[i]] == 1 ||
                                  (strides[stride_indices[i]] != 0 &&
                                   strides[stride_indices[i]] == strides[stride_indices[i - 1]] *
    ...
    bool                          tensor_contiguity)
{
    ...
    if (stride_concrete_sizes.has_value())
    {
        ...
        TORCH_INTERNAL_ASSERT(
            size_concrete_sizes.has_value() &&
            size_concrete_sizes->size() == stride_concrete_sizes->size());
        auto sprops =
    ...
    else
    {
        ...
        return TensorType::create(
            ...
            device,
            symbol_sizes,
            VaryingShape<Stride>(sizes_opt->size()),
            requires_grad,
            undefined);
    }
}
...
{
    return TensorType::create(
        scalar_type, device, SymbolicShape(dim), VaryingShape<Stride>(dim), requires_grad);
}
        ...
        fmap(
            *sizes_.sizes(),
            [](ShapeSymbol ss)
            {
                ...
                return ss.is_static() ? std::optional<int64_t>(ss.static_size()) : std::nullopt;
            }));
}
...
TensorTypePtr TensorType::merge(const TensorType& other, bool merge_sizes) const
{
    ...
    return TensorType::create(
        scalar_type,
        dev,
        merge_sizes ? symbolic_sizes().merge(other.symbolic_sizes()) : symbolic_sizes(),
        sprops,
        gr,
        undef);
}
...
bool TensorType::matchTensor(const at::Tensor& t)
{
    ...
    bool matched_strides =
        (!stride_properties().size()) || (!t.has_storage() && !stride_properties().isComplete()) ||
    ...
    return scalarType().value_or(t.scalar_type()) == t.scalar_type() &&
           device().value_or(t.device()) == t.device() && requiresGrad().value_or(rg) == rg &&
           matched_strides && is_null_or_equal(sizes().concrete_sizes(), t.sizes());
}
...
bool TensorType::equals(const c10::Type& rhs) const
{
    ...
    return scalar_type_ == rt->scalarType() && sizes() == rt->sizes() &&
           stride_properties() == rt->stride_properties() && device() == rt->device() &&
...
TensorTypePtr TensorType::createContiguous(
    at::ScalarType scalar_type, at::Device device, at::IntArrayRef sizes)
{
