...
public:
    ...
    Tensor& operator=(Tensor&& x) & noexcept { return operator=(static_cast<TensorBase&&>(x)); }
    ...
    C10_DEPRECATED_MESSAGE(
        "Tensor.type() is deprecated. Instead use Tensor.options(), which in many cases (e.g. in a "
        "constructor) is a drop-in replacement. If you were using data from type(), that is now "
        "available from Tensor itself, so instead of tensor.type().scalar_type(), use "
        "tensor.scalar_type() instead and instead of tensor.type().backend() use tensor.device().")
    DeprecatedTypeProperties& type() const
    {
        return globalDeprecatedTypePropertiesRegistry().getDeprecatedTypeProperties(
            dispatchKeyToBackend(legacyExtractDispatchKey(key_set())), scalar_type());
    }
    ...
    C10_DEPRECATED_MESSAGE(
        "Tensor.is_variable() is deprecated; everything is a variable now. (If you want to assert "
        "that variable has been appropriately handled already, use "
        "at::impl::variable_excluded_from_dispatch())")
    bool is_variable() const noexcept { return !at::impl::variable_excluded_from_dispatch(); }
    ...
    Tensor  operator~() const { return bitwise_not(); }
    ...
    Tensor& operator&=(const Tensor& other) { return bitwise_and_(other); }
    Tensor& operator|=(const Tensor& other) { return bitwise_or_(other); }
    Tensor& operator^=(const Tensor& other) { return bitwise_xor_(other); }
    ...
    Tensor operator[](const Tensor& index) const
    {
        ...
        if (index.dim() != 0)
        {
            TORCH_CHECK_INDEX(false, "Can only index with tensors that are scalars (zero-dim)");
        }
        // The Scalar(Tensor) constructor is explicit, so we need to call it.
        return this->operator[](index.item());
    }
    Tensor operator[](int64_t index) const { return select(0, index); }
    ...
    {
        return to(
            options().device(c10::DeviceType::Vulkan), /*non_blocking*/ false, /*copy*/ false);
    }
    ...
    /// thread safe.
    Tensor& mutable_grad() const { return impl_->mutable_grad(); }
    ...
    const Tensor& grad() const
    {
        ...
        {
            TORCH_WARN(
                "The .grad attribute of a Tensor that is not a leaf Tensor is being accessed. Its "
                ".grad "
                "attribute won't be populated during autograd.backward(). If you indeed want the "
                ".grad "
                "field to be populated for a non-leaf Tensor, use .retain_grad() on the non-leaf "
                "Tensor. "
                "If you access the non-leaf Tensor by mistake, make sure you access the leaf "
                "Tensor "
    ...
    Tensor std(int dim) const { return std(IntArrayRef{dim}); }
    ...
    {
        return this->to(
            device, /*scalar_type=*/typeMetaToScalarType(type_meta), non_blocking, copy);
    }
    ...
    at::Tensor tensor_data() const { return TensorBase::tensor_data(); }
    ...
    at::Tensor variable_data() const { return TensorBase::variable_data(); }
    ...
    using hook_return_void_t =
        std::enable_if_t<std::is_void<typename std::invoke_result_t<T&, Tensor>>::value, unsigned>;
    template <typename T>
    using hook_return_var_t = std::
        enable_if_t<std::is_same_v<typename std::invoke_result_t<T&, Tensor>, Tensor>, unsigned>;
    ...
    Tensor data() const { return TensorBase::data(); }
...
struct ExclusivelyOwnedTraits<at::Tensor>
{
    ...
    static repr_type nullRepr() { return at::Tensor(); }
    ...
    static repr_type moveToRepr(at::Tensor&& x) { return std::move(x); }
    ...
    static at::Tensor take(at::Tensor& x) { return std::move(x); }
...
inline c10::MaybeOwned<Tensor> borrow_from_optional_tensor(const std::optional<Tensor>& opt)
{
