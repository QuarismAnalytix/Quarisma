...
struct TORCH_API SparseTensorImpl : public TensorImpl
{
    ...
    void refresh_numel() { TensorImpl::safe_refresh_numel(); }
...
public:
    ...
    c10::SymInt sym_nnz() const { return values_.sym_size(0); }
    ...
    void raw_resize_(int64_t sparse_dim, int64_t dense_dim, IntArrayRef size)
    {
        ...
        TORCH_CHECK(
            !has_symbolic_sizes_strides_, "raw_resize_ called on tensor with symbolic shape")
    ...
    {
        TORCH_CHECK(
            allow_tensor_metadata_change(), "resize_ ", err_msg_tensor_metadata_change_not_allowed);
...
2. If you need to resize this tensor, you have the following options:\n\
    ...
    2. Or, create a new sparse tensor with the correct indices and values from this sparse tensor.";
        ...
        const bool size_equals_sizes = std::equal(
            size.begin(), size.end(), sizes_and_strides.begin(), sizes_and_strides.end());
        ...
        VariableVersion&& version_counter, bool allow_tensor_metadata_change) const
    {
        ...
        if (mode_stack_len > 0 && !c10::impl::tls_is_dispatch_key_excluded(DispatchKey::Python))
        {
