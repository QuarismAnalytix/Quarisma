...
public:
        ...
        std::optional<c10::SymInt> step_index  = std::nullopt)
    {
        ...
        TORCH_CHECK_VALUE(
            step_.sym_ne(0).expect_true(__FILE__, __LINE__), "slice step cannot be zero");
...
struct TORCH_API TensorIndex final
{
    ...
    TensorIndex(at::indexing::EllipsisIndexType /*unused*/) : type_(TensorIndexType::Ellipsis) {}
    ...
    TensorIndex(SymInt integer) : integer_(std::move(integer)), type_(TensorIndexType::SymInt) {}
    ...
    inline SymInt integer() const { return integer_; }
...
TORCH_API std::ostream& operator<<(
    std::ostream& stream, const std::vector<TensorIndex>& tensor_indices);
    ...
    const std::optional<SymIntArrayRef>& self_sizes)
{
    ...
    TORCH_CHECK_VALUE(
        step.sym_gt(0).expect_true(__FILE__, __LINE__), "step must be greater than zero");
    ...
    if (self_sizes.has_value() && !self_sizes.value().empty())
    {
        ...
        // the slice.
        SymInt length = (self_device == at::kCPU || self_device == at::kCUDA) ? (*self_sizes)[dim]
                                                                              : self.sym_size(dim);
        if (!disable_slice_optimization && TORCH_STATICALLY_KNOWN_TRUE(start.sym_eq(0)) &&
            TORCH_STATICALLY_KNOWN_TRUE(length.sym_le(stop)) && step == 1)
        {
            return self;
        }
    }
    return self.slice_symint(dim, std::move(start), std::move(stop), std::move(step));
}
    ...
    if (self_sizes.has_value())
    {
        ...
        if (maybe_index.has_value())
        {
            TORCH_CHECK_INDEX(
                ...
                "Use `tensor.item()` in Python or `tensor.item<T>()` in C++ to convert a 0-dim "
                "tensor to a number");
        }
...
inline Tensor boolToIndexingTensorNonNativeDeviceType(const Tensor& self, bool value)
{
    // booleans add a dimension of size 1. true indexes this dimension as if 0:,
    // false as empty.
    if (value)
...
inline Tensor boolToIndexingTensor(const Tensor& self, bool value, const at::Device& self_device)
{
...
inline Tensor scalarToTensorNonNativeDeviceType(const Scalar& v, const TensorOptions& options)
{
    return at::scalar_tensor(v, options);
}

inline void recordTensorIndex(
    const Tensor& tensor, std::vector<Tensor>& outIndices, int64_t* dim_ptr)
{
    if (outIndices.empty())
    {
...
inline c10::List<::std::optional<Tensor>> typeConvertIndices(
    const Tensor& /*self*/, std::vector<Tensor>&& indices)
{
...
inline Tensor scalarToTensor(
    const Scalar& v, const TensorOptions& options, const at::Device& self_device)
{
    ...
    const std::optional<SymIntArrayRef>& prev_dim_result_sizes)
{
    ...
    else if (index.is_boolean())
    {
        ...
        impl::recordTensorIndex(
            impl::boolToIndexingTensor(result, index.boolean(), original_tensor_device),
            outIndices,
            dim_ptr);
        return result;
    }
...
inline Tensor dispatch_index_put_(Tensor& self, std::vector<Tensor>&& indices, const Tensor& value)
{
    // Remove trailing null elements from indices
    while (!indices.empty() && !indices.back().defined())
    {
    ...
    bool                         disable_slice_optimization = false)
{
    ...
    // tensor size
    std::optional<SymIntArrayRef> self_sizes =
        self.is_nested() ? std::optional<SymIntArrayRef>(std::nullopt)
    ...
    Tensor              sliced = impl::applySlicing(
        self, indices, tensorIndices, disable_slice_optimization, self_device, self_sizes);
