...
TensorIteratorConfig& TensorIteratorConfig::declare_static_dtype_and_device(
    ScalarType dtype, Device device)
{
    TORCH_CHECK(
        !check_all_same_dtype_,
        "check_all_same_dtype(false) must be called before declare_static_dtype(...)");
    static_dtype_  = dtype;
...
TensorIteratorConfig& TensorIteratorConfig::declare_static_shape(IntArrayRef shape)
{
    ...
    TORCH_CHECK(
        !resize_outputs_, "resize_outputs() must be called before declare_static_shape(...)")
    static_shape_ = DimVector(shape);
    return *this;
}

TensorIteratorConfig& TensorIteratorConfig::declare_static_shape(
    IntArrayRef shape, IntArrayRef squash_dims)
{
    ...
    {
        TORCH_CHECK(
            squash_dim >= 0 && squash_dim < static_cast<int64_t>(static_shape_->size()),
            "squash_dim ",
            squash_dim,
            " must be in [0, ",
            static_shape_->size(),
...
{
    return std::find(const_tensor_indices_.begin(), const_tensor_indices_.end(), idx) !=
...
void TensorIteratorBase::compute_types(const TensorIteratorConfig& config)
{
    ...
    if ((has_different_input_dtypes || all_ops_are_scalars_) &&
    ...
    for (auto& op : operands_)
    {
        ...
        if (config.check_all_same_device_)
        {
            ...
            if (!common_device.is_cpu() && config.allow_cpu_scalars_ && !op.is_output &&
                op.tensor_base().dim() == 0 && op.tensor_base().is_cpu())
            {
                TORCH_CHECK(
                    current_cpu_scalars_on_non_cpu < max_cpu_scalars_on_non_cpu,
        ...
        if (config.enforce_safe_casting_to_output_ && op.is_output &&
        ...
        if (common_device == kCPU)
        {
            ...
            if (config.cast_common_dtype_to_outputs_ && op.is_output &&
                op.current_dtype != common_dtype_ && !is_meta_)
            {
            ...
            if (config.promote_inputs_to_common_dtype_ && !op.is_output &&
                op.current_dtype != common_dtype_)
            {
                op.exchange_tensor(
...
DimVector TensorIteratorBase::invert_perm(IntArrayRef input) const
{
    ...
    auto res = DimVector(
        input.size());  //no initialization needed, every value in res should be written to.
    for (const auto dim : c10::irange(ndim()))
    {
        ...
        if (!op.tensor_base().defined() || op.will_resize)
        {
            ...
            else
            {
                ...
                set_output_raw_strided(
                    i, tensor_shape, tensor_stride, original_options(op), names_);
            }
...
void TensorIteratorBase::for_each(loop2d_t loop, int64_t grain_size)
{
    ...
    else
    {
        at::parallel_for(
            ...
            numel,
            grain_size,
            [&](int64_t begin, int64_t end) { serial_for_each(loop, {begin, end}); });
    }
}
...
void TensorIteratorBase::serial_for_each(loop2d_t loop, Range range) const
{
    ...
    at::internal::serial_for_each(shape_, strides, ptrs.data(), ptrs.size(), loop, range);
}
...
void TensorIteratorBase::build_borrowing_binary_float_op(
    const TensorBase& out, const TensorBase& a, const TensorBase& b)
{
    build(BINARY_FLOAT_OP_CONFIG().add_output(out).add_const_input(a).add_const_input(b));
}
...
void TensorIteratorBase::build_binary_op(
    const TensorBase& out, const TensorBase& a, const TensorBase& b)
{
    build(
        BINARY_OP_CONFIG().add_owned_output(out).add_owned_const_input(a).add_owned_const_input(b));
}

void TensorIteratorBase::build_borrowing_binary_op(
    const TensorBase& out, const TensorBase& a, const TensorBase& b)
{
    build(BINARY_OP_CONFIG().add_output(out).add_const_input(a).add_const_input(b));
}
...
{
    build(UNARY_FLOAT_OP_CONFIG().add_owned_output(out).add_owned_const_input(a));
}

void TensorIteratorBase::build_borrowing_unary_float_op(const TensorBase& out, const TensorBase& a)
{
    build(UNARY_FLOAT_OP_CONFIG().add_output(out).add_const_input(a));
}
...
{
    build(UNARY_OP_CONFIG().add_owned_output(out).add_owned_const_input(a));
}

void TensorIteratorBase::build_borrowing_unary_op(const TensorBase& out, const TensorBase& a)
{
    build(UNARY_OP_CONFIG().add_output(out).add_const_input(a));
}
...
TensorIterator TensorIterator::binary_float_op(
    TensorBase& out, const TensorBase& a, const TensorBase& b)
{
    TensorIterator iter;
...
#define NULLARY_OP_CONFIG()                                                                         \
    TensorIteratorConfig()                                                                          \
        ...
        .check_all_same_dtype(                                                                      \
            false) /* FIXME: workaround for bug: https://github.com/pytorch/pytorch/issues/20342 */ \
        .resize_outputs(false)

TensorIterator TensorIterator::nullary_op(TensorBase& out)
{
    return NULLARY_OP_CONFIG().add_owned_output(out).build();
}

TensorIterator TensorIterator::borrowing_nullary_op(const TensorBase& out)
{
    return NULLARY_OP_CONFIG().add_output(out).build();
}
...
void TensorIteratorBase::compute_shape(const TensorIteratorConfig& config)
{
    ...
    for (auto& op : operands_)
    {
        ...
        if (config.resize_outputs_ && op.is_output)
            continue;
        TORCH_CHECK(
            !op.tensor_base().unsafeGetTensorImpl()->has_symbolic_sizes_strides(),
            "TensorIterator does not support symbolic shapes; please implement this operator in "
            "torch/_refs "
            "using the elementwise or reduction helpers (look at backtrace to find out what "
            "operator this is)");
...
bool TensorIteratorBase::fast_set_up(const TensorIteratorConfig& config)
{
    ...
    case FastSetupType::NON_OVERLAPPING_DENSE:
    {
        ...
        {
            if (tensor(i_defined).defined())
                break;
        }
        TORCH_CHECK(
            i_defined >= 0, "Can not find a defined tensor when fast allocating memory to outputs");
        for (const auto i : c10::irange(num_outputs_))
        {
            auto& op = operands_[i];
            if (!op.tensor_base().defined())
            ...
            set_output_raw_strided(
                i, shape_, tensor_base(i_defined).strides(), original_options(op), names_);
        }
        break;
    }
    default:
        TORCH_INTERNAL_ASSERT(
            false, "Unsupported fast setup type", std::to_string((int)setup_type));
    }
...
void TensorIteratorBase::build(TensorIteratorConfig& config)
{
    ...
    auto privateuse1_without_storage =
        common_device_.type() == DeviceType::PrivateUse1 && !has_storage;
    ...
    if (privateuse1_without_storage || common_device_.type() == DeviceType::XLA ||
        common_device_.type() == DeviceType::IPU || common_device_.type() == DeviceType::Lazy ||
        common_device_.type() == DeviceType::MAIA || common_device_.type() == DeviceType::HPU)
        return;
...
// funny business.
void TensorIterator::set_output_raw_strided(
    int64_t       output_idx,
    IntArrayRef   sizes,
    IntArrayRef   strides,
    TensorOptions options,
    DimnameList   names)
{
    ...
    if (!op.tensor_base().defined())
    {
        ...
        {
            op.tensor(
                c10::MaybeOwned<TensorBase>::owned(at::empty_strided(sizes, strides, options)));
        }
        op.current_dtype = op.target_dtype;
    }
    else if (op.will_resize)
