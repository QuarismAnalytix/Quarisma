...
public:
    ...
    OpaqueOptionalTensorRef& operator=(OpaqueOptionalTensorRef&&) noexcept = default;
    ~OpaqueOptionalTensorRef();

    OptionalTensorRef*       get() { return reinterpret_cast<OptionalTensorRef*>(data_.data()); }
...
struct TORCH_API OperandInfo
{
    ...
    TensorOptions options() const { return TensorOptions(target_dtype).device(device); }
    ...
    /// coalescing.
    const Tensor&     tensor() const { return tensor_storage_.getTensor(); }
    const TensorBase& tensor_base() const { return *tensor_base_; }
    ...
    const Tensor&     original_tensor() const { return original_tensor_storage_.getTensor(); }
    const TensorBase& original_tensor_base() const { return *original_tensor_base_; }
...
struct TORCH_API TensorIteratorBase : public impl::MetaBase
{
    ...
    using loop2d_t =
        c10::function_ref<void(char** data, const int64_t* strides, int64_t size0, int64_t size1)>;
    ...
    ScalarType  common_dtype() const
    {
        TORCH_INTERNAL_ASSERT(
            common_dtype_ != ScalarType::Undefined, "Queried for invalid common dtype!");
    ...
    c10::DeviceType device_type(int64_t arg = 0) const { return device(arg).type(); }
    ...
    const TensorBase& tensor_base(int64_t arg) const { return operands_[arg].tensor_base(); }
...
public:
    template <
        ...
        std::enable_if_t<
            std::is_convertible_v<
                loop1d_t,
                c10::function_ref<void(char**, const int64_t* strides, int64_t size)>>,
            int> = 0>
    ...
    void _unsafe_set_arg_data(const int64_t arg, void* data) { operands_[arg].data = data; }
...
#define TORCH_DISALLOW_TEMPORARIES_IMPL(methodname, maybestatic)                                 \
    maybestatic void methodname(TensorBase&& out, const TensorBase& a, const TensorBase& b) =    \
        delete;                                                                                  \
    maybestatic void methodname(const TensorBase& out, TensorBase&& a, const TensorBase& b) =    \
        delete;                                                                                  \
    maybestatic void methodname(const TensorBase& out, const TensorBase& a, TensorBase&& b) =    \
        delete;                                                                                  \
...
public:
    ...
    TensorIteratorConfig& add_input(const TensorBase& input) { return add_borrowed_input(input); }
