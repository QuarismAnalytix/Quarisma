...
static c10::SymInt get_nbytes(const Tensor& value)
{
    ...
    if (value.unsafeGetTensorImpl()->has_symbolic_sizes_strides())
    {
        ...
        return at::detail::computeStorageNbytes(
            value.sym_sizes(),
            value.sym_strides(),
            static_cast<int64_t>(value.dtype().itemsize()),
            value.sym_storage_offset());
    }
    // XLA storage objects also do not properly track nbytes.
    return static_cast<int64_t>(at::detail::computeStorageNbytes(
        value.sizes(), value.strides(), value.dtype().itemsize(), value.storage_offset()));
}
      ...
      base_(base)
{
    ...
    if (base.unsafeGetTensorImpl()->has_storage() &&
        data_ptr().device().type() != c10::DeviceType::XLA)
    {
        original_storage_size_ =
            base.unsafeGetTensorImpl()->unsafe_storage().unsafeGetStorageImpl()->sym_nbytes();
    }
    else
    {
...
void FunctionalStorageImpl::add_update(
    const Tensor& updated_val, const std::vector<std::shared_ptr<ViewMeta>>& metas)
{
    ...
    if (metas.size() > 1)
    {
        for (size_t i = 1; i < metas.size(); ++i)
        {
            ...
            TORCH_CHECK(
                updated_val.device().type() == c10::DeviceType::XLA || !metas[i]->is_as_strided,
                "During torch.compile, encountered a mutation on a view chain of length ",
                metas.size(),
                ", where view ",
                i,
                " was an as_strided() call. as_strided() is non-compositional, and therefore is "
                "not possible to functionalize properly today,"
                "so this behavior is banned in compile. As a workaround, you can either remove the "
                "mutation from the model code, or you "
                "can insert a graph break right before the mutation with "
                "torch._dynamo.graph_break(). If you would like this behavior to "
                "work properly, please comment on "
