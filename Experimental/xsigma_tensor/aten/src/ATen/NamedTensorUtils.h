...
TORCH_API std::vector<Dimname> unify_from_right(
    DimnameList names, DimnameList other, const char* action = "broadcast");
...
const Tensor& propagate_names_if_present_and_nonempty(
    const Tensor& result, std::optional<DimnameList> maybe_names, bool validate_names = false);
...
TORCH_API const Tensor& propagate_names_if_nonempty(
    const Tensor& result, DimnameList maybe_names, bool validate_names = false);
...
TORCH_API const Tensor& propagate_names(
    const Tensor& result, DimnameList names, bool validate_names = false);
...
TORCH_API void propagate_names_except(
    const Tensor& result, const Tensor& src, IntArrayRef excluded_idxs);

// Used for reduction ops that have a `keepdim` arg.
TORCH_API void propagate_names_for_reduction(
    const Tensor& result, const Tensor& src, IntArrayRef excluded_idxs, bool keepdim);
...
TORCH_API std::vector<Dimname> compute_broadcast_outnames(const Tensor& self, const Tensor& other);
...
TORCH_API TensorImpl* propagate_names_if_nonempty(
    TensorImpl* result, DimnameList maybe_names, bool validate_names = false);

TORCH_API TensorImpl* propagate_names(
    TensorImpl* result, DimnameList names, bool validate_names = false);

TORCH_API void propagate_names(TensorImpl* result, /*const */ TensorImpl* src);

inline void propagate_names(
    const TensorBase& result, DimnameList names, bool validate_names = false)
{
