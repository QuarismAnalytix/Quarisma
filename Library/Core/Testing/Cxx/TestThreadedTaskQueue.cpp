...
XSIGMATEST(ThreadedTaskQueue, simple_int_task)
{
...
XSIGMATEST(ThreadedTaskQueue, multiple_tasks_strict_ordering)
{
...
XSIGMATEST(ThreadedTaskQueue, multiple_arguments)
{
...
XSIGMATEST(ThreadedTaskQueue, try_pop_empty)
{
...
XSIGMATEST(ThreadedTaskQueue, try_pop_available)
{
...
XSIGMATEST(ThreadedTaskQueue, is_empty_check)
{
    auto                          worker = [](int x) { return x; };
    threaded_task_queue<int, int> queue(worker, true, -1, 2);
...
XSIGMATEST(ThreadedTaskQueue, flush_operation)
{
    auto worker = [](int x)
    {
...
XSIGMATEST(ThreadedTaskQueue, buffer_size_limit)
{
    auto worker = [](int x)
    {
...
XSIGMATEST(ThreadedTaskQueue, non_strict_ordering)
{
...
XSIGMATEST(ThreadedTaskQueue, void_task_execution)
{
    std::atomic<int> counter{0};
...
XSIGMATEST(ThreadedTaskQueue, void_task_multiple_args)
{
...
XSIGMATEST(ThreadedTaskQueue, void_task_is_empty)
{
    std::atomic<int> counter{0};
...
XSIGMATEST(ThreadedTaskQueue, concurrent_execution)
{
...
XSIGMATEST(ThreadedTaskQueue, thread_safety)
{
...
XSIGMATEST(ThreadedTaskQueue, many_threads)
{
    std::atomic<int> counter{0};
...
XSIGMATEST(ThreadedTaskQueue, single_thread)
{
    auto                          worker = [](int x) { return x * 2; };
...
XSIGMATEST(ThreadedTaskQueue, pop_empty_queue)
{
    auto                          worker = [](int x) { return x; };
    threaded_task_queue<int, int> queue(worker, true, -1, 2);
...
XSIGMATEST(ThreadedTaskQueue, flush_empty_queue)
{
    auto                          worker = [](int x) { return x; };
    threaded_task_queue<int, int> queue(worker, true, -1, 2);
...
XSIGMATEST(ThreadedTaskQueue, complex_return_type)
{
...
XSIGMATEST(ThreadedTaskQueue, stress_test)
{
    auto                          worker = [](int x) { return x; };
    threaded_task_queue<int, int> queue(worker, true, -1, 4);
...
XSIGMATEST(ThreadedTaskQueue, void_task_buffer_size)
{
    std::atomic<int> counter{0};
...
XSIGMATEST(ThreadedTaskQueue, mixed_speed_tasks)
{
    auto worker = [](int x)
    {
        if (x % 2 == 0)
