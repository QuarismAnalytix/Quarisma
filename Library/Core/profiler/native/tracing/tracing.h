...
QUARISMA_API const char* get_event_category_name(event_category category);

// Interface for CPU profiler events.
class QUARISMA_VISIBILITY event_collector
{
public:
...
private:
    friend QUARISMA_API void set_event_collector(
        event_category category, const event_collector* collector);
    friend QUARISMA_API const event_collector* get_event_collector(event_category category);
...
QUARISMA_API void set_event_collector(event_category category, const event_collector* collector);

// Returns the active collector for the category if tracing is enabled.
QUARISMA_API const event_collector* get_event_collector(event_category category);

// Utility helpers for generating identifiers passed to collectors.
QUARISMA_API uint64_t get_unique_arg();
QUARISMA_API uint64_t get_arg_for_name(std::string_view name);

// Records an instant event through the registered collector.
QUARISMA_API void record_event(event_category category, uint64_t arg);

// Records a region through the registered collector for the lifetime of the instance.
class QUARISMA_VISIBILITY scoped_region
{
public:
...
QUARISMA_API const char* get_log_dir();
