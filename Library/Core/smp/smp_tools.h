...
struct smp_tools_range_functor<Iterator, Functor, false>
{
  Functor& F;
  Iterator& Begin;
  smp_tools_range_functor(Iterator& begin, Functor& f)
    : F(f)
    , Begin(begin)
  {
  }
...
public:
    ...
    Iter begin, Iter end, size_t grain, Functor& f)
  {
    ...
    typename conductor::detail::smp::smp_tools_lookup_range_for<Iter, Functor>::type fi(begin, f);
  ...
  {
    size_t size = std::distance(begin, end);
    typename conductor::detail::smp::smp_tools_lookup_range_for<Iter, Functor const>::type fi(begin, f);
    smp_tools::For(0, size, grain, fi);
  }
  ///@}
  ...
  template <typename Iter, typename Functor>
  static conductor::detail::smp::resolved_not_int<Iter> For(Iter begin, Iter end, Functor const& f)
  {
    smp_tools::For(begin, end, 0, f);
  }
  ///@}
  ...
  struct config
  {
    ...
    config() = default;
    config(int max_num_threads)
      : max_number_of_threads(max_num_threads)
    {
    }
    config(std::string backend_name)
