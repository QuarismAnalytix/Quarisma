 ...
 * executed.
 *
 * When a task is pushed, a `quarismaSharedFuture` is returned. This instance can be used to get the
...
#ifndef quarismaThreadedCallbackQueue_h
#define quarismaThreadedCallbackQueue_h
...
public:
   ...
   */
    ...
    /**
   * `quarismaSharedFutureBase` is the base block to store, run, get the returned value of the tasks that
   * are pushed in the queue.
   */
    class quarismaSharedFutureBase
    {
    public:
        /**
         * @brief Default constructor
         ...
         */
        quarismaSharedFutureBase() : NumberOfPriorSharedFuturesRemaining(0), Status(CONSTRUCTING) {}
         ...
         */
        virtual ~quarismaSharedFutureBase() = default;
     ...
     */
        std::vector<ptr_mutable<quarismaSharedFutureBase>> Dependents;

        mutable std::mutex              Mutex;
        mutable std::condition_variable ConditionVariable;

        quarismaSharedFutureBase(const quarismaSharedFutureBase& other) = delete;
        void operator=(const quarismaSharedFutureBase& other)         = delete;
    };

    /**
   * A `quarismaSharedFuture` is an object returned by the methods `Push` and `PushDependent`.
   */
    template <class ReturnT>
    class quarismaSharedFuture : public quarismaSharedFutureBase
    {
    public:
    ...
    private:
        ...
        quarismaSharedFuture(const quarismaSharedFuture<ReturnT>& other) = delete;
        void operator=(const quarismaSharedFuture<ReturnT>& other)     = delete;
    };

    using SharedFutureBasePointer = ptr_mutable<quarismaSharedFutureBase>;
    template <class ReturnT>
    using SharedFuturePointer = ptr_mutable<quarismaSharedFuture<ReturnT>>;
   ...
   * This method returns a `quarismaSharedFuture`, which is an object
   ...
   * pass rvalue references or smart pointers (`quarismaSmartPointer` or `std::shared_ptr` for example)
   *
   ...
   *   void f(S&&, const S&) {}
   *
   *   quarismaNew<threaded_callback_queue> queue;
   *   int x;
   *   S s;
   *
   ...
   */
    template <class ReturnT>
    typename quarismaSharedFuture<ReturnT>::ReturnLValueRef Get(SharedFuturePointer<ReturnT>& future);
    template <class ReturnT>
    typename quarismaSharedFuture<ReturnT>::ReturnConstLValueRef Get(
...
private:
    ///@{
    /**
   * A `quarismaInvoker` subclasses `quarismaSharedFuture`. It provides storage and capabilities to run the
   * input function with the given parameters.
   */
    template <class FT, class... ArgsT>
    class quarismaInvoker;
    ///@}

    struct InvokerImpl;

    template <class FT, class... ArgsT>
    using InvokerPointer = quarismaInvoker<FT, ArgsT...>;
   ...
   */
    void SignalDependentSharedFutures(ptr_mutable<quarismaSharedFutureBase> invoker);
   ...
   */
    void Invoke(ptr_mutable<quarismaSharedFutureBase> invoker);
   ...
   */
    bool TryInvoke(ptr_mutable<quarismaSharedFutureBase> invoker);
   ...
   */
    quarisma_map<std::thread::id, std::shared_ptr<std::atomic_int>> ThreadIdToIndex;
...
#endif
#endif
// QUARISMA-HeaderTest-Exclude: threaded_callback_queue.h
