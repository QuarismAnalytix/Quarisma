...
public:
  ...
  public:
    ...
    using return_const_lvalue_ref = typename return_value_wrapper<ReturnT>::return_const_lvalue_ref;
   ...
   */
  template <class ReturnT>
  XSIGMA_API typename shared_future<ReturnT>::return_lvalue_ref get(shared_future_pointer<ReturnT>& future);
  template <class ReturnT>
...
private:
  ...
  void handle_dependent_invoker(SharedFutureContainerT&& prior_shared_futures, InvokerT&& invoker);
...
struct threaded_callback_queue::return_value_wrapper<ReturnT, false /* IsLValueReference */>
{
  ...
  return_value_wrapper(ReturnTT&& value)
    : m_value(std::forward<ReturnTT>(value))
  {
  }
...
class threaded_callback_queue::invoker_impl::invoker_handle<true /* IsMemberFunctionPointer */, FT,
  ObjectT, ArgsT...>
{
public:
  ...
  invoke_result<FT> operator()() { return this->invoke_impl(make_integer_sequence<sizeof...(ArgsT)>()); }
...
//=============================================================================
template <class FT, class... ArgsT>
class threaded_callback_queue::invoker_impl::invoker_handle<false /* IsMemberFunctionPointer */, FT,
  ArgsT...>
{
public:
...
public:
  ...
  template <class... ArgsTT>
  invoker(ArgsTT&&... args)
    : m_impl(std::forward<ArgsTT>(args)...)
  {
  }

  void operator()() override
  {
    assert(this->m_status.load(std::memory_order_relaxed) == RUNNING && "Status should be RUNNING");
  ...
  if (!prior_shared_futures.empty())
  {
    for (const auto& prior : prior_shared_futures)
    {
      ...
      if (prior->m_status.load(std::memory_order_acquire) != READY)
      {
        ...
        lock.unlock();
        invoker->m_number_of_prior_shared_futures_remaining.fetch_add(1, std::memory_order_release);
      }
    }
  }
...
void threaded_callback_queue::wait(SharedFutureContainerT&& prior_shared_futures)
{
  ...
  auto empty_lambda = [] {};
  auto invoker_ptr = invoker_pointer<decltype(empty_lambda)>(invoker<decltype(empty_lambda)>::create(std::move(empty_lambda)));
  ...
  SharedFutureContainerT&& prior_shared_futures, FT&& f, ArgsT&&... args)
{
  ...
  auto invoker_ptr = invoker_pointer_type(invoker<FT, ArgsT...>::create(std::forward<FT>(f), std::forward<ArgsT>(args)...));

  this->push(
    &threaded_callback_queue::handle_dependent_invoker<SharedFutureContainerT, invoker_pointer_type>,
    this, prior_shared_futures_r, invoker_ptr);
...
void threaded_callback_queue::push_control(FT&& f, ArgsT&&... args)
{
  ...
  auto invoker_ptr = invoker_pointer_type(invoker<worker, threaded_callback_queue*, FT, ArgsT...>::create(
