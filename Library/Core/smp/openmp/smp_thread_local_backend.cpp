...
{
public:
  lock_guard(omp_lock_t& lock, bool wait)
    : m_lock(lock)
    , m_status(0)
  {
    if (wait)
...
static slot* lookup_slot(hash_table_array* array, thread_id_type thread_id, size_t hash)
{
  ...
  for (size_t idx = hash & mask;; idx = (idx + 1) & mask) // linear probing
  {
    ...
    {
      slot_ptr = lookup_slot(array->prev, thread_id, hash);
      break;
    }
    ...
    if (!slot_thread_id)                                 // unused?
    {
      ...
      if (lguard.success())
      {
        ...
        if (!slot_ptr->thread_id.load()) // not acquired in the meantime?
        {
          ...
          slot* prev_slot = lookup_slot(array->prev, thread_id, hash);
          if (prev_slot)
          {
  ...
  : m_count(0)
{
  ...
  m_root = new hash_table_array(init_size_lg);
}

thread_specific::~thread_specific()
{
  hash_table_array* array = m_root;
  while (array)
  {
    hash_table_array* to_free = array;
    array = array->prev;
    delete to_free;
  }
}
...
#pragma omp critical(HashTableResize)
      if (m_root == array)
      {
        hash_table_array* new_array = new hash_table_array(array->size_lg + 1);
        new_array->prev = array;
        m_root.store(new_array); // atomic copy
      }
    }
