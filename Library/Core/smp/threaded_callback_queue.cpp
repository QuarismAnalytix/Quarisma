...
public:
  ...
  thread_worker(thread_worker&& other) noexcept
    ...
    , m_thread_index(std::move(other.m_thread_index))
  {
  }
...
private:
  ...
  bool on_hold() const
  {
    return *m_thread_index < m_queue->m_number_of_threads &&
      !m_queue->m_destroying.load(std::memory_order_acquire) && m_queue->m_invoker_queue.empty();
  }
  ...
  this->push_control(
    [this, number_of_threads]()
    {
      ...
      else if (size < number_of_threads)
      {
        ...
        std::generate_n(std::back_inserter(m_threads), number_of_threads - size,
          [this]
          {
            auto thread_index =
      ...
      {
        {
          ...
          std::atomic_int& thread_index = *m_thread_id_to_index.at(std::this_thread::get_id());
          if (thread_index && thread_index >= number_of_threads)
          {
            std::atomic_int& thread0_index = *m_thread_id_to_index.at(m_threads[0].get_id());
            lock.unlock();
...
{
  std::for_each(m_threads.begin() + start_id, m_threads.end(),
...
void threaded_callback_queue::signal_dependent_shared_futures(shared_future_base* invoker)
{
  ...
  if (!invokers_to_launch.empty())
  {
    ...
    {
      assert(inv->m_status.load(std::memory_order_acquire) == ON_HOLD && "Status should be ON_HOLD");
