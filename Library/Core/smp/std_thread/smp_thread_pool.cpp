...
smp_thread_pool::proxy::proxy(std::unique_ptr<proxy_data>&& data)
...
void smp_thread_pool::proxy::join()
{
  ...
  else
  {
    ...
    while (true)
    {
      ...
      auto it = std::find_if(thread_data_ref.m_jobs.begin(), thread_data_ref.m_jobs.end(),
      ...
      const auto job_index = static_cast<std::size_t>(std::distance(thread_data_ref.m_jobs.begin(), it));
...
std::size_t smp_thread_pool::get_thread_id() const noexcept
{
  ...
  if (thread_data_ptr)
  {
    ...
    auto& proxy_threads = thread_data_ptr->m_jobs[thread_data_ptr->m_running_job].m_proxy->m_threads;
    lock.unlock();
...
bool smp_thread_pool::get_single_thread() const
{
  ...
  if (thread_data_ptr)
  {
    ...
    assert(thread_data_ptr->m_running_job != no_running_job && "Invalid state");
    return thread_data_ptr->m_jobs[thread_data_ptr->m_running_job].m_proxy->m_threads[0].m_thread == thread_data_ptr;
  }
  ...
  return std::thread{ [this]()
    {
      ...
      while (true)
      {
        ...
        thread_data_ref.m_condition_variable.wait(lock,
          [this, &thread_data_ref]
          { return !thread_data_ref.m_jobs.empty() || m_joining.load(std::memory_order_acquire); });
