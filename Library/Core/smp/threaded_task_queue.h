...
{
public:
  XSIGMA_API threaded_task_queue(std::function<R(Args...)> worker, bool strict_ordering = true,
...
{
public:
  XSIGMA_API threaded_task_queue(std::function<void(Args...)> worker, bool strict_ordering = true,
    int buffer_size = -1, int max_concurrent_tasks = -1);
  XSIGMA_API ~threaded_task_queue();
...
{
public:
  result_queue(bool strict_ordering)
    : m_next_result_id(0)
    , m_strict_ordering(strict_ordering)
  {
  }
...
threaded_task_queue<R, Args...>::threaded_task_queue(
  ...
  , m_number_of_threads(max_concurrent_tasks <= 0 ? multi_threader::get_global_default_number_of_threads()
...
//-----------------------------------------------------------------------------
template <typename... Args>
threaded_task_queue<void, Args...>::threaded_task_queue(std::function<void(Args...)> worker,
  ...
  auto f = [this](int thread_id)
  {
    while (true)
    {
      ...
      if (m_tasks->pop(task_id, task))
      {
        ...
        m_next_result_id = std::max(static_cast<std::uint64_t>(m_next_result_id), task_id + 1);
        lk.unlock();
