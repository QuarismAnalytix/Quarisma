...
{
public:
    smp_thread_local() : m_internal() {}

    explicit smp_thread_local(const T& exemplar) : m_internal(exemplar) {}

    T& local() { return m_internal.local(); }

    size_t size() const { return m_internal.size(); }
