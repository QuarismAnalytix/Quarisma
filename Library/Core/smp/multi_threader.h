 ...
 * 
 * This class serves as the foundational threading infrastructure for the QUARISMA
...
#include "common/macros.h"   // for QUARISMA_DELETE_COPY
#include "quarisma_threads.h"  // for QUARISMA_MAX_THREADS, QUARISMA_USE_PTHREADS

#if defined(QUARISMA_USE_PTHREADS)
...
// If QUARISMA_USE_PTHREADS is defined, then pthread_create() will be
...
// Defined in quarismaThreads.h:
//   QUARISMA_MAX_THREADS - Maximum number of threads supported
//   __QUARISMA_THREAD_RETURN_VALUE__ - Return value for thread functions
//   __QUARISMA_THREAD_RETURN_TYPE__ - Return type for thread functions

// Define platform-specific thread function types and thread ID types
#ifdef QUARISMA_USE_PTHREADS
typedef void* (*quarismaThreadFunctionType)(void*);  // NOLINT
typedef pthread_t quarismaThreadProcessIDType;
// #define __QUARISMA_THREAD_RETURN_VALUE__  nullptr
// #define __QUARISMA_THREAD_RETURN_TYPE__   void *
typedef pthread_t quarismaMultiThreaderIDType;
#endif
...
using quarismaWindowsDWORD                  = DWORD;
using quarismaWindowsPVOID                  = PVOID;
using quarismaWindowsLPVOID                 = LPVOID;
using quarismaWindowsHANDLE                 = HANDLE;
using quarismaWindowsLPTHREAD_START_ROUTINE = LPTHREAD_START_ROUTINE;
#else
// Define types from the windows header file.
using quarismaWindowsDWORD                  = unsigned long;
using quarismaWindowsPVOID                  = void*;
using quarismaWindowsLPVOID                 = quarismaWindowsPVOID;
using quarismaWindowsHANDLE                 = quarismaWindowsPVOID;
using quarismaWindowsLPTHREAD_START_ROUTINE = quarismaWindowsDWORD(__stdcall*)(quarismaWindowsLPVOID);
#endif

using quarismaThreadFunctionType  = quarismaWindowsLPTHREAD_START_ROUTINE;
using quarismaThreadProcessIDType = quarismaWindowsHANDLE;
// #define __QUARISMA_THREAD_RETURN_VALUE__ 0
// #define __QUARISMA_THREAD_RETURN_TYPE__ DWORD __stdcall
using quarismaMultiThreaderIDType = quarismaWindowsDWORD;
#endif
...
using quarismaThreadFunctionType  = void (*)(void*);
using quarismaThreadProcessIDType = int;
// #define __QUARISMA_THREAD_RETURN_VALUE__
// #define __QUARISMA_THREAD_RETURN_TYPE__ void
using quarismaMultiThreaderIDType = int;
#endif
...
public:
     ...
     * 
     * The number will be clamped to the range 1 - QUARISMA_MAX_THREADS.
     ...
     * @return Maximum thread count (defined by QUARISMA_MAX_THREADS)
     */
     ...
     * QUARISMA_MAX_THREADS in the configuration.
     * 
     ...
     * By default, it's set to the number of processors or QUARISMA_MAX_THREADS,
     * whichever is less.
     * 
     ...
     * 
     * @param f Function pointer to execute (must be of type quarismaThreadFunctionType)
     * @param data User data to pass to each thread (available in ThreadInfo)
     */
    void SetSingleMethod(quarismaThreadFunctionType f, void* data);
     ...
     */
    void SetMultipleMethod(int index, quarismaThreadFunctionType f, void* data);
     ...
     * @param data User data to pass to the thread
     * @return Thread ID for the new thread (0 to QUARISMA_MAX_THREADS-1)
     */
    int SpawnThread(quarismaThreadFunctionType f, void* data);
     ...
     */
    static quarismaMultiThreaderIDType GetCurrentThreadID();
     ...
     */
    static bool ThreadsEqual(quarismaMultiThreaderIDType t1, quarismaMultiThreaderIDType t2);
...
protected:
    ...
    ThreadInfo ThreadInfoArray[QUARISMA_MAX_THREADS];

    // Function pointers for thread methods
    quarismaThreadFunctionType SingleMethod;  // Function for single method execution
    quarismaThreadFunctionType
        MultipleMethod[QUARISMA_MAX_THREADS];  // Functions for multiple method execution

    // Storage for spawned threads
    int                       SpawnedThreadActiveFlag[QUARISMA_MAX_THREADS];      // Activity flags
    std::mutex*               SpawnedThreadActiveFlagLock[QUARISMA_MAX_THREADS];  // Mutex locks
    quarismaThreadProcessIDType SpawnedThreadProcessID[QUARISMA_MAX_THREADS];  // Thread process IDs
    ThreadInfo                SpawnedThreadInfoArray[QUARISMA_MAX_THREADS];  // Thread info structures

    // User data storage
    void* SingleData;                        // Data for single method execution
    void* MultipleData[QUARISMA_MAX_THREADS];  // Data for multiple method execution
